...
- Updated `includes/activation.php` so `vms_square_nightly_sync` now has a retired no-op callback, removes all WP-Cron argument variants, cancels pending Action Scheduler rows, deletes failed/canceled retired-hook history in bounded batches, retries only in safe maintenance contexts, and stores the completion marker only after the full Action Scheduler portion actually finishes.
