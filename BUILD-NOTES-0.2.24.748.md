...
## Purpose
Stabilize the unreleased public event sidebar and vendor profile follow-up work before PR #1 leaves draft by fixing the current-event sidebar guard, enforcing Vendor Type requirements at runtime, retiring the orphaned legacy Square nightly sync hook, and surfacing explicit Admissions session-expired guidance in the admin UI.
...
- Updated `includes/activation.php` so `vms_square_nightly_sync` is cleaned up from WP-Cron and Action Scheduler on activation/deactivation and once per site update without touching unrelated schedules.
- Updated `includes/modules/admissions/rest.php` and `assets/js/vms-admissions-admin.js` so expired Admissions REST nonces return explicit refresh guidance and the admin UI preserves that message instead of collapsing it into generic load/action failures.
- Added focused regression coverage in `tests/public-event-sidebar-guards.php` and `tests/legacy-square-sync-cleanup.php`, and extended `tests/admissions-rest-permissions.php` to assert the refreshed expired-session message.
