...
if (!class_exists('VMS_Tours_Service')) {
	class VMS_Tours_Service
	{
		...
		public function render_missing_tour_notice(): void
		{
			...
			echo '<p>' . esc_html__('No guided tour is registered for this screen yet.', 'vms') . ' <code>' . esc_html($screen_key) . '</code></p>';
			echo '</div>';
		}
		...
		public function ajax_save_prefs(): void
		{
			...
			$screen_key = isset($_POST['screen_key']) ? $this->sanitize_screen_key((string) wp_unslash($_POST['screen_key'])) : '';
			if (!current_user_can('manage_options')) {
		...
		public function ajax_mark_complete(): void
		{
			...
			$user_id = get_current_user_id();
			$tour_id = isset($_POST['tour_id']) ? $this->sanitize_tour_id((string) wp_unslash($_POST['tour_id'])) : '';
			$mode = isset($_POST['mode']) ? sanitize_key((string) wp_unslash($_POST['mode'])) : 'complete';
			$tour_version = isset($_POST['tour_version']) ? sanitize_text_field((string) wp_unslash($_POST['tour_version'])) : '';
		...
		private function register_core_tours(): void
		{
			$this->register_tour(array(
				...
				'title' => __('VMS Dashboard Basics', 'vms'),
				'screen' => 'admin:vms-dashboard',
				'version' => '1.0.0',
				'level' => 'beginner',
				'description' => __('Walk through the key dashboard areas and where to click next.', 'vms'),
				'audience' => array(
				...
				'steps' => array(
						array(
							...
							'title' => __('Scope Filters', 'vms'),
							'body' => wp_kses_post(__('Set venue and inclusion filters first so every dashboard panel reflects the right scope.', 'vms')),
							'placement' => 'bottom',
						...
						array(
							...
							'title' => __('Quick Actions', 'vms'),
							'body' => wp_kses_post(__('Use these shortcuts to jump into Event Plans, Schedule, Vendors, and Venues.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Financial Snapshot', 'vms'),
							'body' => wp_kses_post(__('Review this panel first for immediate money visibility.', 'vms')),
							'placement' => 'right',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('This Week', 'vms'),
						'body' => wp_kses_post(__('Track near-term operational load, then cross-check staffing and due dates.', 'vms')),
						'placement' => 'left',
						'guard' => array('type' => 'element_exists'),
					),
						array(
							...
							'title' => __('Staffing Readiness', 'vms'),
							'body' => wp_kses_post(__('Spot coverage gaps before event days arrive.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Start Help Quickly', 'vms'),
							'body' => wp_kses_post(__('Use this launcher to run the dashboard tour immediately whenever you need a refresher.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
					array(
						...
						'title' => __('Upcoming Bills', 'vms'),
						'body' => wp_kses_post(__('Use this panel to prevent missed payouts and late obligations.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
				),
				...
				'title' => __('Event Plan Editor Basics', 'vms'),
				'screen' => 'admin:vms_event_plan',
				'version' => '1.2.0',
				'level' => 'beginner',
				'description' => __('Cover the essential Event Plan editor controls before publishing.', 'vms'),
				'audience' => array(
				...
				'steps' => array(
					array(
						...
						'title' => __('Venue Selector', 'vms'),
						'body' => wp_kses_post(__('Choose the venue first because compensation defaults and availability checks depend on it.', 'vms')),
						'placement' => 'bottom',
						'guard' => array(
							'type' => 'field_is_empty',
							'selector' => '#vms_venue_id',
					...
					array(
						...
						'title' => __('Event Date', 'vms'),
						'body' => wp_kses_post(__('Set the event date so holiday rules and date defaults are applied correctly.', 'vms')),
						'placement' => 'bottom',
						'guard' => array(
							'type' => 'field_is_empty',
							'selector' => '#vms_event_date',
					...
					array(
						...
						'title' => __('Status and Publish Controls', 'vms'),
						'body' => wp_kses_post(__('Use Save Draft while iterating, then mark Ready before Publish Now.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('Compensation Section', 'vms'),
						'body' => wp_kses_post(__('Review and draft compensation here while you edit the plan. Base + Attendance Bonus now lives in this section alongside the existing structures, and locked pay should only happen after the basic event details have been saved.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'body' => wp_kses_post(__('Choose this structure when the event has a guaranteed base plus an attendance-driven payout. Operators can switch between Step and Continuous bonus styles, and can set an optional Max Bonus cap without faking door-split math.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('Attendance Preview', 'vms'),
						'body' => wp_kses_post(__('When Base + Attendance Bonus is selected, this preview shows the formula and projected payouts. If you set a Max Bonus cap, the preview extends progression out to the cap so the stopping point is obvious before you lock anything.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('Lock Draft Pay', 'vms'),
						'body' => wp_kses_post(__('Use Lock Draft Pay to freeze the current payout values after Date, Venue, Start Time, End Time, and Primary Vendor are saved. The locked snapshot is the payout source of truth, including any attendance-bonus terms.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('Staff Roles Area', 'vms'),
						'body' => wp_kses_post(__('Set staffing headcount and assignments so readiness reflects real coverage.', 'vms')),
						'placement' => 'top',
						'guard' => array('type' => 'element_exists'),
					),
					array(
						...
						'title' => __('Save Progress', 'vms'),
						'body' => wp_kses_post(__('Save Draft once the basics are in place, then return to compensation or status actions. This first save is what enables safe pay locking.', 'vms')),
						'placement' => 'left',
						'guard' => array('type' => 'element_exists'),
					),
					),
				...
				$this->register_tour(array(
					...
					'title' => __('Comp Package Editor Basics', 'vms'),
					'screen' => 'admin:vms_comp_package',
					'version' => '1.0.0',
					'level' => 'beginner',
					'description' => __('Set venue-scoped compensation packages, including the new attendance-bonus structure.', 'vms'),
					'audience' => array(
						'capabilities_any' => array('manage_options', 'edit_posts'),
						'capabilities_all' => array(),
						'roles_any' => array(),
					...
					'steps' => array(
						array(
							...
							'title' => __('Start Tour Anytime', 'vms'),
							'body' => wp_kses_post(__('Use this button whenever you want to relaunch the Comp Package editor tour.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Venue Scope', 'vms'),
							'body' => wp_kses_post(__('Comp Packages are usually venue-scoped. Pick the venue first so operators only see relevant package options on Event Plans.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Comp Type', 'vms'),
							'body' => wp_kses_post(__('Use Comp Type to choose Flat Fee, Door Split, Flat Fee + Door Split, or Base + Attendance Bonus. Attendance bonuses use explicit Step or Continuous progression.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Guaranteed Amount', 'vms'),
							'body' => wp_kses_post(__('This field becomes Base Pay when the package uses attendance bonuses. After you switch the type to Base + Attendance Bonus, Bonus Style, Bonus Starts After, Step or Continuous fields, and the optional Max Bonus cap appear below, and saving clears the opposite-mode fields so package terms stay canonical.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
					),
				...
				$this->register_tour(array(
					...
					'title' => __('Event Plans List Basics', 'vms'),
					'screen' => 'admin:edit-vms_event_plan',
					'version' => '1.0.0',
					'level' => 'beginner',
					'description' => __('Use filters, what-if status inclusion, and list tools to manage Event Plans quickly.', 'vms'),
					'audience' => array(
						'capabilities_any' => array('manage_options', 'edit_posts'),
						'capabilities_all' => array(),
						'roles_any' => array(),
					...
					'steps' => array(
						array(
							...
							'title' => __('Start Tour Anytime', 'vms'),
							'body' => wp_kses_post(__('Use this button whenever you want to relaunch the Event Plans list tour.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Include Draft/Ready What-If', 'vms'),
							'body' => wp_kses_post(__('Toggle Draft/Ready inclusion so planning what-if records can be reviewed alongside published plans.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Search by Title', 'vms'),
							'body' => wp_kses_post(__('Use search to quickly jump to a specific Event Plan before editing status or compensation details.', 'vms')),
							'placement' => 'left',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Plan Rows', 'vms'),
							'body' => wp_kses_post(__('Review status, readiness, and warning indicators directly in the list table before opening a plan.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Create a New Event Plan', 'vms'),
							'body' => wp_kses_post(__('Use Add New to open the Event Plan editor and build the next show record.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
					),
				...
				$this->register_tour(array(
					...
					'title' => __('Guided Tours Page Basics', 'vms'),
					'screen' => 'admin:vms-guided-tours',
					'version' => '1.0.0',
					'level' => 'beginner',
					'description' => __('Review global defaults, reset progress, and run tours by registry entry.', 'vms'),
					'audience' => array(
					...
					'steps' => array(
						array(
							...
							'title' => __('Global Defaults', 'vms'),
							'body' => wp_kses_post(__('These controls define global guided-tour defaults, including auto-run behavior and help button visibility.', 'vms')),
							'placement' => 'right',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Reset Your Progress', 'vms'),
							'body' => wp_kses_post(__('Reset clears your personal completed-tour state and dismissed auto-run flags.', 'vms')),
							'placement' => 'right',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Tour Registry', 'vms'),
							'body' => wp_kses_post(__('Run tours directly from this table and verify completion status/version for your account.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
					),
				...
				$this->register_tour(array(
					...
					'title' => __('Ticketing UI Settings', 'vms'),
					'screen' => 'admin:vms-settings',
					'version' => '1.0.1',
					'level' => 'beginner',
					'description' => __('Configure progressive ticket UI rollout, admin preview, add-on section labels, per-event overrides, and instant rollback behavior.', 'vms'),
					'audience' => array(
						'capabilities_any' => array('manage_options'),
						'capabilities_all' => array(),
						'roles_any' => array(),
					...
					'steps' => array(
						array(
							...
							'title' => __('What the Ticket UI Options Are', 'vms'),
							'body' => wp_kses_post(__('Progressive mode keeps all admission choices together in one card and tucks optional add-ons below while preserving existing business rules.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Admin Preview', 'vms'),
							'body' => wp_kses_post(__('Turn this on to preview V2 as an admin while public users remain in Safe Mode (TEC-only). Use the Progressive layout option when you are ready to test the new grouped flow publicly or per event.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Choose the Public Ticket Layout', 'vms'),
							'body' => wp_kses_post(__('Set Ticket UI Layout to Progressive to publish the streamlined Tickets + Add-ons flow for all users. The add-on section heading and subtext can be customized for each venue.', 'vms')),
							'placement' => 'bottom',
							'guard' => array('type' => 'element_exists'),
						),
						array(
							...
							'title' => __('Rollback Instantly', 'vms'),
							'body' => wp_kses_post(__('For instant rollback, switch layout to Safe Mode (TEC-only), disable Admin Preview, or force Legacy / Safe Mode on a single Event Plan.', 'vms')),
							'placement' => 'top',
							'guard' => array('type' => 'element_exists'),
						),
					),
