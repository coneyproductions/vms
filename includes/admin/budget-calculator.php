...
function bvmgr_budget_parse_money($raw): float
{
  $s = is_string($raw) ? $raw : '';
  $s = trim($s);
...
function bvmgr_budget_parse_percent($raw, float $min = 0.0, float $max = 100.0): float
{
  $v = bvmgr_budget_parse_money($raw);
  if ($v < $min) $v = $min;
  if ($v > $max) $v = $max;
  return $v;
}

function bvmgr_budget_fmt_money($v): string
{
  return '$' . number_format((float) $v, 2);
}

function bvmgr_budget_fmt_int($v): string
{
  return number_format((int) $v);
}

function bvmgr_budget_calculator_default_cost_items(): array
{
...
function bvmgr_budget_calculator_default_autoscale_items(): array
{
...
function bvmgr_budget_calculator_cost_profiles(): array
{
  // Built-in presets. No saving in v1.x.
  $base = bvmgr_budget_calculator_default_cost_items();
...
function bvmgr_budget_calculator_defaults(): array
{
  ...
  return array(
    ...
    'annual_include_drafts'  => bvmgr_budget_calculator_default_forecast_include_drafts(),

    'cost_profile'      => 'custom',
    'cost_items'        => bvmgr_budget_calculator_default_cost_items(),

    'autoscale_items'   => bvmgr_budget_calculator_default_autoscale_items(),
  );
}

function bvmgr_budget_calculator_default_forecast_include_drafts(): int
{
...
function bvmgr_budget_post_value(string $key, $default = '')
{
...
function bvmgr_budget_post_array(string $key): array
{
  // phpcs:ignore WordPress.Security.NonceVerification.Missing,WordPress.Security.NonceVerification.Recommended -- The calculator POST is nonce-gated in the page handler; this helper keeps repopulation reads localized.
  if (!isset($_POST[$key])) {
    return array();
...
function bvmgr_budget_request_method(): string
{
...
function bvmgr_budget_calculator_is_valid_ymd(string $ymd): bool
{
...
function bvmgr_budget_calculator_event_plan_ticket_stats_key(): string
{
...
function bvmgr_budget_calculator_collect_event_plans_for_year(int $year, bool $include_drafts): array
{
  $year = max(2000, min(2100, $year));
  $start_ymd = sprintf('%04d-01-01', $year);
  $end_ymd = sprintf('%04d-12-31', $year);
  $date_key = bvmgr_budget_calculator_event_plan_date_key();
  $status_key = bvmgr_budget_calculator_event_plan_status_key();

  bvmgr_budget_calculator_ensure_inclusion_helpers();
  ...
  foreach ((array) $plan_ids as $pid_raw) {
    ...
    if (!bvmgr_budget_calculator_is_valid_ymd($event_date)) continue;
...
function bvmgr_budget_calculator_get_plan_ticket_stats(int $plan_id): array
{
  ...
  $raw = get_post_meta($plan_id, bvmgr_budget_calculator_event_plan_ticket_stats_key(), true);
...
function bvmgr_budget_calculator_resolve_plan_band_pay(int $plan_id, string $event_date_ymd, float $fallback_band_pay): array
{
  ...
  if ($needs_flat && $flat_fee !== null && $flat_fee > 0) {
    ...
    if ($venue_id > 0 && bvmgr_budget_calculator_is_valid_ymd($event_date_ymd)) {
...
function bvmgr_budget_calculator_band_pay_source_label(string $source): string
{
...
function bvmgr_budget_calculator_ticket_source_label(string $source): string
{
...
function bvmgr_budget_calculator_progress_pct(float $value, float $goal): ?float
{
  if ($goal <= 0.0) return null;
  return ($value / $goal) * 100.0;
}

function bvmgr_budget_calculator_sanitize_cost_items(array $items): array
{
  $out = array();
  foreach ($items as $it) {
  ...
  if (empty($out)) $out = bvmgr_budget_calculator_default_cost_items();

  return $out;
}

function bvmgr_budget_calculator_sanitize_autoscale_items(array $items): array
{
  $out = array();
  ...
  if (empty($out)) $out = bvmgr_budget_calculator_default_autoscale_items();

  return $out;
}

function bvmgr_budget_calculator_read_input(array $defaults): array
{
  $in = $defaults;

  $in['mode'] = ('period' === sanitize_key((string) bvmgr_budget_post_value('mode', $defaults['mode']))) ? 'period' : 'single';

  $in['events_count'] = (int) bvmgr_budget_post_value('events_count', $defaults['events_count']);
  if ($in['events_count'] < 1) $in['events_count'] = 1;
  if ($in['events_count'] > 365) $in['events_count'] = 365;

  $in['tickets_sold']  = (int) bvmgr_budget_post_value('tickets_sold', $defaults['tickets_sold']);
  if ($in['tickets_sold'] < 0) $in['tickets_sold'] = 0;
  if ($in['tickets_sold'] > 1000000) $in['tickets_sold'] = 1000000;

  $in['ticket_price']  = bvmgr_budget_parse_money(bvmgr_budget_post_value('ticket_price', $defaults['ticket_price']));
  $in['attendance_percent'] = bvmgr_budget_parse_percent(bvmgr_budget_post_value('attendance_percent', $defaults['attendance_percent']), 0.0, 200.0);
  $in['bar_per_head']  = bvmgr_budget_parse_money(bvmgr_budget_post_value('bar_per_head', $defaults['bar_per_head']));
  $in['other_revenue'] = bvmgr_budget_parse_money(bvmgr_budget_post_value('other_revenue', $defaults['other_revenue']));

  $in['band_pay']      = bvmgr_budget_parse_money(bvmgr_budget_post_value('band_pay', $defaults['band_pay']));

  $in['fee_percent']   = bvmgr_budget_parse_percent(bvmgr_budget_post_value('fee_percent', $defaults['fee_percent']), 0.0, 100.0);
  $in['fee_fixed']     = bvmgr_budget_parse_money(bvmgr_budget_post_value('fee_fixed', $defaults['fee_fixed']));

  $in['target_profit'] = bvmgr_budget_parse_money(bvmgr_budget_post_value('target_profit', $defaults['target_profit']));

  $in['forecast_year'] = (int) bvmgr_budget_post_value('forecast_year', $defaults['forecast_year']);
  if ($in['forecast_year'] < 2000) $in['forecast_year'] = 2000;
  if ($in['forecast_year'] > 2100) $in['forecast_year'] = 2100;

  $in['annual_goal_revenue'] = bvmgr_budget_parse_money(bvmgr_budget_post_value('annual_goal_revenue', $defaults['annual_goal_revenue']));
  $in['annual_goal_profit']  = bvmgr_budget_parse_money(bvmgr_budget_post_value('annual_goal_profit', $defaults['annual_goal_profit']));
  $in['annual_include_drafts'] = !empty(bvmgr_budget_post_value('annual_include_drafts')) ? 1 : 0;
  ...
  $profiles = bvmgr_budget_calculator_cost_profiles();

  $in['cost_profile'] = sanitize_key((string) bvmgr_budget_post_value('cost_profile', $defaults['cost_profile']));
  if (!isset($profiles[$in['cost_profile']])) $in['cost_profile'] = 'custom';

  $action = sanitize_key((string) bvmgr_budget_post_value('vms_budget_action', ''));

  // Read cost items from POST if present (regardless of profile), so changing one input does not wipe custom rows.
  $enableds = bvmgr_budget_post_array('cost_enabled');
  $labels   = bvmgr_budget_post_array('cost_label');
  $amounts  = bvmgr_budget_post_array('cost_amount');
  $types    = bvmgr_budget_post_array('cost_type');
  ...
  if ($max_rows > 0) {
    ...
    for ($i = 0; $i < $max_rows; $i++) {
      $items[] = array(
        ...
        'amount'  => bvmgr_budget_parse_money($amounts[$i] ?? 0),
        'type'    => isset($types[$i]) ? (string) $types[$i] : 'fixed',
      );
    }
    $in['cost_items'] = bvmgr_budget_calculator_sanitize_cost_items($items);
  } else {
    $in['cost_items'] = bvmgr_budget_calculator_sanitize_cost_items($profiles[$in['cost_profile']]['items']);
  }

  // Read auto-scaling items from POST (or default).
  $a_enabled = bvmgr_budget_post_array('auto_enabled');
  $a_label   = bvmgr_budget_post_array('auto_label');
  $a_unit    = bvmgr_budget_post_array('auto_unit_cost');
  $a_pern    = bvmgr_budget_post_array('auto_per_n');
  $a_min     = bvmgr_budget_post_array('auto_min_units');
  $a_max     = bvmgr_budget_post_array('auto_max_units');
  ...
  if ($a_rows > 0) {
    ...
    for ($i = 0; $i < $a_rows; $i++) {
      $items[] = array(
        ...
        'unit_cost' => bvmgr_budget_parse_money($a_unit[$i] ?? 0),
        'per_n'     => (int) ($a_pern[$i] ?? 0),
        'min_units' => (int) ($a_min[$i] ?? 0),
        'max_units' => (int) ($a_max[$i] ?? 0),
      );
    }
    $in['autoscale_items'] = bvmgr_budget_calculator_sanitize_autoscale_items($items);
  } else {
    $in['autoscale_items'] = bvmgr_budget_calculator_sanitize_autoscale_items($defaults['autoscale_items']);
  }
...
function bvmgr_budget_calculator_cost_totals(array $items): array
{
  $fixed = 0.0;
  $per_ticket = 0.0;
...
function bvmgr_budget_calculator_compute_autoscale(array $autoscale_items, int $headcount): array
{
  $total = 0.0;
  $rows = array();
...
function bvmgr_budget_calculator_compute_core(array $in): array
{
  ...
  $totals = bvmgr_budget_calculator_cost_totals($in['cost_items']);
  ...
  $auto = bvmgr_budget_calculator_compute_autoscale($in['autoscale_items'], $headcount);
...
function bvmgr_budget_calculator_compute(array $in): array
{
  // Wrapper that adds “tickets needed” values.
  // This must not be inside vms_budget_calculator_compute_core() to avoid recursion.
  $r = bvmgr_budget_calculator_compute_core($in);

  $r['tickets_needed_be'] = bvmgr_budget_calculator_find_tickets_for_profit($in, 0.0, 5000);
  $r['tickets_needed_target'] = bvmgr_budget_calculator_find_tickets_for_profit($in, (float) $in['target_profit'], 5000);

  return $r;
}

function bvmgr_budget_calculator_profit_for_tickets(array $in, int $tickets): array
{
  $tmp = $in;
  $tmp['tickets_sold'] = max(0, $tickets);
  return bvmgr_budget_calculator_compute_core($tmp);
}

function bvmgr_budget_calculator_ticket_price_needed_for(array $in, int $tickets, float $target_profit): ?float
{
  $tmp = $in;
  ...
  $totals = bvmgr_budget_calculator_cost_totals($tmp['cost_items']);
  $fixed_costs_other = $totals['fixed'];
  $per_ticket_costs  = $totals['per_ticket'];
  ...
  $auto = bvmgr_budget_calculator_compute_autoscale($tmp['autoscale_items'], $headcount);
  $autoscale_costs = (float) $auto['total'];
...
function bvmgr_budget_calculator_find_tickets_for_profit(array $in, float $target_profit, int $max_tickets = 5000): ?int
{
  ...
  for ($t = 0; $t <= $max_tickets; $t++) {
    $r = bvmgr_budget_calculator_profit_for_tickets($in, $t);
...
function bvmgr_budget_calculator_collect_actual_ticket_revenue(int $year, bool $include_drafts, string $end_ymd = ''): array
{
  $rows = bvmgr_budget_calculator_collect_event_plans_for_year($year, $include_drafts);
  $has_end = bvmgr_budget_calculator_is_valid_ymd($end_ymd);
  ...
  foreach ($rows as $row) {
    ...
    $events_total++;
    $stats = bvmgr_budget_calculator_get_plan_ticket_stats((int) ($row['plan_id'] ?? 0));
...
function bvmgr_budget_calculator_compute_annual_forecast(array $in): array
{
  ...
  $plans = bvmgr_budget_calculator_collect_event_plans_for_year($year, $include_drafts);
  ...
  foreach ($plans as $plan) {
    ...
    $band_pay = bvmgr_budget_calculator_resolve_plan_band_pay($plan_id, $event_date, (float) ($in['band_pay'] ?? 0.0));
    if (($band_pay['source'] ?? '') !== 'input_assumption') {
      $events_using_plan_band_pay++;
    }

    $stats = bvmgr_budget_calculator_get_plan_ticket_stats($plan_id);
    ...
    $model = bvmgr_budget_calculator_compute_core($tmp);
    ...
    $rows[] = array(
      ...
      'band_pay_source_label' => bvmgr_budget_calculator_band_pay_source_label((string) ($band_pay['source'] ?? 'input_assumption')),
      'ticket_revenue' => $ticket_revenue,
      'ticket_source' => $ticket_source,
      'ticket_source_label' => bvmgr_budget_calculator_ticket_source_label($ticket_source),
      'gross_revenue' => $gross_revenue,
      'net_profit' => $net_profit,
    );
  }
  ...
  $revenue_goal_progress_pct = bvmgr_budget_calculator_progress_pct($forecast_gross_revenue, $goal_revenue);
  $profit_goal_progress_pct = bvmgr_budget_calculator_progress_pct($forecast_net_profit, $goal_profit);
  ...
  $prior_full = bvmgr_budget_calculator_collect_actual_ticket_revenue($prior_year, $include_drafts);
  ...
  if ($year === $current_year) {
    try {
      ...
      $prior_to_date = bvmgr_budget_calculator_collect_actual_ticket_revenue($prior_year, $include_drafts, $prior_cutoff);
    } catch (Throwable $e) {
      $prior_to_date = array('revenue' => 0.0, 'events_with_revenue' => 0, 'events_total' => 0);
    }
  }
...
function bvmgr_budget_calculator_field_money(string $name, string $label, $value): void
{
  echo '<p>';
...
function bvmgr_budget_calculator_field_int(string $name, string $label, $value, int $min = 0, int $max = 1000000): void
{
  echo '<p>';
  echo '<label for="' . esc_attr($name) . '"><strong>' . esc_html($label) . '</strong></label><br>';
...
function bvmgr_budget_calculator_field_select(string $name, string $label, string $value, array $options): void
{
  echo '<p>';
  echo '<label for="' . esc_attr($name) . '"><strong>' . esc_html($label) . '</strong></label><br>';
...
function bvmgr_budget_calculator_render_cost_items(array $items): void
{
...
function bvmgr_budget_calculator_render_autoscale_items(array $items): void
{
  $rows = $items;
  while (count($rows) < 8) {
...
function bvmgr_render_budget_calculator_page(): void
{
  ...
  if ('POST' === bvmgr_budget_request_method()) {
    ...
    $action = sanitize_key((string) bvmgr_budget_post_value('vms_budget_action', ''));
    if ($action === 'calculate') {
      $results  = bvmgr_budget_calculator_compute($in);
      $annual_results = bvmgr_budget_calculator_compute_annual_forecast($in);
      $did_calc = true;
    }
  } else {
  ...
  bvmgr_budget_calculator_field_int('tickets_sold', __('Tickets sold (estimate)', 'backstage-venue-manager'), $in['tickets_sold'], 0, 1000000);
  bvmgr_budget_calculator_field_money('ticket_price', __('Ticket price', 'backstage-venue-manager'), $in['ticket_price']);
  ...
  bvmgr_budget_calculator_field_money('bar_per_head', __('Bar revenue per head', 'backstage-venue-manager'), $in['bar_per_head']);
  bvmgr_budget_calculator_field_money('other_revenue', __('Other revenue (per event)', 'backstage-venue-manager'), $in['other_revenue']);

  echo '<hr>';

  echo '<h3>' . esc_html__('Music Vendor / talent', 'backstage-venue-manager') . '</h3>';
  bvmgr_budget_calculator_field_money('band_pay', __('Music Vendor pay (per event)', 'backstage-venue-manager'), $in['band_pay']);
  bvmgr_budget_calculator_field_money('target_profit', __('Target profit (per event)', 'backstage-venue-manager'), $in['target_profit']);

  echo '<hr>';

  echo '<h3>' . esc_html__('Fees (ticket revenue only)', 'backstage-venue-manager') . '</h3>';
  bvmgr_budget_calculator_field_money('fee_fixed', __('Fee per ticket', 'backstage-venue-manager'), $in['fee_fixed']);
  bvmgr_budget_calculator_field_money('fee_percent', __('Fee percent (e.g., 2.9)', 'backstage-venue-manager'), $in['fee_percent']);

  echo '<hr>';

  echo '<h3>' . esc_html__('Annual goals + live forecast', 'backstage-venue-manager') . '</h3>';
  bvmgr_budget_calculator_field_int('forecast_year', __('Forecast year', 'backstage-venue-manager'), $in['forecast_year'], 2000, 2100);
  bvmgr_budget_calculator_field_money('annual_goal_revenue', __('Annual revenue goal', 'backstage-venue-manager'), $in['annual_goal_revenue']);
  bvmgr_budget_calculator_field_money('annual_goal_profit', __('Annual net profit goal', 'backstage-venue-manager'), $in['annual_goal_profit']);
  ...
  bvmgr_budget_calculator_field_select('cost_profile', __('Cost profile (preset)', 'backstage-venue-manager'), $in['cost_profile'], $profile_options);
  ...
  bvmgr_budget_calculator_render_cost_items($in['cost_items']);
  ...
  bvmgr_budget_calculator_render_autoscale_items($in['autoscale_items']);
  echo '</details>';
  ...
  if ($did_calc && is_array($results)) {
    ...
    echo '<p><strong>' . esc_html__('Expected headcount:', 'backstage-venue-manager') . '</strong> ' . esc_html(bvmgr_budget_fmt_int($headcount)) . '</p>';

    // Per-event summary
    echo '<h3>' . esc_html__('Per event', 'backstage-venue-manager') . '</h3>';
    echo '<table class="widefat striped">';
    echo '<tbody>';
    echo '<tr><th>' . esc_html__('Gross revenue', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['gross_revenue'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Fees (estimated)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['fee_percent_amt'] + $results['fee_fixed_amt'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Other costs (fixed)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['fixed_costs_other'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Other costs (per-ticket)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['variable_costs'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Auto-scaling costs (staff rules)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['autoscale_costs'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Music Vendor pay', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($in['band_pay'])) . '</td></tr>';
    echo '<tr><th>' . esc_html__('Net profit', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money($results['net_profit'])) . '</strong></td></tr>';
    echo '</tbody>';
    echo '</table>';
    ...
    if (!empty($results['autoscale_rows'])) {
      ...
      foreach ($results['autoscale_rows'] as $r) {
        ...
        echo '<td>' . esc_html(bvmgr_budget_fmt_int((int) $r['qty'])) . '</td>';
        echo '<td>' . esc_html(bvmgr_budget_fmt_money((float) $r['unit_cost'])) . '</td>';
        echo '<td><strong>' . esc_html(bvmgr_budget_fmt_money((float) $r['total'])) . '</strong></td>';
        echo '</tr>';
      }
    ...
    if ($in['mode'] === 'period' && $in['events_count'] > 1) {
      ...
      echo '<tr><th>' . esc_html__('Gross revenue', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['gross_revenue'] * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Fees (estimated)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money(($results['fee_percent_amt'] + $results['fee_fixed_amt']) * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Other costs (fixed)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['fixed_costs_other'] * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Other costs (per-ticket)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['variable_costs'] * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Auto-scaling costs (staff rules)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['autoscale_costs'] * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Music Vendor pay', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($in['band_pay'] * $k)) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Net profit', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money($results['net_profit'] * $k)) . '</strong></td></tr>';
      echo '</tbody></table>';
    }
    ...
    echo '<tr><th>' . esc_html__('Max Music Vendor pay (still hits target profit)', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money($results['max_band_pay_for_target'])) . '</strong></td></tr>';

    if ($results['ticket_price_needed'] !== null) {
      echo '<tr><th>' . esc_html__('Ticket price needed (hits target profit, at this ticket count)', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money($results['ticket_price_needed'])) . '</strong></td></tr>';
    } else {
    ...
    if ($results['break_even_price'] !== null) {
      echo '<tr><th>' . esc_html__('Break-even ticket price (at this ticket count)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money($results['break_even_price'])) . '</td></tr>';
    }

    if ($results['tickets_needed_target'] !== null) {
      echo '<tr><th>' . esc_html__('Tickets needed (hits target profit, at this ticket price)', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_int((int) $results['tickets_needed_target'])) . '</strong></td></tr>';
    } else {
    ...
    if ($results['tickets_needed_be'] !== null) {
      echo '<tr><th>' . esc_html__('Break-even tickets (at this ticket price)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) $results['tickets_needed_be'])) . '</td></tr>';
    } else {
    ...
    foreach ($ladder as $row) {
      ...
      $p_need = bvmgr_budget_calculator_ticket_price_needed_for($in, max(1, $t), (float) $in['target_profit']);

      $heads = isset($r['headcount']) ? (int) $r['headcount'] : 0;

      echo '<tr>';
      echo '<td>' . esc_html($row['label'] . ' = ' . (string) $t) . '</td>';
      echo '<td>' . esc_html(bvmgr_budget_fmt_int($heads)) . '</td>';
      echo '<td><strong>' . esc_html(bvmgr_budget_fmt_money($r['net_profit'])) . '</strong></td>';
      echo '<td>' . ($p_need === null ? esc_html__('Not possible', 'backstage-venue-manager') : esc_html(bvmgr_budget_fmt_money($p_need))) . '</td>';
      echo '</tr>';
    }
    ...
    if (is_array($annual_results)) {
      ...
      if ($rev_gap !== null) {
        ...
        if ($rev_gap_f > 0) $rev_gap_text = sprintf(__('Need %s more', 'backstage-venue-manager'), bvmgr_budget_fmt_money($rev_gap_f));
        /* translators: %s: currency amount above the revenue goal. */
        else $rev_gap_text = sprintf(__('Goal met (+%s)', 'backstage-venue-manager'), bvmgr_budget_fmt_money(abs($rev_gap_f)));
      }
      ...
      if ($profit_gap !== null) {
        ...
        if ($profit_gap_f > 0) $profit_gap_text = sprintf(__('Need %s more', 'backstage-venue-manager'), bvmgr_budget_fmt_money($profit_gap_f));
        /* translators: %s: currency amount above the profit goal. */
        else $profit_gap_text = sprintf(__('Goal met (+%s)', 'backstage-venue-manager'), bvmgr_budget_fmt_money(abs($profit_gap_f)));
      }
      ...
      echo '<div class="vms-bcalc-progress-value"><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['forecast_gross_revenue'] ?? 0.0))) . '</strong>';
      echo ' / ' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['goal_revenue'] ?? 0.0))) . '</div>';
      ...
      echo '<div class="vms-bcalc-progress-value"><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['forecast_net_profit'] ?? 0.0))) . '</strong>';
      echo ' / ' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['goal_profit'] ?? 0.0))) . '</div>';
      ...
      echo '<table class="widefat striped"><tbody>';
      echo '<tr><th>' . esc_html__('Included Event Plans', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['event_count'] ?? 0))) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Past / future plans', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['past_event_count'] ?? 0)) . ' / ' . bvmgr_budget_fmt_int((int) ($annual_results['future_event_count'] ?? 0))) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Projected gross revenue', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['forecast_gross_revenue'] ?? 0.0))) . '</strong></td></tr>';
      echo '<tr><th>' . esc_html__('Projected total costs', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['forecast_total_costs'] ?? 0.0))) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Projected net profit', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['forecast_net_profit'] ?? 0.0))) . '</strong></td></tr>';
      echo '<tr><th>' . esc_html__('Music Vendor pay using Event Plan values', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['events_using_plan_band_pay'] ?? 0))) . '</td></tr>';
      echo '</tbody></table>';

      echo '<h4>' . esc_html__('Past revenue context', 'backstage-venue-manager') . '</h4>';
      echo '<table class="widefat striped"><tbody>';
      echo '<tr><th>' . esc_html__('Past ticket revenue (actual snapshots)', 'backstage-venue-manager') . '</th><td><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['past_actual_ticket_revenue'] ?? 0.0))) . '</strong></td></tr>';
      echo '<tr><th>' . esc_html__('Past events with actual ticket stats', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['past_actual_ticket_events'] ?? 0))) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Past events missing ticket stats', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['past_missing_ticket_stats_events'] ?? 0))) . '</td></tr>';
      echo '<tr><th>' . esc_html__('Prior year actual ticket revenue (full year)', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['prior_year_actual_ticket_revenue'] ?? 0.0))) . ' (' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['prior_year_actual_ticket_events'] ?? 0))) . '/' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['prior_year_total_events'] ?? 0))) . ' ' . esc_html__('events with stats', 'backstage-venue-manager') . ')</td></tr>';
      if ($year === (int) wp_date('Y')) {
        echo '<tr><th>' . esc_html__('Prior year to-date ticket revenue', 'backstage-venue-manager') . '</th><td>' . esc_html(bvmgr_budget_fmt_money((float) ($annual_results['prior_year_to_date_actual_ticket_revenue'] ?? 0.0))) . ' (' . esc_html(bvmgr_budget_fmt_int((int) ($annual_results['prior_year_to_date_actual_ticket_events'] ?? 0))) . ' ' . esc_html__('events with stats', 'backstage-venue-manager') . ')</td></tr>';
      }
      echo '</tbody></table>';
      ...
      if (!empty($forecast_rows)) {
        ...
        foreach ($forecast_rows as $row) {
          ...
          echo '<td>' . esc_html(bvmgr_budget_fmt_money((float) ($row['band_pay'] ?? 0.0))) . '<br><span class="vms-bcalc-source">' . esc_html((string) ($row['band_pay_source_label'] ?? '')) . '</span></td>';
          echo '<td><span class="vms-bcalc-source">' . esc_html((string) ($row['ticket_source_label'] ?? '')) . '</span><br>' . esc_html(bvmgr_budget_fmt_money((float) ($row['ticket_revenue'] ?? 0.0))) . '</td>';
          echo '<td><strong>' . esc_html(bvmgr_budget_fmt_money((float) ($row['net_profit'] ?? 0.0))) . '</strong></td>';
          echo '</tr>';
        }
