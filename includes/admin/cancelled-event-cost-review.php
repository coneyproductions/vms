...
if (!function_exists('vms_cancelled_cost_review_add_metabox')) {
    function vms_cancelled_cost_review_add_metabox(): void
    {
        add_meta_box(
            ...
            __('Cancelled Event Cost Review', 'vms'),
...
if (!function_exists('vms_cancelled_cost_review_render_metabox')) {
    function vms_cancelled_cost_review_render_metabox(WP_Post $post): void
    {
        ...
        if ($event_plan_id <= 0) {
            echo '<p>' . esc_html__('No event plan loaded.', 'vms') . '</p>';
            return;
        }

        if (!vms_cancelled_cost_review_is_cancelled($event_plan_id)) {
            echo '<p>' . esc_html__('This review box appears when the event is marked Cancelled.', 'vms') . '</p>';
            return;
        }
        ...
        echo '<div class="notice notice-warning inline"><p><strong>' . esc_html__('Cancelled plans should review estimated costs.', 'vms') . '</strong></p>';
        if ($total_cents > 0) {
            ...
            echo '<p>' . esc_html__('No estimated labor or vendor/direct costs are currently loaded on this cancelled event.', 'vms') . '</p>';
        }
        echo '<p>' . esc_html__('Money changes are not auto-cleared here on purpose. Review the actual obligations first, then update the event plan accordingly.', 'vms') . '</p>';
        echo '<p><a class="button button-secondary" href="' . esc_url($profitability_url) . '">' . esc_html__('Open profitability board', 'vms') . '</a></p>';
        echo '</div>';
    }
}
