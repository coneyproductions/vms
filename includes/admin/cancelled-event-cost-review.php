...
if (!function_exists('bvmgr_cancelled_cost_review_is_cancelled')) {
    function bvmgr_cancelled_cost_review_is_cancelled(int $event_plan_id): bool
    {
...
if (!function_exists('bvmgr_cancelled_cost_review_vendor_direct_cents')) {
    function bvmgr_cancelled_cost_review_vendor_direct_cents(int $event_plan_id): int
    {
        if (function_exists('bvmgr_goals_get_default_direct_costs_cents')) {
            return max(0, (int) bvmgr_goals_get_default_direct_costs_cents($event_plan_id));
        }
...
if (!function_exists('bvmgr_cancelled_cost_review_labor_cents')) {
    function bvmgr_cancelled_cost_review_labor_cents(int $event_plan_id): int
    {
...
if (!function_exists('bvmgr_cancelled_cost_review_add_metabox')) {
    function bvmgr_cancelled_cost_review_add_metabox(): void
    {
        add_meta_box(
            'vms-cancelled-cost-review',
            __('Cancelled Event Cost Review', 'backstage-venue-manager'),
            'bvmgr_cancelled_cost_review_render_metabox',
            'vms_event_plan',
            'side',
            'high'
        );
...
add_action('add_meta_boxes_vms_event_plan', 'bvmgr_cancelled_cost_review_add_metabox', 30);

if (!function_exists('bvmgr_cancelled_cost_review_render_metabox')) {
    function bvmgr_cancelled_cost_review_render_metabox(WP_Post $post): void
    {
        ...
        if (!bvmgr_cancelled_cost_review_is_cancelled($event_plan_id)) {
            echo '<p>' . esc_html__('This review box appears when the event is marked Cancelled.', 'backstage-venue-manager') . '</p>';
            return;
        }

        $labor_cents = bvmgr_cancelled_cost_review_labor_cents($event_plan_id);
        $vendor_direct_cents = bvmgr_cancelled_cost_review_vendor_direct_cents($event_plan_id);
        ...
        if ($total_cents > 0) {
            ...
            echo '<li>' . esc_html(sprintf(__('Labor estimate: %s', 'backstage-venue-manager'), function_exists('bvmgr_goals_fmt_money') ? bvmgr_goals_fmt_money($labor_cents) : ('$' . number_format($labor_cents / 100, 2)))) . '</li>';
            /* translators: %s: Formatted vendor/direct estimate amount. */
            echo '<li>' . esc_html(sprintf(__('Vendor/direct estimate: %s', 'backstage-venue-manager'), function_exists('bvmgr_goals_fmt_money') ? bvmgr_goals_fmt_money($vendor_direct_cents) : ('$' . number_format($vendor_direct_cents / 100, 2)))) . '</li>';
            /* translators: %s: Formatted total loaded estimate amount. */
            echo '<li>' . esc_html(sprintf(__('Total still loaded: %s', 'backstage-venue-manager'), function_exists('bvmgr_goals_fmt_money') ? bvmgr_goals_fmt_money($total_cents) : ('$' . number_format($total_cents / 100, 2)))) . '</li>';
            echo '</ul>';
        } else {
