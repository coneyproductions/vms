...
if (!function_exists('vms_notify_render_user_profile_fields')) {
	function vms_notify_render_user_profile_fields(WP_User $user): void
	{
		...
		echo '<h2>' . esc_html__('VMS Notifications', 'vms') . '</h2>';
		echo '<table class="form-table" role="presentation">';
		echo '<tr>';
		echo '<th><label for="vms_locale_preference">' . esc_html__('Locale preference', 'vms') . '</label></th>';
		echo '<td>';
		echo '<select id="vms_locale_preference" name="vms_locale_preference">';
		echo '<option value="" ' . selected($locale_pref, '', false) . '>' . esc_html(sprintf(__('Use site locale (%s)', 'vms'), $site_locale)) . '</option>';
		...
		echo '<tr>';
		echo '<th>' . esc_html__('Channels', 'vms') . '</th>';
		echo '<td>';
		echo '<label><input type="checkbox" name="vms_notify_email_enabled" value="1" ' . checked($email_checked, true, false) . '> ' . esc_html__('Email', 'vms') . '</label><br>';
		echo '<label><input type="checkbox" name="vms_notify_sms_enabled" value="1" ' . checked($sms_checked, true, false) . '> ' . esc_html__('SMS', 'vms') . '</label><br>';
		echo '<label><input type="checkbox" name="vms_notify_whatsapp_enabled" value="1" ' . checked($wa_checked, true, false) . '> ' . esc_html__('WhatsApp', 'vms') . '</label>';
		echo '<p class="description">' . esc_html__('SMS/WhatsApp require a provider add-on and valid E.164 phone number.', 'vms') . '</p>';
		echo '</td>';
		echo '</tr>';

		echo '<tr>';
		echo '<th><label for="vms_phone_e164">' . esc_html__('Phone (E.164)', 'vms') . '</label></th>';
		...
		echo '<p class="description">' . esc_html__('Format: +{countrycode}{number}, e.g. +15551234567', 'vms') . '</p></td>';
		echo '</tr>';
		echo '</table>';
	}
}
