...
if (!function_exists('bvmgr_feedback_admin_register_page')) {
	function bvmgr_feedback_admin_register_page(): void
	{
		...
		bvmgr_register_admin_page(array(
			...
			'callback' => 'bvmgr_render_event_feedback_admin_page',
...
add_action('vms_admin_register_pages', 'bvmgr_feedback_admin_register_page', 30);

if (!function_exists('bvmgr_feedback_admin_enqueue_assets')) {
	function bvmgr_feedback_admin_enqueue_assets(string $hook): void
	{
		unset($hook);
		$page = sanitize_key(bvmgr_feedback_admin_query_arg('page'));
...
if (!function_exists('bvmgr_feedback_admin_redirect_url')) {
	function bvmgr_feedback_admin_redirect_url(int $event_plan_id = 0): string
	{
...
if (!function_exists('bvmgr_feedback_admin_query_arg')) {
	function bvmgr_feedback_admin_query_arg(string $key): string
	{
...
if (!function_exists('bvmgr_feedback_admin_has_query_arg')) {
	function bvmgr_feedback_admin_has_query_arg(string $key): bool
	{
...
if (!function_exists('bvmgr_feedback_admin_handle_save_settings')) {
	function bvmgr_feedback_admin_handle_save_settings(): void
	{
		...
		if (function_exists('bvmgr_feedback_save_notification_settings')) {
			bvmgr_feedback_save_notification_settings($enabled, $recipients);
		}
		wp_safe_redirect(add_query_arg('vms_feedback_settings_saved', '1', bvmgr_feedback_admin_redirect_url($event_plan_id)));
		exit;
	}
}
add_action('admin_post_vms_save_event_feedback_settings', 'bvmgr_feedback_admin_handle_save_settings');

if (!function_exists('bvmgr_feedback_admin_handle_delete_response')) {
	function bvmgr_feedback_admin_handle_delete_response(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
		...
		if ($response_id <= 0 || get_post_type($response_id) !== BVMGR_CPT_FEEDBACK_RESPONSE) {
			wp_safe_redirect(add_query_arg('vms_feedback_deleted', 'missing', bvmgr_feedback_admin_redirect_url($event_plan_id)));
			exit;
		}
		if (
		...
		if ($event_plan_id <= 0) {
			$event_plan_id = absint(get_post_meta($response_id, bvmgr_feedback_meta_key('event_plan_id'), true));
		}
		$deleted = wp_delete_post($response_id, true);
		wp_safe_redirect(add_query_arg('vms_feedback_deleted', $deleted ? '1' : '0', bvmgr_feedback_admin_redirect_url($event_plan_id)));
		exit;
	}
}
add_action('admin_post_vms_delete_event_feedback_response', 'bvmgr_feedback_admin_handle_delete_response');


if (!function_exists('bvmgr_feedback_admin_get_page_state')) {
	/**
	 * @return array{selected_event_plan_id:int,context:array<string,mixed>,show_missing_plan_notice:bool}
	 */
	function bvmgr_feedback_admin_get_page_state(): array
	{
		static $state = null;
		static $cache_key = null;

		$request_key = bvmgr_feedback_admin_has_query_arg('event_plan_id') ? wp_json_encode(bvmgr_feedback_admin_query_arg('event_plan_id')) : 'missing';
		if (!is_string($request_key)) {
			$request_key = bvmgr_feedback_admin_has_query_arg('event_plan_id') ? 'non-scalar' : 'missing';
		}
		...
		$selected_event_plan_id = absint(bvmgr_feedback_admin_query_arg('event_plan_id'));
		$context = array();
		$show_missing_plan_notice = false;

		if ($selected_event_plan_id > 0) {
			$context = bvmgr_feedback_get_event_context($selected_event_plan_id);
...
if (!function_exists('bvmgr_feedback_admin_render_notices')) {
	/**
	 * @param array{include_redirect_notices?:bool,include_missing_plan_notice?:bool,state?:array<string,mixed>} $args
	 */
	function bvmgr_feedback_admin_render_notices(array $args = array()): void
	{
		...
		if ($include_redirect_notices) {
			if (bvmgr_feedback_admin_query_arg('vms_feedback_settings_saved') !== '') {
			...
			if (bvmgr_feedback_admin_has_query_arg('vms_feedback_deleted')) {
				$status = sanitize_key(bvmgr_feedback_admin_query_arg('vms_feedback_deleted'));
				if ($status === '1') {
		...
		if ($include_missing_plan_notice) {
			$state = is_array($state) ? $state : bvmgr_feedback_admin_get_page_state();
...
if (!function_exists('bvmgr_feedback_admin_render_notification_settings')) {
	function bvmgr_feedback_admin_render_notification_settings(int $event_plan_id): void
	{
		$settings = function_exists('bvmgr_feedback_get_notification_settings') ? bvmgr_feedback_get_notification_settings() : array('enabled' => false, 'recipients' => array());
...
if (!function_exists('bvmgr_feedback_recent_event_plans')) {
	/**
	 * @return WP_Post[]
	 */
	function bvmgr_feedback_recent_event_plans(int $limit = 75): array
	{
...
if (!function_exists('bvmgr_feedback_admin_event_label')) {
	function bvmgr_feedback_admin_event_label(int $event_plan_id): string
	{
		$title = get_the_title($event_plan_id);
		$date = bvmgr_feedback_get_event_plan_date($event_plan_id);
		if ($date !== '') {
...
if (!function_exists('bvmgr_feedback_admin_render_event_selector')) {
	function bvmgr_feedback_admin_render_event_selector(int $selected_event_plan_id): void
	{
		$events = bvmgr_feedback_recent_event_plans();
		...
		foreach ($events as $event) {
			...
			echo '<option value="' . esc_attr((string) $event_id) . '"' . selected($selected_event_plan_id, $event_id, false) . '>' . esc_html(bvmgr_feedback_admin_event_label($event_id)) . '</option>';
		}
		echo '</select> ';
...
if (!function_exists('bvmgr_feedback_admin_rating_label')) {
	function bvmgr_feedback_admin_rating_label($rating): string
	{
		$rating = bvmgr_feedback_sanitize_rating($rating);
		if ($rating <= 0) {
			return '--';
		}
...
if (!function_exists('bvmgr_feedback_admin_average_label')) {
	function bvmgr_feedback_admin_average_label(array $avg): string
	{
...
if (!function_exists('bvmgr_feedback_admin_render_summary_card')) {
	/**
	 * @param WP_Post[] $responses
	 */
	function bvmgr_feedback_admin_render_summary_card(array $responses): void
	{
		$metrics = array(
		...
		foreach ($metrics as $label => $path) {
			$avg = bvmgr_feedback_average($responses, $path);
			echo '<div class="vms-feedback-metric"><span>' . esc_html((string) $label) . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label($avg)) . '</strong></div>';
		}
		echo '</div>';
		echo '</div>';
	}
...
if (!function_exists('bvmgr_feedback_admin_choice_label')) {
	function bvmgr_feedback_admin_choice_label(string $value): string
	{
		return bvmgr_feedback_admin_option_label($value, bvmgr_feedback_yes_maybe_no_options());
	}
}

if (!function_exists('bvmgr_feedback_admin_option_label')) {
	function bvmgr_feedback_admin_option_label(string $value, array $options): string
	{
...
if (!function_exists('bvmgr_feedback_admin_classify_saved_option_labels')) {
	/**
	 * @param string[] $values
	 * @return array{current:string[],legacy:string[],unknown:string[]}
	 */
	function bvmgr_feedback_admin_classify_saved_option_labels(array $values, array $current_options, array $legacy_options): array
	{
		$classified = array(
			'current' => array(),
			'legacy' => array(),
...
if (!function_exists('bvmgr_feedback_admin_payload_path_value')) {
	/**
	 * @return mixed
	 */
	function bvmgr_feedback_admin_payload_path_value(array $payload, string $path)
	{
		return function_exists('bvmgr_feedback_payload_path_value') ? bvmgr_feedback_payload_path_value($payload, $path) : null;
	}
}

if (!function_exists('bvmgr_feedback_admin_choice_counts')) {
	/**
	 * @param WP_Post[] $responses
	 * @return array<string,int>
	 */
	function bvmgr_feedback_admin_choice_counts(array $responses, string $path, ?array $allowed = null, ?callable $include_response = null): array
	{
		$allowed = (!empty($allowed) && is_array($allowed)) ? $allowed : bvmgr_feedback_yes_maybe_no_options();
		...
		foreach ($responses as $response) {
			...
			$value = sanitize_key((string) bvmgr_feedback_admin_payload_path_value($payload, $path));
...
if (!function_exists('bvmgr_feedback_admin_choice_counts_label')) {
	function bvmgr_feedback_admin_choice_counts_label(array $counts, ?array $options = null): string
	{
		$options = (!empty($options) && is_array($options)) ? $options : bvmgr_feedback_yes_maybe_no_options();
...
if (!function_exists('bvmgr_feedback_admin_vendor_has_detail_data')) {
	function bvmgr_feedback_admin_vendor_has_detail_data(array $vendor): bool
	{
		foreach (array('wait_time', 'friendliness', 'selection', 'value', 'quality', 'accuracy') as $key) {
			if (bvmgr_feedback_sanitize_rating($vendor[$key] ?? 0) > 0) {
				return true;
			}
		}
...
if (!function_exists('bvmgr_feedback_admin_render_website_summary')) {
	/**
	 * @param WP_Post[] $responses
	 */
	function bvmgr_feedback_admin_render_website_summary(array $responses): void
	{
		$usage_options = bvmgr_feedback_website_usage_options();
		$payment_issue_options = bvmgr_feedback_website_payment_issue_options();
		$website_detail_filter = static function (array $payload): bool {
			$website_used = sanitize_key((string) bvmgr_feedback_admin_payload_path_value($payload, 'website.website_used'));
			return function_exists('bvmgr_feedback_website_details_enabled') ? bvmgr_feedback_website_details_enabled($website_used) : ($website_used !== '' && $website_used !== 'did_not_use');
		};

		echo '<div class="vms-feedback-admin-card">';
		echo '<h2>' . esc_html__('Website / Ticket Purchase Experience', 'backstage-venue-manager') . '</h2>';
		echo '<p><strong>' . esc_html__('Website usage', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_counts_label(bvmgr_feedback_admin_choice_counts($responses, 'website.website_used', $usage_options), $usage_options)) . '</p>';
		echo '<p><strong>' . esc_html__('Reported issues', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_counts_label(bvmgr_feedback_admin_choice_counts($responses, 'website.website_payment_issues', $payment_issue_options, $website_detail_filter), $payment_issue_options)) . '</p>';
		echo '<div class="vms-feedback-metric-grid">';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Find Event', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label(bvmgr_feedback_average_filtered($responses, 'website.website_find_event', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Ticket Selection', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label(bvmgr_feedback_average_filtered($responses, 'website.website_ticket_selection', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Checkout', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label(bvmgr_feedback_average_filtered($responses, 'website.website_checkout_smoothness', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Confirmation', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label(bvmgr_feedback_average_filtered($responses, 'website.website_confirmation', $website_detail_filter))) . '</strong></div>';
		echo '</div>';
		echo '</div>';
	}
}

if (!function_exists('bvmgr_feedback_admin_render_primary_vendor_summary')) {
	/**
	 * @param WP_Post[] $responses
	 * @param array<string,mixed> $context
	 */
	function bvmgr_feedback_admin_render_primary_vendor_summary(array $responses, array $context): void
	{
		...
		echo '<div class="vms-feedback-metric-grid">';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Performance', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label(bvmgr_feedback_average($responses, 'primary_vendor.performance'))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Would like them back', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_feedback_admin_choice_counts_label(bvmgr_feedback_admin_choice_counts($responses, 'primary_vendor.bring_back'))) . '</strong></div>';
		echo '</div>';
		echo '</section>';
		echo '</div>';
	}
...
if (!function_exists('bvmgr_feedback_admin_render_vendor_summary')) {
	/**
	 * @param WP_Post[] $responses
	 * @param array<string,mixed> $context
	 */
	function bvmgr_feedback_admin_render_vendor_summary(array $responses, array $context): void
	{
		...
		foreach ($secondary as $vendor) {
			...
			$order_options = bvmgr_feedback_secondary_vendor_order_options();
			$ordered_filter = static function (array $payload) use ($vendor_id): bool {
				$did_order = sanitize_key((string) bvmgr_feedback_admin_payload_path_value($payload, 'secondary_vendors.' . $vendor_id . '.did_order'));
				if (function_exists('bvmgr_feedback_secondary_vendor_details_enabled') && bvmgr_feedback_secondary_vendor_details_enabled($did_order)) {
					return true;
				}
				if ($did_order === '') {
					$vendor_payload = bvmgr_feedback_admin_payload_path_value($payload, 'secondary_vendors.' . $vendor_id);
					return is_array($vendor_payload) && bvmgr_feedback_admin_vendor_has_detail_data($vendor_payload);
				}
				return $did_order === 'yes';
			};
			echo '<section class="vms-feedback-vendor-summary">';
			echo '<h3>' . esc_html((string) ($vendor['name'] ?? get_the_title($vendor_id))) . '</h3>';
			echo '<p><strong>' . esc_html__('Did you order from them?', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_counts_label(bvmgr_feedback_admin_choice_counts($responses, 'secondary_vendors.' . $vendor_id . '.did_order', $order_options), $order_options)) . '</p>';
			echo '<p><strong>' . esc_html__('Would bring back', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_counts_label(bvmgr_feedback_admin_choice_counts($responses, 'secondary_vendors.' . $vendor_id . '.bring_back', bvmgr_feedback_yes_maybe_no_options(), $ordered_filter))) . '</p>';
			echo '<div class="vms-feedback-metric-grid">';
			foreach ($paths as $label => $path) {
				$avg = bvmgr_feedback_average_filtered($responses, $path, $ordered_filter);
				echo '<div class="vms-feedback-metric"><span>' . esc_html((string) $label) . '</span><strong>' . esc_html(bvmgr_feedback_admin_average_label($avg)) . '</strong></div>';
			}
			echo '</div>';
			echo '</section>';
...
if (!function_exists('bvmgr_feedback_admin_render_link_card')) {
	function bvmgr_feedback_admin_render_link_card(int $event_plan_id): void
	{
		$survey_url = bvmgr_feedback_survey_url($event_plan_id);
...
if (!function_exists('bvmgr_feedback_admin_render_response')) {
	function bvmgr_feedback_admin_render_response(WP_Post $response, bool $is_duplicate = false): void
	{
		$payload = bvmgr_feedback_get_response_payload((int) $response->ID);
		...
		$submitted = get_date_from_gmt((string) get_post_meta((int) $response->ID, bvmgr_feedback_meta_key('submitted_at_gmt'), true), get_option('date_format') . ' ' . get_option('time_format'));
		if ($submitted === '') {
		...
		$event_plan_id = absint(get_post_meta((int) $response->ID, bvmgr_feedback_meta_key('event_plan_id'), true));
		...
		echo '<p><strong>' . esc_html__('Overall night', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_rating_label($overall['event_rating'] ?? 0)) . '</p>';
		echo '<p><strong>' . esc_html__('Attend again', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_label((string) ($overall['attend_again'] ?? ''))) . '</p>';
		echo '<p><strong>' . esc_html__('Recommend', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_label((string) ($overall['recommend'] ?? ''))) . '</p>';
		echo '</div>';
		...
		foreach (array('overall' => __('Overall', 'backstage-venue-manager'), 'bar' => __('Bar', 'backstage-venue-manager'), 'bathrooms' => __('Bathrooms', 'backstage-venue-manager'), 'arrival' => __('Arrival', 'backstage-venue-manager'), 'sound' => __('Sound', 'backstage-venue-manager')) as $key => $label) {
			echo '<p><strong>' . esc_html((string) $label) . ':</strong> ' . esc_html(bvmgr_feedback_admin_rating_label($venue[$key] ?? 0)) . '</p>';
		}
		echo '</div>';
		if (!empty($venue['bar_details']) || !empty($venue['bar_comment'])) {
			$bar_details = bvmgr_feedback_admin_classify_saved_option_labels(
				(array) ($venue['bar_details'] ?? array()),
				bvmgr_feedback_bar_detail_options(),
				function_exists('bvmgr_feedback_legacy_bar_detail_options') ? bvmgr_feedback_legacy_bar_detail_options() : array()
			);
		...
		if (!empty($venue['bathroom_details']) || !empty($venue['bathroom_comment'])) {
			$bathroom_details = bvmgr_feedback_admin_classify_saved_option_labels(
				(array) ($venue['bathroom_details'] ?? array()),
				bvmgr_feedback_bathroom_detail_options(),
				function_exists('bvmgr_feedback_legacy_bathroom_detail_options') ? bvmgr_feedback_legacy_bathroom_detail_options() : array()
			);
		...
		$has_website_data = ($website_used !== '')
			...
			|| bvmgr_feedback_sanitize_rating($website['website_find_event'] ?? 0) > 0
			|| bvmgr_feedback_sanitize_rating($website['website_ticket_selection'] ?? 0) > 0
			|| bvmgr_feedback_sanitize_rating($website['website_checkout_smoothness'] ?? 0) > 0
			|| bvmgr_feedback_sanitize_rating($website['website_confirmation'] ?? 0) > 0;
		if ($has_website_data) {
			...
			echo '<p><strong>' . esc_html__('Website usage', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_option_label($website_used, bvmgr_feedback_website_usage_options())) . '</p>';
			if ($website_details_enabled) {
				...
				foreach (array(
					...
					if ($row['type'] === 'rating') {
						$rating = bvmgr_feedback_sanitize_rating($row['value'] ?? 0);
						if ($rating <= 0) {
							continue;
						}
						$website_rows[] = '<p><strong>' . esc_html((string) $label) . ':</strong> ' . esc_html(bvmgr_feedback_admin_rating_label($rating)) . '</p>';
						continue;
					}
					...
					$website_rows[] = '<p><strong>' . esc_html((string) $label) . ':</strong> ' . esc_html(bvmgr_feedback_admin_option_label($value, bvmgr_feedback_website_payment_issue_options())) . '</p>';
				}
		...
		if (!empty($primary)) {
			...
			echo '<p><strong>' . esc_html__('Performance', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_rating_label($primary['performance'] ?? 0)) . '</p>';
			echo '<p><strong>' . esc_html__('Bring back', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_label((string) ($primary['bring_back'] ?? ''))) . '</p>';
		...
		foreach ($secondary as $vendor) {
			...
			$show_details = function_exists('bvmgr_feedback_secondary_vendor_details_enabled') ? bvmgr_feedback_secondary_vendor_details_enabled($did_order) : ($did_order === 'yes');
			if (!$show_details && $did_order === '' && bvmgr_feedback_admin_vendor_has_detail_data($vendor)) {
				$show_details = true;
			}
			echo '<h4>' . esc_html((string) ($vendor['name'] ?? __('Secondary vendor', 'backstage-venue-manager'))) . '</h4>';
			if ($did_order !== '') {
				echo '<p><strong>' . esc_html__('Ordered?', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_option_label($did_order, bvmgr_feedback_secondary_vendor_order_options())) . '</p>';
			}
			if ($show_details) {
				echo '<div class="vms-feedback-response-grid">';
				foreach (array('wait_time' => __('Wait', 'backstage-venue-manager'), 'friendliness' => __('Friendliness', 'backstage-venue-manager'), 'selection' => __('Selection', 'backstage-venue-manager'), 'value' => __('Value', 'backstage-venue-manager'), 'quality' => __('Quality', 'backstage-venue-manager'), 'accuracy' => __('Accuracy', 'backstage-venue-manager')) as $key => $label) {
					echo '<p><strong>' . esc_html((string) $label) . ':</strong> ' . esc_html(bvmgr_feedback_admin_rating_label($vendor[$key] ?? 0)) . '</p>';
				}
				echo '<p><strong>' . esc_html__('Bring back', 'backstage-venue-manager') . ':</strong> ' . esc_html(bvmgr_feedback_admin_choice_label((string) ($vendor['bring_back'] ?? ''))) . '</p>';
				echo '</div>';
...
if (!function_exists('bvmgr_feedback_admin_render_resolved_content')) {
	/**
	 * @param array{selected_event_plan_id:int,context:array<string,mixed>,show_missing_plan_notice:bool} $state
	 */
	function bvmgr_feedback_admin_render_resolved_content(array $state, bool $render_missing_plan_notice = false): void
	{
		...
		if (!empty($state['show_missing_plan_notice'])) {
			if ($render_missing_plan_notice) {
				bvmgr_feedback_admin_render_notices(array(
		...
		$responses = bvmgr_feedback_get_responses($selected_event_plan_id);
		$partition = function_exists('bvmgr_feedback_partition_duplicate_responses') ? bvmgr_feedback_partition_duplicate_responses($responses) : array('unique' => $responses, 'duplicate_ids' => array());
		...
		echo '<div>';
		bvmgr_feedback_admin_render_link_card($selected_event_plan_id);
		bvmgr_feedback_admin_render_summary_card($unique_responses);
		bvmgr_feedback_admin_render_website_summary($unique_responses);
		bvmgr_feedback_admin_render_primary_vendor_summary($unique_responses, $context);
		bvmgr_feedback_admin_render_vendor_summary($unique_responses, $context);
		echo '</div>';
		...
		if (function_exists('bvmgr_feedback_admin_render_notification_settings')) {
			bvmgr_feedback_admin_render_notification_settings($selected_event_plan_id);
		}
		echo '</aside>';
		echo '</div>';
		...
		if (empty($responses)) {
			...
			foreach ($responses as $response) {
				bvmgr_feedback_admin_render_response($response, in_array((int) $response->ID, $duplicate_ids, true));
			}
		}
		echo '</div>';
	}
...
if (!function_exists('bvmgr_feedback_admin_render_content')) {
	function bvmgr_feedback_admin_render_content(): void
	{
		$state = bvmgr_feedback_admin_get_page_state();
		$selected_event_plan_id = isset($state['selected_event_plan_id']) ? (int) $state['selected_event_plan_id'] : 0;
		bvmgr_feedback_admin_render_event_selector($selected_event_plan_id);
		bvmgr_feedback_admin_render_resolved_content($state);
	}
}

if (!function_exists('bvmgr_feedback_admin_render_page_without_shell')) {
	function bvmgr_feedback_admin_render_page_without_shell(): void
	{
		$state = bvmgr_feedback_admin_get_page_state();
		$selected_event_plan_id = isset($state['selected_event_plan_id']) ? (int) $state['selected_event_plan_id'] : 0;
		...
		bvmgr_feedback_admin_render_event_selector($selected_event_plan_id);
		bvmgr_feedback_admin_render_resolved_content($state, true);
		echo '</div>';
	}
}

if (!function_exists('bvmgr_render_event_feedback_admin_page')) {
	function bvmgr_render_event_feedback_admin_page(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
		...
		if (function_exists('bvmgr_admin_ui_render_shell')) {
			bvmgr_admin_ui_render_shell(
				array(
					...
					'notices_callback' => 'bvmgr_feedback_admin_render_notices',
				),
				'bvmgr_feedback_admin_render_content'
			);
			return;
		}
...
if (!function_exists('bvmgr_feedback_add_event_plan_metabox')) {
	function bvmgr_feedback_add_event_plan_metabox(): void
	{
		add_meta_box(
			'vms_event_feedback_link',
			__('Post-Event Feedback', 'backstage-venue-manager'),
			'bvmgr_feedback_render_event_plan_metabox',
			'vms_event_plan',
			'side',
			'default'
		);
...
add_action('add_meta_boxes_vms_event_plan', 'bvmgr_feedback_add_event_plan_metabox');

if (!function_exists('bvmgr_feedback_render_event_plan_metabox')) {
	function bvmgr_feedback_render_event_plan_metabox(WP_Post $post): void
	{
...
if (!function_exists('bvmgr_feedback_register_tours')) {
	function bvmgr_feedback_register_tours(array $tours): array
	{
		$tours[] = array(
...
add_filter('vms_tours_register', 'bvmgr_feedback_register_tours');
