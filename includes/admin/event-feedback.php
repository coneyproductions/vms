...
if (!function_exists('vms_feedback_admin_register_page')) {
	function vms_feedback_admin_register_page(): void
	{
		...
		vms_register_admin_page(array(
			...
			'page_title' => __('Event Feedback', 'vms'),
			'menu_title' => __('Event Feedback', 'vms'),
			'capability' => 'manage_options',
			'callback' => 'vms_render_event_feedback_admin_page',
			'section' => 'reports_finance',
			'order' => 35,
			'source' => 'vms-core',
			'description' => __('Private post-event survey links and response summaries.', 'vms'),
			'top_nav' => true,
			'directory' => true,
			'register' => true,
			'shell' => true,
...
if (!function_exists('vms_feedback_admin_handle_save_settings')) {
	function vms_feedback_admin_handle_save_settings(): void
	{
		...
		if (!isset($_POST['vms_feedback_settings_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash((string) $_POST['vms_feedback_settings_nonce'])), 'vms_feedback_save_settings')) {
			wp_die(esc_html__('Settings form expired. Please try again.', 'vms'));
		}
...
if (!function_exists('vms_feedback_admin_handle_delete_response')) {
	function vms_feedback_admin_handle_delete_response(): void
	{
		...
		if (!isset($_POST['vms_feedback_delete_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash((string) $_POST['vms_feedback_delete_nonce'])), 'vms_feedback_delete_response_' . $response_id)) {
			wp_die(esc_html__('Delete request expired. Please try again.', 'vms'));
		}
		if ($event_plan_id <= 0) {
...
if (!function_exists('vms_feedback_admin_render_notices')) {
	function vms_feedback_admin_render_notices(): void
	{
		if (!empty($_GET['vms_feedback_settings_saved'])) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Event Feedback notification settings saved.', 'vms') . '</p></div>';
		}
		if (isset($_GET['vms_feedback_deleted'])) {
			$status = sanitize_key((string) $_GET['vms_feedback_deleted']);
			if ($status === '1') {
				echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Feedback response deleted.', 'vms') . '</p></div>';
			} elseif ($status === 'missing') {
				echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__('Feedback response could not be found.', 'vms') . '</p></div>';
			} else {
				echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__('Feedback response could not be deleted.', 'vms') . '</p></div>';
			}
		}
	}
}
...
if (!function_exists('vms_feedback_admin_render_notification_settings')) {
	function vms_feedback_admin_render_notification_settings(int $event_plan_id): void
	{
		...
		echo '<h3>' . esc_html__('New submission notifications', 'vms') . '</h3>';
		echo '<p class="description">' . esc_html__('Optionally email an operator whenever a new private feedback response is submitted.', 'vms') . '</p>';
		...
		echo '<label class="vms-feedback-checkbox-inline"><input type="checkbox" name="notify_enabled" value="1"' . checked(!empty($settings['enabled']), true, false) . '> ' . esc_html__('Email me new submissions', 'vms') . '</label>';
		echo '<label class="vms-feedback-settings-label" for="vms-feedback-notify-recipients">' . esc_html__('Recipient emails', 'vms') . '</label>';
		...
		echo '<p class="description">' . esc_html__('Separate multiple emails with commas or line breaks. Leave blank to use the site admin email.', 'vms') . '</p>';
		echo '<p><button type="submit" class="button button-secondary">' . esc_html__('Save Notification Settings', 'vms') . '</button></p>';
		echo '</form>';
	}
}
...
if (!function_exists('vms_feedback_admin_render_event_selector')) {
	function vms_feedback_admin_render_event_selector(int $selected_event_plan_id): void
	{
		...
		echo '<label for="vms-feedback-event-plan-id"><strong>' . esc_html__('Choose event', 'vms') . '</strong></label>';
		echo '<select id="vms-feedback-event-plan-id" name="event_plan_id">';
		echo '<option value="0">' . esc_html__('Select an Event Plan...', 'vms') . '</option>';
		...
		echo '</select> ';
		submit_button(__('View Feedback', 'vms'), 'primary', '', false);
		echo '</form>';
	}
}
...
if (!function_exists('vms_feedback_admin_render_summary_card')) {
	...
	{
		$metrics = array(
			__('Overall night', 'vms') => 'overall.event_rating',
			__('Venue overall', 'vms') => 'venue.overall',
			__('Bar', 'vms') => 'venue.bar',
			__('Bathrooms', 'vms') => 'venue.bathrooms',
			__('Arrival / check-in', 'vms') => 'venue.arrival',
			__('Sound', 'vms') => 'venue.sound',
			__('Primary vendor', 'vms') => 'primary_vendor.performance',
		);

		echo '<div class="vms-feedback-admin-card" data-vms-tour="event-feedback.summary">';
		echo '<h2>' . esc_html__('At-a-glance ratings', 'vms') . '</h2>';
...
if (!function_exists('vms_feedback_admin_render_website_summary')) {
	...
	function vms_feedback_admin_render_website_summary(array $responses): void
	{
		...
		echo '<p><strong>' . esc_html__('Website usage', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_counts_label(vms_feedback_admin_choice_counts($responses, 'website.website_used', $usage_options), $usage_options)) . '</p>';
		echo '<p><strong>' . esc_html__('Reported issues', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_counts_label(vms_feedback_admin_choice_counts($responses, 'website.website_payment_issues', $payment_issue_options, $website_detail_filter), $payment_issue_options)) . '</p>';
		echo '<div class="vms-feedback-metric-grid">';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Find Event', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_average_label(vms_feedback_average_filtered($responses, 'website.website_find_event', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Ticket Selection', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_average_label(vms_feedback_average_filtered($responses, 'website.website_ticket_selection', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Checkout', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_average_label(vms_feedback_average_filtered($responses, 'website.website_checkout_smoothness', $website_detail_filter))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Confirmation', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_average_label(vms_feedback_average_filtered($responses, 'website.website_confirmation', $website_detail_filter))) . '</strong></div>';
		echo '</div>';
		echo '</div>';
	}
}
...
if (!function_exists('vms_feedback_admin_render_primary_vendor_summary')) {
	...
	function vms_feedback_admin_render_primary_vendor_summary(array $responses, array $context): void
	{
		...
		echo '<h2>' . esc_html__('Primary vendor details', 'vms') . '</h2>';
		echo '<section class="vms-feedback-vendor-summary">';
		echo '<h3>' . esc_html((string) ($primary['name'] ?? get_the_title($vendor_id))) . '</h3>';
		echo '<div class="vms-feedback-metric-grid">';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Performance', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_average_label(vms_feedback_average($responses, 'primary_vendor.performance'))) . '</strong></div>';
		echo '<div class="vms-feedback-metric"><span>' . esc_html__('Would like them back', 'vms') . '</span><strong>' . esc_html(vms_feedback_admin_choice_counts_label(vms_feedback_admin_choice_counts($responses, 'primary_vendor.bring_back'))) . '</strong></div>';
		echo '</div>';
		echo '</section>';
		echo '</div>';
	}
...
if (!function_exists('vms_feedback_admin_render_vendor_summary')) {
	...
	function vms_feedback_admin_render_vendor_summary(array $responses, array $context): void
	{
		...
		echo '<h2>' . esc_html__('Secondary vendor details', 'vms') . '</h2>';
		foreach ($secondary as $vendor) {
			...
			$paths = array(
				__('Wait time / speed', 'vms') => 'secondary_vendors.' . $vendor_id . '.wait_time',
				__('Friendliness', 'vms') => 'secondary_vendors.' . $vendor_id . '.friendliness',
				__('Selection', 'vms') => 'secondary_vendors.' . $vendor_id . '.selection',
				__('Price / value', 'vms') => 'secondary_vendors.' . $vendor_id . '.value',
				__('Quality', 'vms') => 'secondary_vendors.' . $vendor_id . '.quality',
				__('Accuracy', 'vms') => 'secondary_vendors.' . $vendor_id . '.accuracy',
			);
			...
			echo '<p><strong>' . esc_html__('Did you order from them?', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_counts_label(vms_feedback_admin_choice_counts($responses, 'secondary_vendors.' . $vendor_id . '.did_order', $order_options), $order_options)) . '</p>';
			echo '<p><strong>' . esc_html__('Would bring back', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_counts_label(vms_feedback_admin_choice_counts($responses, 'secondary_vendors.' . $vendor_id . '.bring_back', vms_feedback_yes_maybe_no_options(), $ordered_filter))) . '</p>';
			echo '<div class="vms-feedback-metric-grid">';
...
if (!function_exists('vms_feedback_admin_render_link_card')) {
	function vms_feedback_admin_render_link_card(int $event_plan_id): void
	{
		...
		echo '<h2>' . esc_html__('Private survey link', 'vms') . '</h2>';
		echo '<p>' . esc_html__('Share this link with ticket buyers or attendees. Responses stay private in VMS.', 'vms') . '</p>';
		echo '<div class="vms-feedback-link-row"><input type="text" class="large-text code" readonly value="' . esc_attr($survey_url) . '"></div>';
		echo '<p class="description">' . esc_html__('Tip: copy the full URL above and send it by email, text, or a private post-event message.', 'vms') . '</p>';
		...
		if (is_string($edit_url) && $edit_url !== '') {
			echo '<a class="button" href="' . esc_url($edit_url) . '">' . esc_html__('Edit Event Plan', 'vms') . '</a> ';
		}
		echo '<a class="button button-secondary" href="' . esc_url($email_preview_url) . '">' . esc_html__('Open Post-Event Email Preview', 'vms') . '</a>';
		echo '</p>';
		echo '</div>';
	}
}
...
if (!function_exists('vms_feedback_admin_render_response')) {
	function vms_feedback_admin_render_response(WP_Post $response, bool $is_duplicate = false): void
	{
		...
		echo '<summary><strong>' . esc_html($submitted) . '</strong> - ' . esc_html($name !== '' ? $name : __('Anonymous', 'vms')) . ($email !== '' ? ' - ' . esc_html($email) : '');
		if ($is_duplicate) {
			echo ' <span class="vms-feedback-duplicate-badge">' . esc_html__('Likely duplicate', 'vms') . '</span>';
		}
		echo '</summary>';
		...
		$confirm = "return confirm('" . esc_js(__('Delete this feedback response? This cannot be undone.', 'vms')) . "');";
		echo '<button type="submit" class="button button-link-delete" onclick="' . esc_attr($confirm) . '">' . esc_html__('Delete response', 'vms') . '</button>';
		echo '</form>';
		echo '<div class="vms-feedback-response-grid">';
		echo '<p><strong>' . esc_html__('Overall night', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_rating_label($overall['event_rating'] ?? 0)) . '</p>';
		echo '<p><strong>' . esc_html__('Attend again', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_label((string) ($overall['attend_again'] ?? ''))) . '</p>';
		echo '<p><strong>' . esc_html__('Recommend', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_label((string) ($overall['recommend'] ?? ''))) . '</p>';
		echo '</div>';

		echo '<h4>' . esc_html__('Venue', 'vms') . '</h4>';
		echo '<div class="vms-feedback-response-grid">';
		foreach (array('overall' => __('Overall', 'vms'), 'bar' => __('Bar', 'vms'), 'bathrooms' => __('Bathrooms', 'vms'), 'arrival' => __('Arrival', 'vms'), 'sound' => __('Sound', 'vms')) as $key => $label) {
		...
		if (!empty($venue['bar_details']) || !empty($venue['bar_comment'])) {
			...
			if (!empty($bar_details['current'])) {
				echo '<p><strong>' . esc_html__('Bar details', 'vms') . ':</strong> ' . esc_html(implode(', ', $bar_details['current'])) . '</p>';
			}
			$bar_legacy_labels = array_merge($bar_details['legacy'], $bar_details['unknown']);
			if (!empty($bar_legacy_labels)) {
				echo '<p><strong>' . esc_html__('Bar details', 'vms') . ':</strong> ' . esc_html(__('Legacy selections:', 'vms') . ' ' . implode(', ', $bar_legacy_labels)) . '</p>';
			}
		...
		if (!empty($venue['bathroom_details']) || !empty($venue['bathroom_comment'])) {
			...
			if (!empty($bathroom_details['current'])) {
				echo '<p><strong>' . esc_html__('Bathroom details', 'vms') . ':</strong> ' . esc_html(implode(', ', $bathroom_details['current'])) . '</p>';
			}
			$bathroom_legacy_labels = array_merge($bathroom_details['legacy'], $bathroom_details['unknown']);
			if (!empty($bathroom_legacy_labels)) {
				echo '<p><strong>' . esc_html__('Bathroom details', 'vms') . ':</strong> ' . esc_html(__('Legacy selections:', 'vms') . ' ' . implode(', ', $bathroom_legacy_labels)) . '</p>';
			}
		...
		if ($has_website_data) {
			...
			echo '<h4>' . esc_html__('Website / Ticket Purchase Experience', 'vms') . '</h4>';
			echo '<p><strong>' . esc_html__('Website usage', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_option_label($website_used, vms_feedback_website_usage_options())) . '</p>';
			if ($website_details_enabled) {
				$website_rows = array();
				foreach (array(
					__('Find event', 'vms') => array('type' => 'rating', 'value' => $website['website_find_event'] ?? 0),
					__('Ticket selection', 'vms') => array('type' => 'rating', 'value' => $website['website_ticket_selection'] ?? 0),
					__('Checkout', 'vms') => array('type' => 'rating', 'value' => $website['website_checkout_smoothness'] ?? 0),
					__('Confirmation', 'vms') => array('type' => 'rating', 'value' => $website['website_confirmation'] ?? 0),
					__('Payment issues', 'vms') => array('type' => 'choice', 'value' => (string) ($website['website_payment_issues'] ?? '')),
				) as $label => $row) {
					if ($row['type'] === 'rating') {
		...
		if (!empty($primary)) {
			echo '<h4>' . esc_html((string) ($primary['name'] ?? __('Primary vendor', 'vms'))) . '</h4>';
			echo '<p><strong>' . esc_html__('Performance', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_rating_label($primary['performance'] ?? 0)) . '</p>';
			echo '<p><strong>' . esc_html__('Bring back', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_label((string) ($primary['bring_back'] ?? ''))) . '</p>';
		...
		foreach ($secondary as $vendor) {
			...
			echo '<h4>' . esc_html((string) ($vendor['name'] ?? __('Secondary vendor', 'vms'))) . '</h4>';
			if ($did_order !== '') {
				echo '<p><strong>' . esc_html__('Ordered?', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_option_label($did_order, vms_feedback_secondary_vendor_order_options())) . '</p>';
			}
			if ($show_details) {
				echo '<div class="vms-feedback-response-grid">';
				foreach (array('wait_time' => __('Wait', 'vms'), 'friendliness' => __('Friendliness', 'vms'), 'selection' => __('Selection', 'vms'), 'value' => __('Value', 'vms'), 'quality' => __('Quality', 'vms'), 'accuracy' => __('Accuracy', 'vms')) as $key => $label) {
					echo '<p><strong>' . esc_html((string) $label) . ':</strong> ' . esc_html(vms_feedback_admin_rating_label($vendor[$key] ?? 0)) . '</p>';
				}
				echo '<p><strong>' . esc_html__('Bring back', 'vms') . ':</strong> ' . esc_html(vms_feedback_admin_choice_label((string) ($vendor['bring_back'] ?? ''))) . '</p>';
				echo '</div>';
				if (!empty($vendor['wait_causes'])) {
					echo '<p><strong>' . esc_html__('Wait causes', 'vms') . ':</strong> ' . esc_html(implode(', ', (array) $vendor['wait_causes'])) . '</p>';
				}
				if (!empty($vendor['comment'])) {
					echo '<p>' . nl2br(esc_html((string) $vendor['comment'])) . '</p>';
				}
			} elseif ($did_order !== '') {
				echo '<p>' . esc_html__('Detailed ratings skipped.', 'vms') . '</p>';
			} else {
				echo '<p>' . esc_html__('No detailed food vendor ratings submitted.', 'vms') . '</p>';
			}
		}

		if (!empty($payload['final_comment'])) {
			echo '<h4>' . esc_html__('Final comments', 'vms') . '</h4>';
...
if (!function_exists('vms_feedback_admin_render_content')) {
	function vms_feedback_admin_render_content(): void
	{
		...
		if ($selected_event_plan_id <= 0) {
			echo '<div class="vms-feedback-admin-card"><h2>' . esc_html__('Get started', 'vms') . '</h2><p>' . esc_html__('Choose an Event Plan to generate a private survey link and review responses.', 'vms') . '</p></div>';
			return;
		}

		$context = vms_feedback_get_event_context($selected_event_plan_id);
		if (empty($context)) {
			echo '<div class="notice notice-error"><p>' . esc_html__('That Event Plan could not be found.', 'vms') . '</p></div>';
			return;
		}
		...
		echo '<h2>' . esc_html__('Response count', 'vms') . '</h2>';
		echo '<p class="vms-feedback-big-number">' . esc_html(number_format_i18n(count($unique_responses))) . '</p>';
		if (!empty($duplicate_ids)) {
			echo '<p class="description">' . esc_html(sprintf(__('%1$d stored response(s); %2$d likely duplicate(s) excluded from averages.', 'vms'), count($responses), count($duplicate_ids))) . '</p>';
		}
		echo '<p class="description">' . esc_html__('Private/internal only. Do not share raw comments with vendors unless you intentionally curate or anonymize them.', 'vms') . '</p>';
		if (function_exists('vms_feedback_admin_render_notification_settings')) {
		...
		echo '<h2>' . esc_html__('Responses', 'vms') . '</h2>';
		if (empty($responses)) {
			echo '<p>' . esc_html__('No responses yet. Share the private survey link above to begin collecting feedback.', 'vms') . '</p>';
		} else {
			foreach ($responses as $response) {
	...
	function vms_render_event_feedback_admin_page(): void
	{
		...
		if (function_exists('vms_admin_ui_render_shell')) {
			vms_admin_ui_render_shell(
				array(
					'title' => __('Event Feedback', 'vms'),
					'subtitle' => __('Private one-stop post-event surveys for venue, bar, bathrooms, primary vendors, and secondary vendors.', 'vms'),
		...
		echo '<div class="wrap"><h1>' . esc_html__('Event Feedback', 'vms') . '</h1>';
		vms_feedback_admin_render_content();
		echo '</div>';
	}
}
...
if (!function_exists('vms_feedback_render_event_plan_metabox')) {
	function vms_feedback_render_event_plan_metabox(WP_Post $post): void
	{
		...
		if ($event_plan_id <= 0 || $post->post_status === 'auto-draft') {
			echo '<p>' . esc_html__('Save the Event Plan before sharing a feedback survey.', 'vms') . '</p>';
			return;
		}
		$survey_url = vms_feedback_survey_url($event_plan_id);
		$admin_url = add_query_arg(array('page' => 'vms-event-feedback', 'event_plan_id' => $event_plan_id), admin_url('admin.php'));
		echo '<p>' . esc_html__('Private survey link for collecting post-event feedback.', 'vms') . '</p>';
		...
		echo '<p><a class="button button-secondary" href="' . esc_url($admin_url) . '">' . esc_html__('View Responses', 'vms') . '</a></p>';
		echo '<p><a class="button button-secondary" href="' . esc_url($email_preview_url) . '">' . esc_html__('Preview Feedback Email', 'vms') . '</a></p>';
	}
}
		...
		$tours[] = array(
			...
			'steps' => array(
				array(
					...
					'title' => __('Start with the event', 'vms'),
					'body' => __('Pick the Event Plan you want feedback for. The survey pulls the event, primary vendor, and secondary vendors from that plan so you do not rebuild a form by hand.', 'vms'),
					'position' => 'bottom',
				),
				array(
					'id' => 'survey-link',
					'selector' => '[data-vms-tour="event-feedback.link"]',
					'title' => __('Share one private link', 'vms'),
					'body' => __('Copy this event-specific survey link into a post-event email, text, or private message. Customer responses stay internal unless you intentionally curate or anonymize them.', 'vms'),
					'position' => 'bottom',
				),
				array(
					'id' => 'summary',
					'selector' => '[data-vms-tour="event-feedback.summary"]',
					'title' => __('Read the quick signal first', 'vms'),
					'body' => __('These cards average the key ratings and ignore likely duplicate submissions so double-clicks, refreshes, or email-link retries do not distort the picture.', 'vms'),
					'position' => 'bottom',
				),
				array(
					'id' => 'vendor-details',
					'selector' => '[data-vms-tour="event-feedback.primary-vendor"]',
					'title' => __('Check vendor-specific patterns', 'vms'),
					'body' => __('Primary vendor and secondary vendor cards help separate the overall night from specific performance, food vendor, or vendor issues.', 'vms'),
					'position' => 'bottom',
				),
				array(
					'id' => 'responses',
					'selector' => '[data-vms-tour="event-feedback.responses"]',
					'title' => __('Keep raw comments private', 'vms'),
					'body' => __('Use the detailed responses for operator review. Avoid sharing raw customer-identifiable comments with vendors unless you choose to summarize or anonymize them. If an early duplicate slips through, use Delete response to remove it from this view.', 'vms'),
					'position' => 'top',
				),
			),
		);
