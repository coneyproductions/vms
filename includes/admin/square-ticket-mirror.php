...
function vms_square_ticket_mirror_render_metabox($post): void
{
    $product_id = is_object($post) && isset($post->ID) ? absint($post->ID) : 0;
    if ($product_id <= 0) {
        echo '<p>' . esc_html__('Product context is unavailable.', 'vms') . '</p>';
        return;
    }
    ...
    if (!empty($eligibility['eligible'])) {
        ...
        echo '<p><strong>' . esc_html__('Eligibility', 'vms') . ':</strong> ' . esc_html((string) ($eligibility['reason_message'] ?? __('Not eligible', 'vms'))) . '</p>';
    }

    if (!empty($source_model['mirror_name'])) {
        echo '<p><strong>' . esc_html__('Mirror Name', 'vms') . ':</strong><br />' . esc_html((string) $source_model['mirror_name']) . '</p>';
    }

    echo '<p><strong>' . esc_html__('SKU', 'vms') . ':</strong> <code>' . esc_html((string) ($eligibility['sku'] ?? '')) . '</code></p>';
    echo '<p><strong>' . esc_html__('Square Category', 'vms') . ':</strong> ' . esc_html(vms_square_ticket_mirror_target_category_name()) . '</p>';

    if (!empty($state['item_id'])) {
        echo '<p><strong>' . esc_html__('Square Item ID', 'vms') . ':</strong><br /><code>' . esc_html((string) $state['item_id']) . '</code></p>';
    }
    if (!empty($state['variation_id'])) {
        echo '<p><strong>' . esc_html__('Square Variation ID', 'vms') . ':</strong><br /><code>' . esc_html((string) $state['variation_id']) . '</code></p>';
    }
    if (!empty($state['location_id'])) {
        echo '<p><strong>' . esc_html__('Square Location', 'vms') . ':</strong><br /><code>' . esc_html((string) $state['location_id']) . '</code></p>';
    } elseif (!empty($square['location_id'])) {
        echo '<p><strong>' . esc_html__('Square Location', 'vms') . ':</strong><br /><code>' . esc_html((string) $square['location_id']) . '</code></p>';
    }

    if (!empty($state['last_sync_gmt'])) {
        echo '<p><strong>' . esc_html__('Last Sync (GMT)', 'vms') . ':</strong><br />' . esc_html((string) $state['last_sync_gmt']) . '</p>';
    }
    if (!empty($state['last_order_stamp_gmt'])) {
        echo '<p><strong>' . esc_html__('Last Order Stamp (GMT)', 'vms') . ':</strong><br />' . esc_html((string) $state['last_order_stamp_gmt']) . '</p>';
    }
    if (!empty($state['last_retired_gmt'])) {
        echo '<p><strong>' . esc_html__('Last Retired (GMT)', 'vms') . ':</strong><br />' . esc_html((string) $state['last_retired_gmt']) . '</p>';
    }

    if (!empty($state['last_error_message'])) {
        echo '<div class="notice notice-error inline"><p><strong>' . esc_html__('Last Error', 'vms') . ':</strong> ' . esc_html((string) $state['last_error_message']) . '</p></div>';
    }

    if (empty($square['ok'])) {
        echo '<div class="notice notice-warning inline"><p>' . esc_html((string) ($square['error_message'] ?? __('WooCommerce Square is unavailable.', 'vms'))) . '</p></div>';
    }
    ...
    if (!empty($eligibility['eligible']) && empty($state['item_id'])) {
        ...
        submit_button(__('Mirror This Ticket to Square', 'vms'), 'primary', 'submit', false);
        echo '</form>';
    }
    ...
    if (!empty($eligibility['eligible']) && (!empty($state['item_id']) || in_array((string) ($state['status'] ?? ''), array('mirror_stale', 'mirror_error', 'mirror_retired'), true))) {
        ...
        echo '<input type="hidden" name="product_id" value="' . esc_attr((string) $product_id) . '" />';
        submit_button(__('Refresh Square Mirror', 'vms'), 'secondary', 'submit', false);
        echo '</form>';
    }
    ...
    if (!empty($state['item_id']) && (string) ($state['status'] ?? '') !== 'mirror_retired') {
        ...
        echo '<input type="hidden" name="product_id" value="' . esc_attr((string) $product_id) . '" />';
        submit_button(__('Retire Square Mirror', 'vms'), 'delete', 'submit', false, array(
            'onclick' => "return confirm('" . esc_js(__('Retire this mirrored Square item from the resolved location?', 'vms')) . "');",
        ));
        echo '</form>';
    }
    ...
    if ((string) ($state['status'] ?? '') === 'mirror_error' || !empty($state['last_error_code']) || !empty($state['last_error_message'])) {
        ...
        echo '<input type="hidden" name="product_id" value="' . esc_attr((string) $product_id) . '" />';
        submit_button(__('Clear Mirror Error', 'vms'), 'secondary', 'submit', false);
        echo '</form>';
    }

    echo '</div>';

    echo '<p class="description">' . esc_html__('Phase 1 adds manual mirror, refresh, retire, and order-item stamping. Automatic expired-ticket archive/cleanup remains a follow-up slice.', 'vms') . '</p>';

    $logs = vms_square_ticket_mirror_recent_logs($product_id, 5);
    if (!empty($logs)) {
        echo '<hr />';
        echo '<p><strong>' . esc_html__('Recent Mirror Logs', 'vms') . '</strong></p>';
        echo '<table class="widefat striped" style="font-size:12px;">';
        echo '<thead><tr><th>' . esc_html__('When', 'vms') . '</th><th>' . esc_html__('Action', 'vms') . '</th><th>' . esc_html__('Result', 'vms') . '</th></tr></thead>';
        echo '<tbody>';
        foreach ($logs as $row) {
            if (!is_array($row)) {
                continue;
    ...
    if ($mirror_action === 'mirror' || $mirror_action === 'refresh') {
        ...
        if (!empty($result['ok'])) {
            $notice_message = $mirror_action === 'mirror'
                ? __('Square mirror created for this ticket.', 'vms')
                : __('Square mirror refreshed for this ticket.', 'vms');
        } else {
            $notice_type = 'error';
            $notice_message = (string) ($result['error_message'] ?? __('Square mirror action failed.', 'vms'));
        }
    } elseif ($mirror_action === 'retire') {
        $result = vms_square_ticket_mirror_retire_product($product_id);
        if (!empty($result['ok'])) {
            $notice_message = __('Square mirror retired for this ticket.', 'vms');
        } else {
            $notice_type = 'error';
            $notice_message = (string) ($result['error_message'] ?? __('Square mirror retire failed.', 'vms'));
        }
    } elseif ($mirror_action === 'clear_error') {
        $result = vms_square_ticket_mirror_clear_error_state($product_id);
        if (!empty($result['ok'])) {
            $notice_message = __('Square mirror error state cleared.', 'vms');
        } else {
            $notice_type = 'error';
            $notice_message = __('Square mirror error state could not be cleared.', 'vms');
        }
    } else {
        $notice_type = 'error';
        $notice_message = __('Unknown Square mirror action.', 'vms');
    }
