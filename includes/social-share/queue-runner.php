...
if (!function_exists('bvmgr_social_retry_backoff_seconds')) {
	/**
	 * @return array<int,int>
	 */
	function bvmgr_social_retry_backoff_seconds(): array
	{
...
if (!function_exists('bvmgr_social_next_attempt_utc')) {
	function bvmgr_social_next_attempt_utc(int $attempt): ?string
	{
		...
		$steps = bvmgr_social_retry_backoff_seconds();
...
if (!function_exists('bvmgr_social_runner_acquire_lock')) {
	function bvmgr_social_runner_acquire_lock(): bool
	{
...
if (!function_exists('bvmgr_social_runner_release_lock')) {
	function bvmgr_social_runner_release_lock(): void
	{
		$key = defined('BVMGR_SOCIAL_LOCK_TRANSIENT') ? (string) BVMGR_SOCIAL_LOCK_TRANSIENT : 'vms_social_runner_lock';
...
if (!function_exists('bvmgr_social_schedule_cron')) {
	function bvmgr_social_schedule_cron(): void
	{
...
add_action('init', 'bvmgr_social_schedule_cron', 30);
...
if (!function_exists('bvmgr_social_queue_render_for_row')) {
	...
	{
		$context = bvmgr_social_context_from_queue_row($row);
		$template = bvmgr_social_template_for_platform((string) ($row['platform'] ?? ''), (int) ($row['template_id'] ?? 0));
		...
		$rendered = bvmgr_social_render_template_payload(
			(string) ($row['platform'] ?? ''),
...
if (!function_exists('bvmgr_social_queue_decode_payload_snapshot')) {
	 ...
	 * @return array<string,mixed>
	 */
	function bvmgr_social_queue_decode_payload_snapshot($raw): array
	{
		$result = array(
			'ok' => false,
			'schema' => 'empty',
...
if (!function_exists('bvmgr_social_queue_process_item')) {
	 ...
	 * @param array<string,mixed> $row
	 */
	function bvmgr_social_queue_process_item(array $row): void
	{
		...
		if (!empty($row['platform_post_id'])) {
			bvmgr_social_queue_update($queue_id, array(
				'status' => 'posted',
				'updated_by' => 0,
			));
			bvmgr_social_audit_log('publish_ok', array('note' => 'already had platform_post_id; normalized to posted'), $queue_id, (string) ($row['platform'] ?? ''), 0);
			return;
		}

		$render = bvmgr_social_queue_render_for_row($row);
		if (empty($render['ok'])) {
			bvmgr_social_queue_update($queue_id, array(
				'status' => 'needs_review',
			...
			bvmgr_social_audit_log('publish_fail', $render, $queue_id, (string) ($row['platform'] ?? ''), 0);
			return;
		}
		...
		if (!empty($rendered['needs_review'])) {
			...
			bvmgr_social_audit_log('publish_fail', array('reason' => 'caption_too_long', 'length' => (int) ($rendered['length'] ?? 0), 'limit' => (int) ($rendered['limit'] ?? 0)), $queue_id, (string) ($row['platform'] ?? ''), 0);
			return;
		}
		...
		$snapshot_state = bvmgr_social_queue_decode_payload_snapshot($row['payload_snapshot_json'] ?? null);
		...
		if (!empty($snapshot_state['ok']) && (string) ($snapshot_state['schema'] ?? '') === 'queued') {
			...
			$map = bvmgr_social_venue_map_for_platform((int) ($row['venue_id'] ?? 0), (string) ($row['platform'] ?? ''));
			if (is_array($map)) {
		...
		if (!($provider instanceof BVMGR_Social_Provider_Interface)) {
			...
			bvmgr_social_audit_log('publish_fail', array('reason' => 'provider_missing'), $queue_id, (string) ($row['platform'] ?? ''), 0);
			return;
		}

		$attempts = (int) ($row['attempts'] ?? 0) + 1;
		$max_attempts = (int) (bvmgr_social_get_settings()['max_attempts'] ?? 5);
		...
		try {
			...
			bvmgr_social_queue_update($queue_id, array(
				...
				'payload_snapshot_json' => wp_json_encode(array(
					'rendered' => $rendered,
					'provider_payload' => bvmgr_social_sanitize_details($provider_payload),
					'provider_result' => bvmgr_social_sanitize_details($result),
				)),
				'last_error_code' => '',
				'last_error_message' => '',
				'updated_by' => 0,
			...
			bvmgr_social_audit_log('publish_ok', array(
				'platform_post_id' => $platform_post_id,
				'attempts' => $attempts,
			...
			if ($auth_expired && $account_id > 0) {
				bvmgr_social_account_set_auth_state($account_id, 'expired', array('last_error' => $message));
			}
			...
			if ($needs_review) {
				...
				$status = 'failed';
				$next_attempt = bvmgr_social_next_attempt_utc($attempts);
			}
			...
			bvmgr_social_audit_log('publish_fail', array(
...
if (!function_exists('bvmgr_social_process_queue')) {
	 ...
	 * @return array<string,mixed>
	 */
	function bvmgr_social_process_queue(int $batch_size = 20): array
	{
		$summary = array(
			'locked' => false,
			'skipped' => false,
			'processed' => 0,
			'run_at' => bvmgr_social_now_mysql_utc(),
		);

		if (!bvmgr_social_runner_acquire_lock()) {
			$summary['locked'] = true;
			return $summary;
		}

		try {
			if (!bvmgr_social_is_enabled() || bvmgr_social_kill_switch_active()) {
				$summary['skipped'] = true;
				return $summary;
			}

			$due = bvmgr_social_queue_due_items($batch_size);
			foreach ($due as $row) {
				$queue_id = (int) ($row['id'] ?? 0);
				if ($queue_id <= 0) {
					continue;
				}
				if (!bvmgr_social_queue_claim($queue_id)) {
					continue;
				}
				$fresh = bvmgr_social_queue_get($queue_id);
				if (!is_array($fresh)) {
					continue;
				}
				bvmgr_social_queue_process_item($fresh);
				$summary['processed']++;
			}
		} finally {
			bvmgr_social_runner_release_lock();
		}
...
add_action($bvmgr_social_cron_hook, function (): void {
	$summary = bvmgr_social_process_queue(20);
	bvmgr_social_audit_log('runner_tick', $summary, 0, '', 0);
});
