...
if (!function_exists('bvmgr_social_platform_character_limit')) {
	function bvmgr_social_platform_character_limit(string $platform): int
	{
...
if (!function_exists('bvmgr_social_build_campaign_slug')) {
	function bvmgr_social_build_campaign_slug(array $context): string
	{
...
if (!function_exists('bvmgr_social_append_utm')) {
	function bvmgr_social_append_utm(string $url, string $source, array $context = array()): string
	{
		$url = esc_url_raw($url);
		if ($url === '') {
			return '';
		...
		$campaign = bvmgr_social_build_campaign_slug($context);
...
if (!function_exists('bvmgr_social_template_token_map')) {
	/**
	 * @param array<string,mixed> $context
	 * @return array<string,string>
	 */
	function bvmgr_social_template_token_map(array $context): array
	{
...
if (!function_exists('bvmgr_social_render_template_body')) {
	function bvmgr_social_render_template_body(string $body, array $context): string
	{
		$rendered = strtr($body, bvmgr_social_template_token_map($context));
		return bvmgr_social_normalize_whitespace((string) $rendered);
	}
}

if (!function_exists('bvmgr_social_template_get')) {
	/**
	 * @return array<string,mixed>|null
	 */
	function bvmgr_social_template_get(int $template_id): ?array
	{
...
if (!function_exists('bvmgr_social_template_for_platform')) {
	 ...
	 * @return array<string,mixed>|null
	 */
	function bvmgr_social_template_for_platform(string $platform, int $template_id = 0): ?array
	{
		$template_id = absint($template_id);
		if ($template_id > 0) {
			$tpl = bvmgr_social_template_get($template_id);
			if (is_array($tpl)) {
				return $tpl;
			}
		}
		return bvmgr_social_template_default_for_platform($platform);
	}
}
...
if (!function_exists('bvmgr_social_render_template_payload')) {
	 ...
	 */
	function bvmgr_social_render_template_payload(string $platform, string $body, array $context, bool $utm_enabled = true): array
	{
		$platform = sanitize_key($platform);
		$caption = bvmgr_social_render_template_body($body, $context);
		...
		if ($utm_enabled && $final_url !== '') {
			...
			$final_url = bvmgr_social_append_utm($final_url, $utm_source, $context);
		}

		$limit = bvmgr_social_platform_character_limit($platform);
