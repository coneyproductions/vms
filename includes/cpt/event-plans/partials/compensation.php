    ...
    document.addEventListener('DOMContentLoaded', () => {
        const wrap = document.getElementById('vms-comp-options');
        if (!wrap) return;
        ...
        const selInp = document.getElementById('vms_comp_selected_option');
	        ...
	        const setSelectedTile = (btn) => {
	            wrap.querySelectorAll('.vms-comp-opt-tile').forEach(b => b.classList.remove('is-selected'));
	            if (btn) btn.classList.add('is-selected');
	        };

	        let applyingDraftFromOption = false;

	        const readTermsFromTile = (btn) => ({
	        ...
	        const setDraftFromTerms = (terms) => {
	            ...
	            [fBonusMode, fBonusStart, fBonusStepSize, fBonusStepBonus, fBonusPerTicket, fBonusMax, fCommissionPercent, fCommissionMode].forEach((field) => {
	                if (!field) return;
	        ...
	        const syncDraftFromSelectedDefault = () => {
	            if (!selInp) return;
	            const selected = String(selInp.value || '').trim();
	            if (!selected.startsWith('default:')) return;

	            const btn = Array.from(wrap.querySelectorAll('.vms-comp-opt-tile.is-selected')).find((el) => {
            ...
            fetch(ajaxurl, { method: 'POST', credentials: 'same-origin', body: form })
                ...
                .then(data => {
                    ...
                    // Update tiles + max guarantee (used by low-guarantee acknowledgment).
                    wrap.innerHTML = data.data.html;

                    const maxInp = document.getElementById('vms_max_guarantee_available');
	                    if (maxInp && typeof data.data.max_guarantee !== 'undefined') {
	                        maxInp.value = String(data.data.max_guarantee || '0');
	                    }

	                    // If a default option is currently selected, keep Draft Pay in sync with
	                    // updated default terms (e.g., vendor default changes).
	        ...
	        wrap.addEventListener('click', (e) => {
	            const btn = e.target.closest('.vms-comp-opt-tile');
	            if (!btn) return;
	        ...
	        const clearSelectedOptionOnManualEdit = () => {
	        ...
	        [fStruct, fFlat, fSplit, fBonusMode, fBonusStart, fBonusStepSize, fBonusStepBonus, fBonusPerTicket, fBonusMax, fCommissionPercent, fCommissionMode].forEach((el) => {
	            if (!el) return;
    ...
    ?>"><?php esc_html_e('Primary Vendor Compensation', 'vms'); ?></h4>
    ...
    <?php $vms_comp_ack_html = $this->capture_event_plan_partial('comp-ack', get_defined_vars()); ?>
	    ...
	    <?php
	        ...
	        $vms_flat_fee_label = $vms_is_attendance_bonus ? __('Base Pay', 'vms') : __('Flat Fee Amount', 'vms');
	        $vms_flat_fee_help = $vms_is_attendance_bonus ? __('The guaranteed amount for this event before attendance bonuses.', 'vms') : '';
	        $vms_comp_has_data = (
	            ((string) $comp_structure !== 'flat_fee')
	            || ((string) $flat_fee_amount !== '')
	            || ((string) $door_split_percent !== '')
    ...
    <div class="vms-ep-card vms-ep-card--white vms-ep-card--comp" data-vms-section-has-data="<?php echo $vms_comp_has_data ? '1' : '0'; ?>">
        <strong><?php esc_html_e('Compensation Options', 'vms'); ?></strong>

        <p class="description vms-mt-6">
            <?php esc_html_e('Load Draft Pay from a tile, review it, then lock it for this event.', 'vms'); ?>
        </p>
        ...
        <?php if (!empty($vms_show_vendor_default_drift_notice)): ?>
            <div class="vms-ep-card vms-ep-card--amber vms-notice vms-notice--warning vms-mt-10">
                <strong class="vms-text-warn">⚠️ <?php esc_html_e('Live Primary Vendor default differs from Draft Pay', 'vms'); ?></strong><br>
                <span class="description"><?php esc_html_e('The current vendor profile resolves to different default terms for this venue/date. Draft Pay stays as-is until you choose to apply the updated default.', 'vms'); ?></span>

                <?php if (!empty($vms_vendor_default_subtitle)): ?>
                    <div class="description vms-mt-6"><strong><?php esc_html_e('Winning source:', 'vms'); ?></strong> <?php echo esc_html((string) $vms_vendor_default_subtitle); ?></div>
                <?php endif; ?>

                <div class="description vms-mt-6"><strong><?php esc_html_e('Resolved live default:', 'vms'); ?></strong> <?php echo esc_html($vms_vendor_default_summary !== '' ? (string) $vms_vendor_default_summary : __('Configured, but no summary available.', 'vms')); ?></div>
                <div class="description vms-mt-4"><strong><?php esc_html_e('Current Draft Pay:', 'vms'); ?></strong> <?php echo esc_html($vms_actual_draft_summary !== '' ? (string) $vms_actual_draft_summary : __('Draft Pay is set, but no summary is available.', 'vms')); ?></div>

                <?php if (!empty($vms_vendor_default_source_rows) && is_array($vms_vendor_default_source_rows)): ?>
                    <div class="description vms-mt-8"><strong><?php esc_html_e('Source ladder:', 'vms'); ?></strong></div>
                        ...
                        <?php foreach ($vms_vendor_default_source_rows as $vms_source_row): ?>
                            <li>
                                ...
                                <?php if (!empty($vms_source_row['is_active'])): ?>
                                    <em><?php esc_html_e('(winning source)', 'vms'); ?></em>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                ...
                <?php if (!empty($vms_vendor_default_diff_rows) && is_array($vms_vendor_default_diff_rows)): ?>
                    <div class="description vms-mt-8"><strong><?php esc_html_e('Fields that differ:', 'vms'); ?></strong></div>
                    <ul class="description vms-mt-4 vms-mb-0" style="margin-left:18px;">
                        <?php foreach ($vms_vendor_default_diff_rows as $vms_diff_row): ?>
                            <li>
                                <strong><?php echo esc_html((string) ($vms_diff_row['label'] ?? '')); ?>:</strong>
                                <?php esc_html_e('Live', 'vms'); ?> <?php echo esc_html((string) ($vms_diff_row['live'] ?? '—')); ?> ·
                                <?php esc_html_e('Draft', 'vms'); ?> <?php echo esc_html((string) ($vms_diff_row['draft'] ?? '—')); ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
                ...
                <p class="vms-mt-8 vms-mb-0">
                    <button type="submit" name="vms_event_plan_action" value="apply_vendor_defaults" class="button button-secondary">
                        <?php esc_html_e('Apply current Primary Vendor default to Draft Pay', 'vms'); ?>
                    </button>
                </p>
            </div>
        <?php endif; ?>
        ...
        <p class="vms-mt-10 vms-mb-0">
            <label>
                ...
                <?php esc_html_e('Auto-fill Draft Pay from date defaults (Holiday overrides Venue when configured)', 'vms'); ?>
            </label>
            <br />
            <span id="vms-venue-defaults-hint" class="description"></span>
        </p>

        <strong class="vms-ep-draft-pay-title"><?php esc_html_e('Draft Pay (Editable)', 'vms'); ?></strong>
        <p class="description vms-mt-6"><?php esc_html_e('These fields stay editable even after you load a default or package.', 'vms'); ?></p>

                <div class="vms-comp-tiles" id="vms-comp-tiles" role="radiogroup" aria-label="<?php esc_attr_e('Compensation Structure', 'vms'); ?>">
            <?php
	            $vms_tile_defs = array(
	                'flat_fee' => array(
	                    'title' => __('Flat Fee', 'vms'),
	                    'sub' => __('Guaranteed payout', 'vms'),
	                ),
                'door_split' => array(
                    'title' => __('Door Split', 'vms'),
                    'sub' => __('Guaranteed payout (0) • variable payout', 'vms'),
                ),
	                'flat_fee_door_split' => array(
	                    'title' => __('Flat Fee + Door Split', 'vms'),
	                    'sub' => __('Guaranteed payout + variable split', 'vms'),
	                ),
	                'attendance_bonus' => array(
	                    'title' => __('Base + Attendance Bonus', 'vms'),
	                    'sub' => __('Guaranteed payout + variable attendance bonus', 'vms'),
	                ),
	            );
        ...
        <?php if ($vms_struct_scale_ref_max > 0): ?>
            <p class="description vms-comp-structure-scale-legend"><?php esc_html_e('Color scale: lower guaranteed pay -> higher guaranteed pay.', 'vms'); ?></p>
        <?php endif; ?>

	        <div class="vms-ep-pay-sections">
	            <div class="vms-ep-pay-section vms-ep-pay-section--vendor">
	                <div class="vms-ep-pay-section__header">
	                    <h4><?php esc_html_e('Vendor Pay', 'vms'); ?></h4>
	                    <p class="description"><?php esc_html_e('What the vendor is paid for this event.', 'vms'); ?></p>
	                </div>
	                <div class="vms-ep-draft-pay-grid">
	                    <p class="vms-comp-structure-select vms-comp-field--structure">
	                        ...
	                        <select id="vms_comp_structure" name="vms_comp_structure">
	                            ...
	                            <option value="attendance_bonus" <?php selected($comp_structure, 'attendance_bonus'); ?>><?php esc_html_e('Base + Attendance Bonus', 'vms'); ?></option>
	                        </select>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--base" data-show-when="flat_fee,flat_fee_door_split,attendance_bonus">
	                        ...
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_flat_fee_amount" name="vms_flat_fee_amount" class="vms-ep-input-sm" value="<?php echo esc_attr($flat_fee_amount); ?>" placeholder="<?php esc_attr_e('0.00', 'vms'); ?>" />
	                    ...
	                    <p class="vms-comp-field vms-comp-field--door" data-show-when="door_split,flat_fee_door_split">
	                        <label for="vms_door_split_percent"><strong><?php esc_html_e('Door Split %', 'vms'); ?></strong></label><br />
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_door_split_percent" name="vms_door_split_percent" class="vms-ep-input-sm" value="<?php echo esc_attr($door_split_percent); ?>" placeholder="<?php esc_attr_e('0', 'vms'); ?>" /> %
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--bonus-mode" data-show-when="attendance_bonus">
	                        ...
	                        <select id="vms_attendance_bonus_mode" name="vms_attendance_bonus_mode" class="vms-ep-input-sm">
	                            <option value=""><?php esc_html_e('Select bonus style', 'vms'); ?></option>
	                    ...
	                    <p class="vms-comp-field vms-comp-field--bonus-start" data-show-when="attendance_bonus">
	                        <label for="vms_attendance_bonus_start_count"><strong><?php esc_html_e('Starts After', 'vms'); ?></strong></label><br />
	                        <input type="text" inputmode="numeric" autocomplete="off" id="vms_attendance_bonus_start_count" name="vms_attendance_bonus_start_count" class="vms-ep-input-sm" value="<?php echo esc_attr($attendance_bonus_start_count); ?>" placeholder="<?php esc_attr_e('0', 'vms'); ?>" />
	                        <span class="description vms-comp-field-help"><?php esc_html_e('No attendance bonus is earned until attendance goes above this number.', 'vms'); ?></span>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--bonus-step-size" data-show-when="attendance_bonus" data-show-when-mode="step">
	                        ...
	                        <input type="text" inputmode="numeric" autocomplete="off" id="vms_attendance_bonus_step_size" name="vms_attendance_bonus_step_size" class="vms-ep-input-sm" value="<?php echo esc_attr($attendance_bonus_step_size); ?>" placeholder="<?php esc_attr_e('0', 'vms'); ?>" />
	                        <span class="description vms-comp-field-help"><?php esc_html_e('How many additional tickets are needed to earn each bonus step.', 'vms'); ?></span>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--bonus-step-bonus" data-show-when="attendance_bonus" data-show-when-mode="step">
	                        ...
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_attendance_bonus_step_bonus" name="vms_attendance_bonus_step_bonus" class="vms-ep-input-sm" value="<?php echo esc_attr($attendance_bonus_step_bonus); ?>" placeholder="<?php esc_attr_e('0.00', 'vms'); ?>" />
	                        <span class="description vms-comp-field-help"><?php esc_html_e('The amount added each time a step is reached.', 'vms'); ?></span>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--bonus-rate" data-show-when="attendance_bonus" data-show-when-mode="continuous">
	                        ...
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_attendance_bonus_per_ticket_rate" name="vms_attendance_bonus_per_ticket_rate" class="vms-ep-input-sm" value="<?php echo esc_attr($attendance_bonus_per_ticket_rate); ?>" placeholder="<?php esc_attr_e('0.00', 'vms'); ?>" />
	                        <span class="description vms-comp-field-help"><?php esc_html_e('The amount added for each ticket above the starting count.', 'vms'); ?></span>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--bonus-max" data-show-when="attendance_bonus">
	                        <label for="vms_attendance_bonus_max_bonus"><strong><?php esc_html_e('Bonus Cap', 'vms'); ?></strong></label><br />
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_attendance_bonus_max_bonus" name="vms_attendance_bonus_max_bonus" class="vms-ep-input-sm" value="<?php echo esc_attr($attendance_bonus_max_bonus); ?>" placeholder="<?php esc_attr_e('Optional', 'vms'); ?>" />
	                        <span class="description vms-comp-field-help"><?php esc_html_e('Optional cap on the total attendance bonus. Leave blank for no cap.', 'vms'); ?></span>
	                    </p>
	                </div>
	            </div>
                <div class="vms-ep-pay-section vms-ep-pay-section--deposit">
                    <div class="vms-ep-pay-section__header">
                        <h4><?php esc_html_e('Deposit / Advance', 'vms'); ?></h4>
                        <p class="description"><?php esc_html_e('Optional event-level deposit terms. These are separate from final pay and are included in Locked Pay snapshots for agreement packets.', 'vms'); ?></p>
                    </div>
                    <div class="vms-ep-agent-grid">
                        <p class="vms-comp-field vms-comp-field--deposit-amount">
                            <label for="vms_deposit_amount"><strong><?php esc_html_e('Deposit Amount', 'vms'); ?></strong></label><br />
                            <input type="text" inputmode="decimal" autocomplete="off" id="vms_deposit_amount" name="vms_deposit_amount" class="vms-ep-input-sm" value="<?php echo esc_attr($deposit_amount); ?>" placeholder="<?php esc_attr_e('0.00', 'vms'); ?>" />
                            <span class="description vms-comp-field-help"><?php esc_html_e('Leave blank when no deposit is required.', 'vms'); ?></span>
                        </p>
                        <p class="vms-comp-field vms-comp-field--deposit-status">
                            <label for="vms_deposit_status"><strong><?php esc_html_e('Status', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--deposit-treatment">
                            <label for="vms_deposit_treatment"><strong><?php esc_html_e('Treatment', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--deposit-due">
                            <label for="vms_deposit_due_date"><strong><?php esc_html_e('Due Date', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--deposit-paid">
                            <label for="vms_deposit_paid_date"><strong><?php esc_html_e('Paid Date', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--deposit-notes">
                            <label for="vms_deposit_notes"><strong><?php esc_html_e('Deposit Notes', 'vms'); ?></strong></label><br />
                            <textarea id="vms_deposit_notes" name="vms_deposit_notes" rows="2" class="large-text" placeholder="<?php esc_attr_e('Optional agreement wording or internal note.', 'vms'); ?>"><?php echo esc_textarea($deposit_notes); ?></textarea>
                            <span class="description vms-comp-field-help"><?php esc_html_e('Use this for plain-English details such as refund deadline, crediting rule, or special agreement context.', 'vms'); ?></span>
                        </p>
                    </div>
                    <?php
                ...
                <div class="vms-ep-pay-section vms-ep-pay-section--final-payment">
                    <div class="vms-ep-pay-section__header">
                        <h4><?php esc_html_e('Final Payment', 'vms'); ?></h4>
                        <p class="description"><?php esc_html_e('When and how the remaining vendor payment is expected to be paid. These terms are captured in agreement snapshots.', 'vms'); ?></p>
                    </div>
                    <div class="vms-ep-agent-grid">
                        <p class="vms-comp-field vms-comp-field--final-payment-timing">
                            <label for="vms_final_payment_timing"><strong><?php esc_html_e('Expected Final Payment', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--final-payment-days">
                            <label for="vms_final_payment_days_after"><strong><?php esc_html_e('Days After Event', 'vms'); ?></strong></label><br />
                            <input type="text" inputmode="numeric" autocomplete="off" id="vms_final_payment_days_after" name="vms_final_payment_days_after" class="vms-ep-input-sm" value="<?php echo esc_attr($final_payment_days_after); ?>" placeholder="<?php esc_attr_e('Example: 7', 'vms'); ?>" />
                            <span class="description vms-comp-field-help"><?php esc_html_e('Used when Expected Final Payment is N days after event.', 'vms'); ?></span>
                        </p>
                        <p class="vms-comp-field vms-comp-field--final-payment-date">
                            <label for="vms_final_payment_date"><strong><?php esc_html_e('Specific Pay Date', 'vms'); ?></strong></label><br />
                            ...
                            <span class="description vms-comp-field-help"><?php esc_html_e('Used when Expected Final Payment is Specific date.', 'vms'); ?></span>
                        </p>
                        <p class="vms-comp-field vms-comp-field--final-payment-custom">
                            <label for="vms_final_payment_custom_text"><strong><?php esc_html_e('Custom Timing', 'vms'); ?></strong></label><br />
                            <input type="text" id="vms_final_payment_custom_text" name="vms_final_payment_custom_text" class="regular-text" value="<?php echo esc_attr($final_payment_custom_text); ?>" placeholder="<?php esc_attr_e('Example: after settlement is approved', 'vms'); ?>" />
                        </p>
                        <p class="vms-comp-field vms-comp-field--final-payment-method">
                            <label for="vms_final_payment_method"><strong><?php esc_html_e('Payment Method', 'vms'); ?></strong></label><br />
                        ...
                        <p class="vms-comp-field vms-comp-field--final-payment-method-other">
                            <label for="vms_final_payment_method_other"><strong><?php esc_html_e('Other Method', 'vms'); ?></strong></label><br />
                            <input type="text" id="vms_final_payment_method_other" name="vms_final_payment_method_other" class="regular-text" value="<?php echo esc_attr($final_payment_method_other); ?>" placeholder="<?php esc_attr_e('Describe payment method', 'vms'); ?>" />
                        </p>
                    </div>
                    <?php
	            ...
	            <div class="vms-ep-pay-section vms-ep-pay-section--agent" data-show-when="flat_fee,flat_fee_door_split,attendance_bonus">
	                <div class="vms-ep-pay-section__header">
	                    <h4><?php esc_html_e('Agent Fee', 'vms'); ?></h4>
	                    <p class="description"><?php esc_html_e('Separate event expense. Blank or 0 means none for this event.', 'vms'); ?></p>
	                </div>
	                <div class="vms-ep-agent-grid">
	                    <p class="vms-comp-field vms-comp-field--agent-pct">
	                        <label for="vms_commission_percent"><strong><?php esc_html_e('Agent Fee %', 'vms'); ?></strong></label><br />
	                        <input type="text" inputmode="decimal" autocomplete="off" id="vms_commission_percent" name="vms_commission_percent" class="vms-ep-input-sm" value="<?php echo esc_attr($commission_percent); ?>" placeholder="<?php esc_attr_e('0', 'vms'); ?>" /> %
	                        <span class="description vms-comp-field-help"><?php esc_html_e('Tracked as its own expense, separate from vendor pay.', 'vms'); ?></span>
	                    </p>
	                    <p class="vms-comp-field vms-comp-field--agent-basis">
	                        <label for="vms_commission_mode"><strong><?php esc_html_e('Basis', 'vms'); ?></strong></label><br />
	                        <select id="vms_commission_mode" name="vms_commission_mode" class="vms-ep-input-sm">
	                            <option value="artist_fee" <?php selected($commission_mode, 'artist_fee'); ?>><?php esc_html_e('Added on top of vendor pay', 'vms'); ?></option>
	                            <option value="gross" <?php selected($commission_mode, 'gross'); ?>><?php esc_html_e('Based on gross / settlement', 'vms'); ?></option>
	                        </select>
	                    </p>
	                </div>
	        ...
	        <div id="vms-attendance-bonus-preview" class="vms-attendance-preview<?php echo $vms_is_attendance_bonus ? '' : ' vms-hidden'; ?>" data-vms-tour="event-plan.attendance-preview">
	            <strong class="vms-attendance-preview__title"><?php esc_html_e('Attendance Bonus Preview', 'vms'); ?></strong>
    ...
    <?php if ($out_of_sync): ?>
        <div class="vms-ep-card vms-ep-card--amber vms-notice vms-notice--warning">
            <strong class="vms-text-warn">⚠️ <?php esc_html_e('Draft Pay differs from Locked Snapshot', 'vms'); ?></strong><br>
            <span class="description"><?php esc_html_e('Draft Pay is newer than Locked Pay. Payout still uses Locked Pay until you lock again.', 'vms'); ?></span>
        </div>
    <?php endif; ?>
...
<div class="vms-ep-review-stack">
    <?php if (!empty($snapshot)): ?>
        <div class="vms-ep-card vms-ep-card--gray vms-ep-review-card vms-ep-review-card--locked">
            <strong><?php esc_html_e('Locked Pay Snapshot (Current payout source)', 'vms'); ?></strong><br>
            <div class="vms-mt-6">
                <?php
    ...
    <div class="vms-ep-lock-actions<?php echo $lock_pay_enabled ? '' : ' is-disabled'; ?>">
        ...
        <?php if (!$lock_pay_enabled): ?>
            <p class="description vms-ep-lock-actions__helper">
                <?php esc_html_e('Save basic event details first to enable pay locking.', 'vms'); ?>
            </p>
            <p class="vms-mt-8 vms-mb-0">
                <button type="submit" name="vms_event_plan_action" value="save_draft" class="button button-secondary">
                    <?php esc_html_e('Save Basic Details', 'vms'); ?>
                </button>
            </p>
        <?php endif; ?>
        <p class="description vms-mt-neg-4">
            <?php esc_html_e('Locks the current Draft Pay for this event so later default changes do not alter payout.', 'vms'); ?>
        </p>
    </div>
</div>
