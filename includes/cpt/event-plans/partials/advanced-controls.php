    ...
    <details id="vms-advanced-controls" class="vms-advanced-controls" data-vms-plan-id="<?php echo (int) $post->ID; ?>" data-vms-stable-draft="<?php echo $has_stable_draft ? '1' : '0'; ?>" <?php echo ((int)$linked_tec_id <= 0 ? 'open' : ''); ?>>
        <summary>
            <strong><?php esc_html_e('Advanced Controls', 'vms'); ?></strong>
            <span class="description"><?php esc_html_e('Calendar link + troubleshooting', 'vms'); ?></span>
        </summary>

        <div class="vms-advanced-controls__body">

            <p class="description"><?php esc_html_e('Most operators can ignore this section. Use it only if you are troubleshooting calendar links or you have been instructed to change these settings.', 'vms'); ?></p>
            ...
            <?php if ($is_autodraft) : ?>
                <div class="notice notice-warning inline vms-notice vms-notice--warning">
                    <p>
                        <strong><?php esc_html_e('Save Draft first.', 'vms'); ?></strong>
                        <?php esc_html_e('These advanced tools are disabled until this Event Plan has been saved at least once. This prevents losing information you have entered.', 'vms'); ?>
                    </p>
                </div>
            <?php else : ?>
            ...
            <p>
                <button type="submit" form="<?php echo esc_attr($resync_form_id); ?>" class="button button-secondary"
                    ...
                    <?php esc_html_e('Re-sync to Calendar', 'vms'); ?>
                </button>
                ...
                <?php if ($linked_tec_id > 0 && $linked_tec_url) : ?>
                    <a class="button" href="<?php echo esc_url($linked_tec_url); ?>" target="_blank" rel="noopener noreferrer">
                        <?php esc_html_e('View in Calendar', 'vms'); ?>
                    </a>
                <?php endif; ?>
                ...
                <?php if ($linked_tec_id > 0 && $tec_admin_edit_url) : ?>
                    <a class="button" href="<?php echo esc_url($tec_admin_edit_url); ?>" target="_blank" rel="noopener noreferrer">
                        <?php esc_html_e('Edit Calendar Event', 'vms'); ?>
                    </a>
                <?php endif; ?>
            </p>

            <p class="description"><?php esc_html_e('Re-sync uses saved Event Plan data only. Click Update first if you need unsaved edits included.', 'vms'); ?></p>
            ...
            <div class="vms-ticketing" data-vms-stable-draft="<?php echo $has_stable_draft ? '1' : '0'; ?>">
                ...
                ?> 
						...
						<?php if ($vms_ticketing_v2_render_mode === 'omit') : ?>
							<p class="description"><?php esc_html_e('The full GA attendance + entitlements editor now opens in the Ticketing metabox above. This panel keeps the legacy link tools and calendar troubleshooting controls.', 'vms'); ?></p>
						<?php else : ?>
						...
						<p class="description"><?php esc_html_e('Linking does not modify the calendar event. Use “Re-sync to Calendar” if you want VMS to update it.', 'vms'); ?></p>
            </div>
            ...
            >
                <label>
                    ...
                    <strong><?php esc_html_e('Allow linked calendar event to remain unpublished', 'vms'); ?></strong>
                    <?php
                        if (function_exists('vms_help_icon')) {
                            vms_help_icon(__('When enabled, this Event Plan will not show “calendar event is not published” as Needs attention. Missing or trashed calendar links are still flagged.', 'vms'));
                        }
                    ?>
                </label>
                <button type="button" class="button button-secondary vms-ml-10" id="vms-calendar-unpublished-suppress-save" disabled="disabled"><?php esc_html_e('Save warning setting', 'vms'); ?></button>
                <span id="vms-calendar-unpublished-suppress-status" class="description vms-ml-10" aria-live="polite"></span>
            </div>

            <p class="description"><?php esc_html_e('This only suppresses the “calendar event is not published” warning for this Event Plan. Missing or trashed calendar links are still flagged.', 'vms'); ?></p>
            <p class="description"><?php esc_html_e('This setting now saves separately and does not update any other Event Plan fields.', 'vms'); ?></p>

            <p class="description"><?php esc_html_e('“Re-sync to Calendar” updates the existing calendar event and will not create a duplicate.', 'vms'); ?></p>
