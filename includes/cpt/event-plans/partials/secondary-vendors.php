...
$secondary_config = array(
	...
	'labels' => array(
		'selectType' => __('-- Select a Vendor Type --', 'vms'),
		'selectVendor' => __('-- Select a Vendor --', 'vms'),
		'selectTypeFirst' => __('-- Select a Vendor Type first --', 'vms'),
		'chooseType' => __('Choose type first', 'vms'),
		'occupancyUnknown' => __('No slot limit set', 'vms'),
		'available' => __('Available', 'vms'),
		'unavailable' => __('Not available', 'vms'),
		'unknownAvailability' => __('Availability unknown', 'vms'),
		'qualified' => __('Qualified', 'vms'),
		'needsAttention' => __('Needs attention', 'vms'),
		'typeMismatch' => __('Type mismatch', 'vms'),
		'missingVendor' => __('Missing vendor', 'vms'),
		'market' => __('Market', 'vms'),
		'standard' => __('Standard', 'vms'),
		'pendingVendor' => __('Select vendor', 'vms'),
		'overCapacity' => __('Over capacity by %d', 'vms'),
		'target' => __('Target %d', 'vms'),
		'needed' => __('%d needed', 'vms'),
		'hiddenFromDispatch' => __('Hidden from ADD', 'vms'),
		'saveUnavailable' => __('Additional Vendors save is not available right now.', 'vms'),
		'saving' => __('Saving Additional Vendors…', 'vms'),
		'saveFailed' => __('Additional Vendors could not be saved. Reload the page and try again.', 'vms'),
	),
);
...
$render_secondary_vendor_status_badges = static function (array $group, int $selected_id): string {
	...
	$badges = array();
	if ($selected_id <= 0) {
		$badges[] = array('label' => __('Select vendor', 'vms'), 'variant' => 'pending');
	} else {
		if (in_array($selected_id, $group_missing, true)) {
			$badges[] = array('label' => __('Missing vendor', 'vms'), 'variant' => 'missing');
		} else {
			$availability_state = sanitize_key((string) ($pool_row['availability_state'] ?? ''));
			if ($availability_state === 'available') {
				$badges[] = array('label' => __('Available', 'vms'), 'variant' => 'available');
			} elseif ($availability_state === 'unavailable') {
				$badges[] = array('label' => __('Not available', 'vms'), 'variant' => 'unavailable');
			} else {
				$badges[] = array('label' => __('Availability unknown', 'vms'), 'variant' => 'unknown');
			}
		}

		if (in_array($selected_id, $group_mismatch, true)) {
			$badges[] = array('label' => __('Type mismatch', 'vms'), 'variant' => 'mismatch');
		}

		if (in_array($selected_id, $group_unqualified, true)) {
			$badges[] = array('label' => __('Needs attention', 'vms'), 'variant' => 'attention');
		} else {
			$badges[] = array('label' => __('Qualified', 'vms'), 'variant' => 'qualified');
		}
	}
...
$render_secondary_vendor_group_summary = static function (array $group): array {
	...
	if (!$has_type) {
		$parts[] = ($slot_limit === null)
			? sprintf(_n('%d selected', '%d selected', $filled, 'vms'), $filled)
			: sprintf(__('%1$d of %2$d filled', 'vms'), $filled, $slot_limit);
		$parts[] = __('Choose type first', 'vms');
	} else {
		$parts[] = ($slot_limit === null)
			? sprintf(_n('%d selected', '%d selected', $filled, 'vms'), $filled)
			: sprintf(__('%1$d of %2$d filled', 'vms'), $filled, $slot_limit);
		$parts[] = $is_market_group ? __('Market', 'vms') : __('Standard', 'vms');
		if ($is_market_group && $needed_slots !== null) {
			$parts[] = sprintf(__('Target %d', 'vms'), $needed_slots);
			$parts[] = $open_for_dispatch
				? sprintf(_n('%d needed', '%d needed', max(0, $needed_slots - $filled), 'vms'), max(0, $needed_slots - $filled))
				: __('Hidden from ADD', 'vms');
		}
		if ($slot_limit === null) {
			$parts[] = __('No slot limit set', 'vms');
		} elseif ($filled > $slot_limit) {
			$over_capacity = true;
			$warning = sprintf(__('Over capacity by %d', 'vms'), $filled - $slot_limit);
			$parts[] = $warning;
		}
	}
...
$render_secondary_vendor_select = static function (array $group, int $selected_id, int $group_index, int $row_index, array $secondary_group_type_options, array $type_pool_map) use ($render_secondary_vendor_status_badges): void {
	...
	if ($type_slug === '') {
		echo '<option value="">' . esc_html__('-- Select a Vendor Type first --', 'vms') . '</option>';
	} else {
	...
	echo '<button type="button" class="button button-secondary vms-secondary-vendor-remove">' . esc_html__('Remove', 'vms') . '</button>';
	echo '</div>';
	echo '</div>';
};
...
$render_secondary_vendor_group = static function (array $group, int $group_index) use ($post, $secondary_group_type_options, $secondary_mode_options, $type_pool_map, $render_secondary_vendor_select, $render_secondary_vendor_group_summary): void {
	...
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('Vendor type', 'vms') . '</span>';
	...
	echo '<option value="">' . esc_html__('-- Select a Vendor Type --', 'vms') . '</option>';
	...
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('Mode', 'vms') . '</span>';
	...
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('Slot limit / capacity', 'vms') . '</span>';
	echo '<input type="number" min="0" step="1" class="small-text vms-secondary-vendor-group-slot-limit" name="' . esc_attr(sprintf('vms_secondary_vendor_assignments[%d][slot_limit]', $group_index)) . '" value="' . esc_attr($slot_limit_display) . '" placeholder="' . esc_attr__('Use default', 'vms') . '" />';
	echo '</label>';

	echo '<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--market-target"' . $market_control_attrs . '>';
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('Market target / needed vendors', 'vms') . '</span>';
	echo '<input type="number" min="0" step="1" class="small-text vms-secondary-vendor-group-needed-slots" name="' . esc_attr(sprintf('vms_secondary_vendor_assignments[%d][needed_slots]', $group_index)) . '" value="' . esc_attr($needed_slots_display) . '" placeholder="' . esc_attr__('Blank', 'vms') . '"' . $market_input_attrs . ' />';
	echo '</label>';
	...
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('ADD visibility', 'vms') . '</span>';
	...
	echo '<span>' . esc_html__('Show this market need in ADD', 'vms') . '</span>';
	echo '</span>';
	echo '</label>';

	echo '<div class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--summary">';
	echo '<span class="vms-secondary-vendor-group__field-label">' . esc_html__('Filled', 'vms') . '</span>';
	...
	echo '<span class="vms-secondary-vendor-group__field-label screen-reader-text">' . esc_html__('Actions', 'vms') . '</span>';
	echo '<button type="button" class="button button-secondary vms-secondary-vendor-remove-group">' . esc_html__('Remove group', 'vms') . '</button>';
	echo '</div>';
	...
	echo '<span>' . esc_html__('Allow over-capacity assignment for this group.', 'vms') . '</span>';
	echo '</label>';

	if (!empty($group_missing)) {
		echo '<div class="notice notice-warning inline vms-notice vms-notice--warning"><p>';
		echo esc_html__('🚩 One or more selected vendors no longer exist (or are in the Trash). Remove or replace them below.', 'vms');
		echo '</p></div>';
	}
	...
	if (!empty($group_mismatch)) {
		...
		foreach ($group_mismatch as $vendor_id) {
			...
			$mismatch_labels[] = trim((string) ($group_titles[$vendor_id] ?? sprintf(__('Vendor #%d', 'vms'), $vendor_id)));
		}
		echo '<div class="notice notice-warning inline vms-notice vms-notice--warning"><p>';
		echo esc_html__('🚩 One or more selected vendors no longer match this vendor type. Review and re-select vendors below.', 'vms');
		if (!empty($mismatch_labels)) {
			echo ' ' . esc_html(sprintf(__('Affected vendor(s): %s', 'vms'), implode(', ', $mismatch_labels)));
		}
		echo '</p></div>';
	}
	...
	if (!empty($group_unqualified)) {
		echo '<div class="notice notice-warning inline vms-notice vms-notice--warning"><p>';
		echo esc_html__('🚩 One or more selected vendors are missing required profile items. They are still attached, but they need attention.', 'vms');
		echo '</p>';
		if (function_exists('vms_help_is_enabled') && vms_help_is_enabled()) {
			...
			foreach ($group_unqualified as $vendor_id) {
				$vendor_id = (int) $vendor_id;
				$vendor_label = trim((string) ($group_titles[$vendor_id] ?? sprintf(__('Vendor #%d', 'vms'), $vendor_id)));
				...
				echo esc_html__('Missing:', 'vms') . ' ' . esc_html(!empty($missing_items) ? implode(', ', $missing_items) : __('Unknown', 'vms')) . ' ';
				echo '<a href="' . esc_url(admin_url('post.php?post=' . $vendor_id . '&action=edit')) . '" target="_blank" rel="noopener">' . esc_html__('Edit vendor', 'vms') . '</a>';
				echo '</li>';
			}
			echo '</ul>';
		}
	...
	echo '<p class="vms-secondary-vendor-group__label">' . esc_html__('Selected vendors', 'vms') . '</p>';
	echo '<p class="description vms-secondary-vendor-group__guidance"' . ($group_has_type ? ' hidden="hidden"' : '') . '>' . esc_html__('Select a vendor type to choose eligible vendors.', 'vms') . '</p>';
	echo '</div>';
	echo '<p class="vms-secondary-vendor-actions vms-secondary-vendor-actions--inline">';
	echo '<button type="button" class="button button-secondary vms-secondary-vendor-add-row">' . esc_html__('Add vendor row', 'vms') . '</button> ';
	echo '<a class="button button-secondary" href="' . esc_url($add_secondary_vendor_url) . '" target="_blank" rel="noopener">' . esc_html__('Add new vendor', 'vms') . '</a>';
	echo '</p>';
	echo '</div>';

	echo '<div class="vms-secondary-vendor-rows-wrap">';
	echo '<div class="vms-secondary-vendor-rows__head" aria-hidden="true">';
	echo '<span>' . esc_html__('Vendor', 'vms') . '</span>';
	echo '<span>' . esc_html__('Status', 'vms') . '</span>';
	echo '<span>' . esc_html__('Action', 'vms') . '</span>';
	echo '</div>';
...
<p class="description">
	<?php esc_html_e('Attach one or more additional vendors to this event. Use separate groups for Food Vendor, Dessert Vendor, Photographer, Market Vendor, and other non-performer vendor types. These vendors will see this date as Tentative when the Event Plan is Draft/Ready and Booked once Published.', 'vms'); ?>
</p>
...
<div id="vms-secondary-vendors-section"
	...
	<p class="description vms-mt-8 vms-mb-8"><?php esc_html_e('Use Save Additional Vendors to save changes in this section.', 'vms'); ?></p>

	<?php if (!$secondary_has_saved_state) : ?>
		<div class="notice notice-info inline vms-notice vms-notice--info">
			<p><?php esc_html_e('Add a vendor group, then save this section to store your additional vendor assignments.', 'vms'); ?></p>
		</div>
	<?php endif; ?>
	...
	<?php if (!empty($secondary_missing) || !empty($secondary_mismatch) || !empty($secondary_unqualified)) : ?>
		<p class="description vms-secondary-vendor-legend">
			<?php
			echo esc_html__('Availability guide: [✓] Available, [✖] Not Available, [?] Unknown. Qualification guide: [Q✓] Qualified, [Q⚠] Needs attention.', 'vms');
			if (function_exists('vms_help_icon')) {
				vms_help_icon(__('“[Q⚠] Needs attention” means this vendor is missing required profile items (usually phone or email).', 'vms'));
			}
			?>
		</p>
	<?php endif; ?>
	...
	<p class="vms-secondary-vendor-actions">
		<button type="button" class="button button-secondary" id="vms-secondary-vendor-add-group"><?php esc_html_e('Add vendor group', 'vms'); ?></button>
		<button type="button" class="button button-secondary" id="vms-secondary-vendor-clear" <?php disabled(!$secondary_has_saved_state); ?>><?php esc_html_e('Clear additional vendors', 'vms'); ?></button>
		<button type="button" class="button button-primary" id="vms-secondary-vendor-save"><?php esc_html_e('Save Additional Vendors', 'vms'); ?></button>
	</p>
		...
		<div class="vms-secondary-vendor-group vms-secondary-vendor-group--type-pending" data-vms-group-index="" data-vms-missing-ids="[]" data-vms-mismatch-ids="[]" data-vms-unqualified-ids="[]">
			<div class="vms-secondary-vendor-group__header">
				<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--type">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('Vendor type', 'vms'); ?></span>
				...
				<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--mode">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('Mode', 'vms'); ?></span>
				...
				<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--capacity">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('Slot limit / capacity', 'vms'); ?></span>
				...
				<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--market-target" hidden="hidden">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('Market target / needed vendors', 'vms'); ?></span>
					<input type="number" min="0" step="1" class="small-text vms-secondary-vendor-group-needed-slots" value="" placeholder="<?php esc_attr_e('Blank', 'vms'); ?>" disabled="disabled" />
				</label>
				<label class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--market-dispatch" hidden="hidden">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('ADD visibility', 'vms'); ?></span>
					<span class="vms-secondary-vendor-group__checkbox-line">
						...
						<span><?php esc_html_e('Show this market need in ADD', 'vms'); ?></span>
					</span>
				</label>
				<div class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--summary">
					<span class="vms-secondary-vendor-group__field-label"><?php esc_html_e('Filled', 'vms'); ?></span>
				...
				<div class="vms-secondary-vendor-group__field vms-secondary-vendor-group__field--actions">
					<span class="vms-secondary-vendor-group__field-label screen-reader-text"><?php esc_html_e('Actions', 'vms'); ?></span>
					<button type="button" class="button button-secondary vms-secondary-vendor-remove-group"><?php esc_html_e('Remove group', 'vms'); ?></button>
				</div>
			</div>
			<label class="vms-secondary-vendor-group__override" hidden="hidden">
				...
				<span><?php esc_html_e('Allow over-capacity assignment for this group.', 'vms'); ?></span>
			</label>
			<div class="vms-secondary-vendor-group__rows-toolbar">
				<div class="vms-secondary-vendor-group__rows-copy">
					<p class="vms-secondary-vendor-group__label"><?php esc_html_e('Selected vendors', 'vms'); ?></p>
					<p class="description vms-secondary-vendor-group__guidance"><?php esc_html_e('Select a vendor type to choose eligible vendors.', 'vms'); ?></p>
				</div>
				<p class="vms-secondary-vendor-actions vms-secondary-vendor-actions--inline">
					<button type="button" class="button button-secondary vms-secondary-vendor-add-row"><?php esc_html_e('Add vendor row', 'vms'); ?></button>
					<a class="button button-secondary vms-secondary-vendor-add-new-link" href="<?php echo esc_url(add_query_arg(array(
						'post_type' => 'vms_vendor',
						'vms_return_to_event_plan' => (int) $post->ID,
						'vms_prefill_vendor_role' => 'secondary',
					), admin_url('post-new.php'))); ?>" target="_blank" rel="noopener"><?php esc_html_e('Add new vendor', 'vms'); ?></a>
				</p>
			</div>
			<div class="vms-secondary-vendor-rows-wrap">
				<div class="vms-secondary-vendor-rows__head" aria-hidden="true">
					<span><?php esc_html_e('Vendor', 'vms'); ?></span>
					<span><?php esc_html_e('Status', 'vms'); ?></span>
					<span><?php esc_html_e('Action', 'vms'); ?></span>
				</div>
	...
	<template id="vms-secondary-vendor-row-template">
		<div class="vms-secondary-vendor-row" data-vms-row-index="">
			...
			<div class="vms-secondary-vendor-row__indicators" data-vms-secondary-row-indicators>
				<span class="vms-secondary-vendor-badge vms-secondary-vendor-badge--pending"><?php esc_html_e('Select vendor', 'vms'); ?></span>
			</div>
			<div class="vms-secondary-vendor-row__action">
				<button type="button" class="button button-secondary vms-secondary-vendor-remove"><?php esc_html_e('Remove', 'vms'); ?></button>
			</div>
		</div>
	</template>
	...
	?>
	<div class="notice notice-info inline vms-notice vms-notice--info">
		<p><strong><?php esc_html_e('Vendor category sync', 'vms'); ?></strong></p>
		<?php if (!empty($vendor_category_rows)) : ?>
			<ul>
				<?php foreach ($vendor_category_rows as $category_row) : ?>
					...
					$category_label = isset($category_row['category_label']) ? (string) $category_row['category_label'] : __('Category', 'vms');
					$category_list = isset($category_row['term_names']) && is_array($category_row['term_names']) ? $category_row['term_names'] : array();
					?>
					<li>
						<strong><?php echo esc_html($source_label); ?><?php if ($vendor_title !== '') : ?>:</strong> <?php echo esc_html($vendor_title); ?><?php else : ?>:</strong> <?php esc_html_e('(not selected)', 'vms'); ?><?php endif; ?>
						<?php if (!empty($category_list)) : ?>
							- <?php echo esc_html($category_label); ?>: <?php echo esc_html(implode(', ', $category_list)); ?>
						<?php else : ?>
							- <?php printf(esc_html__('No %s selected yet.', 'vms'), esc_html(strtolower($category_label))); ?>
						<?php endif; ?>
					</li>
				<?php endforeach; ?>
			</ul>
		<?php else : ?>
			<p><?php esc_html_e('No vendor categories are attached yet. Add categories on each vendor profile, then save this Event Plan to snapshot them here.', 'vms'); ?></p>
		<?php endif; ?>
		<p class="description">
			<?php
			if (!empty($vendor_category_names)) {
				printf(
					esc_html__('TEC Event Categories that will be synced from this plan: %s', 'vms'),
					esc_html(implode(', ', $vendor_category_names))
				);
			} else {
				esc_html_e('When vendor categories exist, they will flow into this Event Plan and then into the linked TEC event categories.', 'vms');
			}
			?>
		</p>
	</div>
