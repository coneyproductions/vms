<?php defined('ABSPATH') || exit; ?>
    <div id="vms-comp-ack-wrap" class="vms-comp-ack-wrap vms-hidden">
        ...
        <div id="vms-pay-override-box" class="vms-comp-ack-section">
            <div class="description">
                <?php esc_html_e('If Draft Pay differs from this venue/date default, acknowledgment is required before Ready, Publish, or Lock.', 'backstage-venue-manager'); ?>
            </div>
            ...
            <?php if ($ack === '1' && $ack_ts > 0 && !$ack_still_valid): ?>
                <div class="description vms-pay-override-note">
                    <?php esc_html_e('Note: Draft Pay or venue/date defaults changed since your last acknowledgment. Please re-check the box before saving again.', 'backstage-venue-manager'); ?>
                </div>
            <?php endif; ?>

            <?php if ($ack === '1' && $ack_ts > 0): ?>
                <div class="description vms-pay-override-last">
                    <strong><?php esc_html_e('Last acknowledgment:', 'backstage-venue-manager'); ?></strong>
    ...
    <div id="vms-low-guarantee-box" class="vms-comp-ack-section vms-low-guarantee-box vms-notice vms-notice--warning <?php echo ($vms_requires_low_guarantee_ack ? '' : 'vms-hidden'); ?>">
        <div class="description">
            <?php esc_html_e('If Draft Pay is below the highest guaranteed tile, acknowledgment is required before Ready, Publish, or Lock.', 'backstage-venue-manager'); ?>
        </div>
        ...
        <?php if ($vms_low_ack === '1' && $vms_low_ack_ts > 0 && !$vms_low_ack_still_valid): ?>
            <div class="description vms-low-guarantee-note">
                <?php esc_html_e('Note: Draft Pay changed since your last acknowledgment. Please re-check the box before saving again.', 'backstage-venue-manager'); ?>
            </div>
        <?php endif; ?>
