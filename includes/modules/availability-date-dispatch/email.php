...
if (!function_exists('vms_add_dispatch_email_subject')) {
	function vms_add_dispatch_email_subject(array $request, array $context): string
	{
		...
		if ($title !== '' && $date !== '') {
			return sprintf(__('Availability Request: %1$s on %2$s', 'vms'), $title, $date);
		}
		if ($title !== '') {
			return sprintf(__('Availability Request: %s', 'vms'), $title);
		}

		return __('Availability Request', 'vms');
	}
}
...
if (!function_exists('vms_add_dispatch_email_body_text')) {
	function vms_add_dispatch_email_body_text(array $request, array $response, array $context): string
	{
		...
		$lines = array(
			__('Availability request from VMS', 'vms'),
			'',
		);

		if ($title !== '') {
			$lines[] = __('Event:', 'vms') . ' ' . $title;
		}
		if ($date !== '') {
			$lines[] = __('Date:', 'vms') . ' ' . $date;
		}
		if ($venue !== '') {
			$lines[] = __('Venue:', 'vms') . ' ' . $venue;
		}
		if ($message !== '') {
			$lines[] = '';
			$lines[] = $message;
		...
		if (!empty($request['include_unknown']) || !empty($request['include_no_response'])) {
			...
			$lines[] = function_exists('vms_add_dispatch_no_response_explanation')
				...
				: __('We’re reaching out because your availability for this date is not currently marked unavailable in the vendor portal.', 'vms');
		}

		$lines[] = '';
		$lines[] = __('Yes, I am available:', 'vms') . ' ' . $yes_url;
		$lines[] = __('No, I am not available:', 'vms') . ' ' . $no_url;
		$lines[] = '';
		$lines[] = __('These links are unique to you and expire automatically.', 'vms');
...
if (!function_exists('vms_add_dispatch_email_body_html')) {
	function vms_add_dispatch_email_body_html(array $request, array $response, array $context): string
	{
		...
		$html .= '<h2 style="margin:0 0 12px;">' . esc_html__('Availability Request', 'vms') . '</h2>';
		$html .= '<div style="background:#f5f8fc;border:1px solid #d7e1ee;border-radius:12px;padding:14px 16px;margin:0 0 14px;">';
		if ($title !== '') {
			$html .= '<div><strong>' . esc_html__('Event:', 'vms') . '</strong> ' . esc_html($title) . '</div>';
		}
		if ($date !== '') {
			$html .= '<div><strong>' . esc_html__('Date:', 'vms') . '</strong> ' . esc_html($date) . '</div>';
		}
		if ($venue !== '') {
			$html .= '<div><strong>' . esc_html__('Venue:', 'vms') . '</strong> ' . esc_html($venue) . '</div>';
		}
		$html .= '</div>';
		if ($message !== '') {
		...
		if (!empty($request['include_unknown']) || !empty($request['include_no_response'])) {
			$html .= '<p>' . esc_html(function_exists('vms_add_dispatch_no_response_explanation')
				? vms_add_dispatch_no_response_explanation()
				: __('We’re reaching out because your availability for this date is not currently marked unavailable in the vendor portal.', 'vms')) . '</p>';
		}
		$html .= '<div style="display:flex;flex-wrap:wrap;gap:10px;margin:18px 0 10px;">';
		$html .= '<a href="' . esc_url($yes_url) . '" style="display:inline-block;background:#1f7a4c;color:#fff;text-decoration:none;padding:12px 16px;border-radius:10px;font-weight:600;">' . esc_html__('YES - I am available', 'vms') . '</a>';
		$html .= '<a href="' . esc_url($no_url) . '" style="display:inline-block;background:#8b2d2d;color:#fff;text-decoration:none;padding:12px 16px;border-radius:10px;font-weight:600;">' . esc_html__('NO - I am not available', 'vms') . '</a>';
		$html .= '</div>';
		$html .= '<p style="color:#55657d;font-size:13px;margin:10px 0 0;">' . esc_html__('These secure links are unique to you and expire automatically.', 'vms') . '</p>';
		$html .= '</div>';
...
if (!function_exists('vms_add_dispatch_send_operator_interest_notification')) {
	function vms_add_dispatch_send_operator_interest_notification(array $request, array $response, array $context): array
	{
		...
		$subject = $event_title !== ''
			? sprintf(__('Vendor Interest: %1$s for %2$s', 'vms'), $vendor_name !== '' ? $vendor_name : __('Vendor', 'vms'), $event_title)
			: __('Vendor Interest Submitted', 'vms');
		...
		$lines = array(
			__('A vendor submitted interest through the vendor portal.', 'vms'),
			'',
		);
		if ($vendor_name !== '') {
			$lines[] = __('Vendor:', 'vms') . ' ' . $vendor_name;
		}
		if ($event_title !== '') {
			$lines[] = __('Event:', 'vms') . ' ' . $event_title;
		}
		if ($event_date !== '') {
			$lines[] = __('Date:', 'vms') . ' ' . $event_date;
		}
		if ($venue_name !== '') {
			$lines[] = __('Venue:', 'vms') . ' ' . $venue_name;
		}
		$lines[] = '';
		$lines[] = __('Review in ADD:', 'vms') . ' ' . $add_url;

		$body_text = implode("
", $lines);
		$body_html = '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;color:#15243d;line-height:1.5;">';
		$body_html .= '<h2 style="margin:0 0 12px;">' . esc_html__('Vendor Interest Submitted', 'vms') . '</h2>';
		$body_html .= '<div style="background:#f5f8fc;border:1px solid #d7e1ee;border-radius:12px;padding:14px 16px;margin:0 0 14px;">';
		if ($vendor_name !== '') {
			$body_html .= '<div><strong>' . esc_html__('Vendor:', 'vms') . '</strong> ' . esc_html($vendor_name) . '</div>';
		}
		if ($event_title !== '') {
			$body_html .= '<div><strong>' . esc_html__('Event:', 'vms') . '</strong> ' . esc_html($event_title) . '</div>';
		}
		if ($event_date !== '') {
			$body_html .= '<div><strong>' . esc_html__('Date:', 'vms') . '</strong> ' . esc_html($event_date) . '</div>';
		}
		if ($venue_name !== '') {
			$body_html .= '<div><strong>' . esc_html__('Venue:', 'vms') . '</strong> ' . esc_html($venue_name) . '</div>';
		}
		$body_html .= '</div>';
		$body_html .= '<p><a href="' . esc_url($add_url) . '" style="display:inline-block;background:#1d4ed8;color:#fff;text-decoration:none;padding:12px 16px;border-radius:10px;font-weight:700;">' . esc_html__('Review in ADD', 'vms') . '</a></p>';
		$body_html .= '</div>';
...
if (!function_exists('vms_add_dispatch_send_operator_interest_withdraw_notification')) {
	function vms_add_dispatch_send_operator_interest_withdraw_notification(array $request, array $response, array $context): array
	{
		...
		$venue_name = trim((string) ($context['venue_name'] ?? ''));
		$subject = $event_title !== ''
			? sprintf(__('Vendor Interest Withdrawn: %1$s for %2$s', 'vms'), $vendor_name !== '' ? $vendor_name : __('Vendor', 'vms'), $event_title)
			: __('Vendor Interest Withdrawn', 'vms');
		...
		$lines = array(
			__('A vendor withdrew a previously submitted portal interest request.', 'vms'),
			'',
		);
		if ($vendor_name !== '') {
			$lines[] = __('Vendor:', 'vms') . ' ' . $vendor_name;
...
", $lines);
		$body_html = '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;color:#15243d;line-height:1.5;">';
		$body_html .= '<h2 style="margin:0 0 12px;">' . esc_html__('Vendor Interest Withdrawn', 'vms') . '</h2>';
		$body_html .= '<div style="background:#f5f8fc;border:1px solid #d7e1ee;border-radius:12px;padding:14px 16px;margin:0 0 14px;">';
		if ($vendor_name !== '') {
			$body_html .= '<div><strong>' . esc_html__('Vendor:', 'vms') . '</strong> ' . esc_html($vendor_name) . '</div>';
		}
