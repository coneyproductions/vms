...
if (!function_exists('bvmgr_add_dispatch_maybe_flush_rewrites')) {
	function bvmgr_add_dispatch_maybe_flush_rewrites(): void
	{
...
add_action('admin_init', 'bvmgr_add_dispatch_maybe_flush_rewrites', 30);

if (!function_exists('bvmgr_add_dispatch_public_response_allowed_html')) {
	function bvmgr_add_dispatch_public_response_allowed_html(): array
	{
		return array(
			'a' => array(
				'class' => true,
...
if (!function_exists('bvmgr_add_dispatch_public_shell_stylesheet_url')) {
	function bvmgr_add_dispatch_public_shell_stylesheet_url(): string
	{
...
if (!function_exists('bvmgr_add_dispatch_render_public_shell')) {
	function bvmgr_add_dispatch_render_public_shell(string $headline, string $content_html): void
	{
		$stylesheet_url = bvmgr_add_dispatch_public_shell_stylesheet_url();
...
if (!function_exists('bvmgr_add_dispatch_render_public_response')) {
	function bvmgr_add_dispatch_render_public_response(string $raw_token): void
	{
		$response = bvmgr_add_dispatch_find_response_by_raw_token($raw_token);
		if (!$response) {
			bvmgr_add_dispatch_render_public_shell(
		...
		if (bvmgr_add_dispatch_response_expired($response)) {
			bvmgr_add_dispatch_render_public_shell(
				__('Availability Response', 'backstage-venue-manager'),
		...
		$choice = bvmgr_add_dispatch_get_request_choice();
		...
		$html .= '<div><strong>' . esc_html__('Date:', 'backstage-venue-manager') . '</strong> ' . esc_html(bvmgr_add_dispatch_format_date((string) ($context['event_date'] ?? ''))) . '</div>';
		...
		if ($choice !== '') {
			$result = bvmgr_add_dispatch_record_public_response($response, $choice, 'email');
			if (is_wp_error($result)) {
				...
				bvmgr_add_dispatch_render_public_shell(__('Availability Response Recorded', 'backstage-venue-manager'), $html);
			}
		}
		...
		if (in_array($current_status, array('available', 'unavailable'), true)) {
			...
			bvmgr_add_dispatch_render_public_shell(__('Availability Response', 'backstage-venue-manager'), $html);
		}
		...
		$html .= '<a class="vms-add-btn vms-add-btn--yes" href="' . esc_url(bvmgr_add_dispatch_build_response_url($response, 'available')) . '">' . esc_html__('YES - I am available', 'backstage-venue-manager') . '</a>';
		$html .= '<a class="vms-add-btn vms-add-btn--no" href="' . esc_url(bvmgr_add_dispatch_build_response_url($response, 'unavailable')) . '">' . esc_html__('NO - I am not available', 'backstage-venue-manager') . '</a>';
		$html .= '</div>';
...
if (!function_exists('bvmgr_add_dispatch_template_router')) {
	function bvmgr_add_dispatch_template_router(): void
	{
		if (is_admin()) {
			return;
		}

		$token = bvmgr_add_dispatch_get_request_token();
		if ($token === '') {
			return;
		}

		bvmgr_add_dispatch_render_public_response($token);
	}
}
add_action('template_redirect', 'bvmgr_add_dispatch_template_router', 0);
