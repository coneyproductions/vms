	...
	function vms_add_dispatch_render_public_response(string $raw_token): void
	{
		...
		if (!$response) {
			vms_add_dispatch_render_public_shell(
				__('Availability Response', 'vms'),
				'<h1>' . esc_html__('Request Not Found', 'vms') . '</h1><p class="vms-add-error">' . esc_html__('This availability link is invalid or can no longer be verified.', 'vms') . '</p>'
			);
		}
		...
		if (!$request || !$context) {
			vms_add_dispatch_render_public_shell(
				__('Availability Response', 'vms'),
				'<h1>' . esc_html__('Request Unavailable', 'vms') . '</h1><p class="vms-add-error">' . esc_html__('This request is no longer available.', 'vms') . '</p>'
			);
		}
		...
		if (sanitize_key((string) ($request['status'] ?? '')) !== 'active') {
			vms_add_dispatch_render_public_shell(
				__('Availability Response', 'vms'),
				'<h1>' . esc_html__('Request Closed', 'vms') . '</h1><p class="vms-add-note">' . esc_html__('This availability request has already been closed by the operator.', 'vms') . '</p>'
			);
		}
		...
		if (vms_add_dispatch_response_expired($response)) {
			vms_add_dispatch_render_public_shell(
				__('Availability Response', 'vms'),
				'<h1>' . esc_html__('Link Expired', 'vms') . '</h1><p class="vms-add-error">' . esc_html__('This availability link has expired. Please contact the operator if you still want to respond.', 'vms') . '</p>'
			);
		}

		$choice = vms_add_dispatch_get_request_choice();
		$current_status = sanitize_key((string) ($response['response_status'] ?? 'requested'));
		$html = '<h1>' . esc_html__('Availability Request', 'vms') . '</h1>';
		$html .= '<div class="vms-add-meta">';
		$html .= '<div><strong>' . esc_html__('Event:', 'vms') . '</strong> ' . esc_html((string) ($context['event_title'] ?? '')) . '</div>';
		$html .= '<div><strong>' . esc_html__('Date:', 'vms') . '</strong> ' . esc_html(vms_add_dispatch_format_date((string) ($context['event_date'] ?? ''))) . '</div>';
		if (!empty($context['venue_name'])) {
			$html .= '<div><strong>' . esc_html__('Venue:', 'vms') . '</strong> ' . esc_html((string) $context['venue_name']) . '</div>';
		}
		$html .= '</div>';
		...
		if ($choice !== '') {
			...
			if (is_wp_error($result)) {
				...
				$label = $final_status === 'available' ? __('Available', 'vms') : __('Unavailable', 'vms');
				$message = $already
					? sprintf(__('Your response was already recorded as %s.', 'vms'), $label)
					: sprintf(__('Your response has been recorded as %s.', 'vms'), $label);
				$html .= '<p class="vms-add-success">' . esc_html($message) . '</p>';
				$html .= '<p class="vms-add-note">' . esc_html__('Thank you. The operator can now use your response in VMS availability and Event Plan staffing decisions.', 'vms') . '</p>';
				vms_add_dispatch_render_public_shell(__('Availability Response Recorded', 'vms'), $html);
			}
		}

		if (in_array($current_status, array('available', 'unavailable'), true)) {
			$label = $current_status === 'available' ? __('Available', 'vms') : __('Unavailable', 'vms');
			$html .= '<p class="vms-add-success">' . esc_html(sprintf(__('Your response is already recorded as %s.', 'vms'), $label)) . '</p>';
			vms_add_dispatch_render_public_shell(__('Availability Response', 'vms'), $html);
		}

		if (!empty($request['message'])) {
			$html .= '<p class="vms-add-note">' . nl2br(esc_html((string) $request['message'])) . '</p>';
		}
		$html .= '<p>' . esc_html__('Choose the option below that best matches your availability for this date.', 'vms') . '</p>';
		$html .= '<div class="vms-add-actions">';
		$html .= '<a class="vms-add-btn vms-add-btn--yes" href="' . esc_url(vms_add_dispatch_build_response_url($response, 'available')) . '">' . esc_html__('YES - I am available', 'vms') . '</a>';
		$html .= '<a class="vms-add-btn vms-add-btn--no" href="' . esc_url(vms_add_dispatch_build_response_url($response, 'unavailable')) . '">' . esc_html__('NO - I am not available', 'vms') . '</a>';
		$html .= '</div>';
