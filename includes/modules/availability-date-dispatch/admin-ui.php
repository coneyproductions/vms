...
if (!function_exists('bvmgr_add_dispatch_enqueue_admin_assets')) {
	function bvmgr_add_dispatch_enqueue_admin_assets(): void
	{
		$page = bvmgr_request_read_key($_GET, 'page'); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Passive ADD admin page routing remains nonce-free while rejecting malformed page values.
		$screen = function_exists('get_current_screen') ? get_current_screen() : null;
		$is_event_plan = is_object($screen) && !empty($screen->post_type) && (string) $screen->post_type === 'vms_event_plan';
		if ($page !== bvmgr_add_dispatch_page_slug() && !$is_event_plan) {
			return;
		}

		wp_enqueue_style(
			'bvmgr-add-dispatch-admin',
			BVMGR_PLUGIN_URL . 'assets/css/vms-add-dispatch-admin.css',
			array('bvmgr-admin'),
			defined('BVMGR_VERSION') ? BVMGR_VERSION : null
		);
		if ($page === bvmgr_add_dispatch_page_slug() && current_user_can('manage_options')) {
			wp_enqueue_script(
				'bvmgr-add-dispatch-admin',
...
add_action('admin_enqueue_scripts', 'bvmgr_add_dispatch_enqueue_admin_assets', 50);

if (!function_exists('bvmgr_add_dispatch_register_admin_page')) {
	function bvmgr_add_dispatch_register_admin_page(): void
	{
		add_submenu_page(
			'vms-dashboard',
			...
			'manage_options',
			bvmgr_add_dispatch_page_slug(),
			'bvmgr_add_dispatch_render_admin_page'
		);
	}
}
add_action('admin_menu', 'bvmgr_add_dispatch_register_admin_page', 40);
if (!function_exists('bvmgr_add_dispatch_strip_menu_badge_markup')) {
	function bvmgr_add_dispatch_strip_menu_badge_markup(string $label): string
	{
		return trim((string) preg_replace('/\s*<span class="awaiting-mod vms-add-dispatch-alert-badge">.*?<\/span>\s*/i', '', $label));
	}
}

if (!function_exists('bvmgr_add_dispatch_menu_badge_markup')) {
	function bvmgr_add_dispatch_menu_badge_markup(int $count): string
	{
...
if (!function_exists('bvmgr_add_dispatch_current_pending_count')) {
	function bvmgr_add_dispatch_current_pending_count(): int
	{
		return function_exists('bvmgr_add_dispatch_pending_portal_interest_count')
			? (int) bvmgr_add_dispatch_pending_portal_interest_count()
			: 0;
	}
}

if (!function_exists('bvmgr_add_dispatch_should_render_shell_count')) {
	function bvmgr_add_dispatch_should_render_shell_count(): bool
	{
		$page = bvmgr_request_read_key($_GET, 'page'); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Passive ADD shell-count routing remains nonce-free while rejecting malformed page values.
		if ($page === bvmgr_add_dispatch_page_slug() || $page === 'vms-dashboard') {
			return true;
		}

		return function_exists('bvmgr_admin_ui_is_vms_screen') && bvmgr_admin_ui_is_vms_screen();
	}
}

if (!function_exists('bvmgr_add_dispatch_add_menu_badge')) {
	function bvmgr_add_dispatch_add_menu_badge(): void
	{
		if (!current_user_can('manage_options')) {
			return;
		}
		$count = bvmgr_add_dispatch_current_pending_count();
		if ($count <= 0) {
			return;
		}

		global $submenu, $menu;
		$markup = bvmgr_add_dispatch_menu_badge_markup($count);

		if (isset($submenu['vms-dashboard']) && is_array($submenu['vms-dashboard'])) {
			foreach ($submenu['vms-dashboard'] as $index => $item) {
				$slug = isset($item[2]) ? (string) $item[2] : '';
				if ($slug !== bvmgr_add_dispatch_page_slug()) {
					continue;
				}
				$label = function_exists('bvmgr_add_dispatch_strip_menu_badge_markup') ? bvmgr_add_dispatch_strip_menu_badge_markup((string) ($item[0] ?? __('ADD Dispatch', 'backstage-venue-manager'))) : (string) ($item[0] ?? __('ADD Dispatch', 'backstage-venue-manager'));
		...
		if (is_array($menu)) {
			foreach ($menu as $index => $item) {
				...
				$label = function_exists('bvmgr_add_dispatch_strip_menu_badge_markup') ? bvmgr_add_dispatch_strip_menu_badge_markup((string) ($item[0] ?? 'Backstage Venue Manager')) : (string) ($item[0] ?? 'Backstage Venue Manager');
...
add_action('admin_menu', 'bvmgr_add_dispatch_add_menu_badge', 1001);
add_action('admin_head', 'bvmgr_add_dispatch_add_menu_badge', 20);

if (!function_exists('bvmgr_add_dispatch_render_menu_badge_css')) {
	function bvmgr_add_dispatch_render_menu_badge_css(): void
	{
		if (!current_user_can('manage_options')) {
			return;
		}
		...
		wp_enqueue_style('bvmgr-admin-menu');
	}
}
add_action('admin_enqueue_scripts', 'bvmgr_add_dispatch_render_menu_badge_css', 21, 0);

if (!function_exists('bvmgr_add_dispatch_render_menu_badge_js')) {
	function bvmgr_add_dispatch_render_menu_badge_js(): void
	{
		if (!current_user_can('manage_options')) {
			return;
		}
...
add_action('admin_enqueue_scripts', 'bvmgr_add_dispatch_render_menu_badge_js', 50, 0);


if (!function_exists('bvmgr_add_dispatch_render_dashboard_card')) {
	function bvmgr_add_dispatch_render_dashboard_card(): void
	{
		if (!current_user_can('manage_options')) {
			return;
		}
		...
		$add_url = function_exists('bvmgr_add_dispatch_admin_url') ? bvmgr_add_dispatch_admin_url() : admin_url('admin.php?page=' . bvmgr_add_dispatch_page_slug());
...
if (!function_exists('bvmgr_add_dispatch_register_shell_page')) {
	function bvmgr_add_dispatch_register_shell_page(array $pages): array
	{
		$pages[] = bvmgr_add_dispatch_page_slug();
		return array_values(array_unique($pages));
	}
}
add_filter('vms_admin_ui_shell_pages', 'bvmgr_add_dispatch_register_shell_page');

if (!function_exists('bvmgr_add_dispatch_add_nav_item')) {
	function bvmgr_add_dispatch_add_nav_item(array $items, string $cluster_key): array
	{
		...
		if (bvmgr_add_dispatch_should_render_shell_count()) {
			$count = bvmgr_add_dispatch_current_pending_count();
			if ($count > 0) {
			$nav_label .= ' (' . $count . ')';
			}
		...
		$items[] = array(
			'label' => $nav_label,
			'url' => bvmgr_add_dispatch_admin_url(),
		);
		return $items;
	}
}
add_filter('vms_admin_ui_nav_cluster_items', 'bvmgr_add_dispatch_add_nav_item', 20, 2);

if (!function_exists('bvmgr_add_dispatch_register_tours')) {
	function bvmgr_add_dispatch_register_tours(array $tours): array
	{
		$tours[] = array(
			'id' => 'vms.add_dispatch.basics',
			'title' => __('Availability & Date Dispatch', 'backstage-venue-manager'),
			'screen' => 'admin:' . bvmgr_add_dispatch_page_slug(),
			'version' => '1.0.1',
			'level' => 'beginner',
...
add_filter('vms_tours_register', 'bvmgr_add_dispatch_register_tours');

if (!function_exists('bvmgr_add_dispatch_pill_allowed_html')) {
	function bvmgr_add_dispatch_pill_allowed_html(): array
	{
		return array(
			'span' => array(
				'class' => true,
...
if (!function_exists('bvmgr_add_dispatch_register_event_plan_metabox')) {
	function bvmgr_add_dispatch_register_event_plan_metabox(): void
	{
		add_meta_box(
			'vms_event_plan_add_dispatch',
			__('ADD Responses', 'backstage-venue-manager'),
			'bvmgr_add_dispatch_render_event_plan_metabox',
			'vms_event_plan',
			'normal',
			'low'
		);
...
add_action('add_meta_boxes', 'bvmgr_add_dispatch_register_event_plan_metabox');

if (!function_exists('bvmgr_add_dispatch_status_pill')) {
	function bvmgr_add_dispatch_status_pill(string $status): string
	{
...
if (!function_exists('bvmgr_add_dispatch_source_pill')) {
	function bvmgr_add_dispatch_source_pill(string $source): string
	{
...
if (!function_exists('bvmgr_add_dispatch_render_event_plan_metabox')) {
	function bvmgr_add_dispatch_render_event_plan_metabox(WP_Post $post): void
	{
		$context = bvmgr_add_dispatch_get_event_plan_context((int) $post->ID);
		if (!$context) {
			echo '<p>' . esc_html__('Save this Event Plan first to use Availability & Date Dispatch.', 'backstage-venue-manager') . '</p>';
			return;
		}

		$open_url = bvmgr_add_dispatch_admin_url(array('event_plan_id' => (int) $post->ID));
		$responses = bvmgr_add_dispatch_get_recent_responses_for_event_plan((int) $post->ID, 8);
		$requests = bvmgr_add_dispatch_get_requests_for_event_plan((int) $post->ID, 6);
		...
		if (!empty($requests)) {
			...
			foreach ($requests as $request) {
				$target = sanitize_key((string) ($request['target_mode'] ?? 'secondary')) === 'primary'
					...
					: (string) bvmgr_add_dispatch_type_label((string) ($request['vendor_type'] ?? ''));
				echo '<tr>';
				echo '<td>' . esc_html((string) ($request['created_at'] ?? '')) . '</td>';
				echo '<td>' . esc_html($target) . '</td>';
				echo '<td>' . wp_kses(bvmgr_add_dispatch_status_pill((string) ($request['status'] ?? 'active')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
		...
		if (!empty($responses)) {
			...
			foreach ($responses as $response) {
				...
				echo '<td>' . wp_kses(bvmgr_add_dispatch_source_pill((string) ($response['response_source'] ?? '')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
				echo '<td>' . wp_kses(bvmgr_add_dispatch_status_pill($status), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
...
if (!function_exists('bvmgr_add_dispatch_render_request_builder')) {
	function bvmgr_add_dispatch_render_request_builder(array $context, array $builder_args, array $recipients): void
	{
		...
		echo '<input type="hidden" name="page" value="' . esc_attr(bvmgr_add_dispatch_page_slug()) . '">';
		...
		$candidates = bvmgr_add_dispatch_collect_recipient_candidates($context, $builder_args);
		$included_count = count($recipients);
		$total_candidates = count($candidates);
		if (!empty($candidates)) {
			echo '<form method="post" action="' . esc_url(admin_url('admin-post.php')) . '">';
			echo '<input type="hidden" name="action" value="vms_add_dispatch_send_request">';
			wp_nonce_field('bvmgr_add_dispatch_send_request', 'bvmgr_add_dispatch_nonce');
			echo '<input type="hidden" name="event_plan_id" value="' . esc_attr((string) $event_plan_id) . '">';
			...
			foreach ($candidates as $recipient) {
				...
				echo '</td>';
				echo '<td>' . wp_kses(bvmgr_add_dispatch_status_pill((string) ($recipient['state'] ?? 'requested')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
...
if (!function_exists('bvmgr_add_dispatch_render_request_history')) {
	function bvmgr_add_dispatch_render_request_history(array $requests, int $event_plan_id = 0): void
	{
		...
		foreach ($requests as $request) {
			...
			$responses = bvmgr_add_dispatch_get_responses_for_request($request_id);
			$target = sanitize_key((string) ($request['target_mode'] ?? 'secondary')) === 'primary'
				? __('Primary vendor', 'backstage-venue-manager')
				: (string) bvmgr_add_dispatch_type_label((string) ($request['vendor_type'] ?? ''));
			$source = '';
			...
			echo '<div><strong>' . esc_html($target) . '</strong> ' . wp_kses(bvmgr_add_dispatch_source_pill($source), bvmgr_add_dispatch_pill_allowed_html()) . '<div class="description">' . esc_html((string) ($request['created_at'] ?? '')) . '</div></div>';
			echo '<div class="vms-add-request-actions">';
			echo wp_kses(bvmgr_add_dispatch_status_pill((string) ($request['status'] ?? 'active')), bvmgr_add_dispatch_pill_allowed_html());
			if (sanitize_key((string) ($request['status'] ?? 'active')) === 'active') {
				$close_url = wp_nonce_url(
					...
					'bvmgr_add_dispatch_close_request_' . $request_id
				);
			...
			if (empty($responses)) {
				...
				foreach ($responses as $response) {
					...
					$status = sanitize_key((string) ($response['response_status'] ?? 'requested'));
					echo '<tr>';
					$target_label = sanitize_key((string) ($request['target_mode'] ?? 'secondary')) === 'primary' ? __('Primary vendor', 'backstage-venue-manager') : (string) bvmgr_add_dispatch_type_label((string) ($request['vendor_type'] ?? ''));
					...
					if ($status === 'available') {
						if (trim((string) ($response['assigned_at'] ?? '')) !== '') {
							...
							$assign_url = bvmgr_add_dispatch_assignment_review_url($response_id, $event_plan_id > 0 ? $event_plan_id : (int) ($request['event_plan_id'] ?? 0));
					...
					if ($status === 'requested') {
						$resend_url = wp_nonce_url(
							...
							'bvmgr_add_dispatch_resend_response_' . $response_id
						);
...
if (!function_exists('bvmgr_add_dispatch_dashboard_filters')) {
	function bvmgr_add_dispatch_dashboard_filters(array $source = array()): array
	{
		return array(
			'show_full_groups' => bvmgr_request_read_bool_flag($source, 'show_full_groups'),
			'show_over_capacity_groups' => bvmgr_request_read_bool_flag($source, 'show_over_capacity_groups'),
			'include_past_events' => bvmgr_request_read_bool_flag($source, 'include_past_events'),
			'include_cancelled_events' => bvmgr_request_read_bool_flag($source, 'include_cancelled_events'),
		);
	}
}

if (!function_exists('bvmgr_add_dispatch_dashboard_need_rows')) {
	function bvmgr_add_dispatch_dashboard_need_rows(array $context, array $filters): array
	{
		$rows = isset($context['vendor_need_rows']) && is_array($context['vendor_need_rows'])
			? (array) $context['vendor_need_rows']
			: (function_exists('bvmgr_add_dispatch_context_vendor_need_rows') ? bvmgr_add_dispatch_context_vendor_need_rows((array) $context, true) : array());
...
if (!function_exists('bvmgr_add_dispatch_need_row_summary_label')) {
	function bvmgr_add_dispatch_need_row_summary_label(array $row): string
	{
...
if (!function_exists('bvmgr_add_dispatch_assignment_review_url')) {
	function bvmgr_add_dispatch_assignment_review_url(int $response_id, int $event_plan_id = 0): string
	{
		$args = array(
		...
		return wp_nonce_url(
			...
			'bvmgr_add_dispatch_assignment_review_' . absint($response_id)
		);
	}
}

if (!function_exists('bvmgr_add_dispatch_render_assignment_review')) {
	function bvmgr_add_dispatch_render_assignment_review(int $response_id): void
	{
		$selected_type = bvmgr_request_read_key($_GET, 'assign_as'); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Passive assignment-review selection remains nonce-free while rejecting malformed choice values.
		$review = bvmgr_add_dispatch_assignment_review($response_id, $selected_type);
		...
		if (is_wp_error($review)) {
			...
			echo '<p><a class="button" href="' . esc_url(bvmgr_add_dispatch_admin_url()) . '">' . esc_html__('Back to ADD', 'backstage-venue-manager') . '</a></p>';
			echo '</div>';
			return;
		}
		...
		echo '<div><span class="vms-add-label">' . esc_html__('Event Plan', 'backstage-venue-manager') . '</span><strong>' . esc_html((string) ($review['event_title'] ?? '')) . '</strong><div class="description">' . esc_html(bvmgr_add_dispatch_format_date((string) ($review['event_date'] ?? ''))) . '</div></div>';
		...
		if (empty($group_rows)) {
			...
			foreach ($group_rows as $group) {
				...
				echo '<td>' . wp_kses(bvmgr_add_dispatch_status_pill((string) ($group['status'] ?? 'full')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
				echo '</tr>';
			}
			echo '</tbody></table>';
		}
		...
		if ($target_mode === 'primary') {
			...
			echo '<input type="hidden" name="event_plan_id" value="' . esc_attr((string) $event_plan_id) . '">';
			wp_nonce_field('bvmgr_add_dispatch_confirm_assignment_' . $response_id, 'bvmgr_add_dispatch_assignment_nonce');
			...
			echo '<p><button type="submit" class="button button-primary">' . esc_html__('Confirm Assignment', 'backstage-venue-manager') . '</button> <a class="button" href="' . esc_url(bvmgr_add_dispatch_admin_url(array('event_plan_id' => $event_plan_id))) . '">' . esc_html__('Cancel', 'backstage-venue-manager') . '</a></p>';
			echo '</form>';
		}
		...
		if ($target_mode === 'primary') {
			...
			echo '<input type="hidden" name="event_plan_id" value="' . esc_attr((string) $event_plan_id) . '">';
			wp_nonce_field('bvmgr_add_dispatch_confirm_assignment_' . $response_id, 'bvmgr_add_dispatch_assignment_nonce');
			echo '<p><button type="submit" class="button button-primary">' . esc_html__('Confirm Primary Vendor Assignment', 'backstage-venue-manager') . '</button> <a class="button" href="' . esc_url(bvmgr_add_dispatch_admin_url(array('event_plan_id' => $event_plan_id))) . '">' . esc_html__('Cancel', 'backstage-venue-manager') . '</a></p>';
			echo '</form>';
		}
...
if (!function_exists('bvmgr_add_dispatch_render_dashboard_home')) {
	function bvmgr_add_dispatch_render_dashboard_home(): void
	{
		$counts = bvmgr_add_dispatch_get_dashboard_counts();
		$dashboard_filters = bvmgr_add_dispatch_dashboard_filters($_GET); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Passive ADD dashboard filters remain nonce-free while the helper normalizes each allowed boolean flag.
		$need_scan = function_exists('bvmgr_add_dispatch_get_event_plan_need_scan')
			? bvmgr_add_dispatch_get_event_plan_need_scan(12, 8, $dashboard_filters)
		...
		$open_events = array_values(array_filter($vendor_need_events, static function (array $context) use ($dashboard_filters): bool {
			return !empty(bvmgr_add_dispatch_dashboard_need_rows($context, $dashboard_filters));
		}));
		$open_events = array_slice($open_events, 0, 10);
		$requests = bvmgr_add_dispatch_get_recent_requests(8);
		$responses = bvmgr_add_dispatch_get_recent_responses(8);
		$portal_interest_rows = function_exists('bvmgr_add_dispatch_get_pending_portal_interest_rows')
			? bvmgr_add_dispatch_get_pending_portal_interest_rows(8)
			: array();
		...
		echo '<a class="button" href="' . esc_url(admin_url('admin.php?page=' . bvmgr_vendor_availability_page_slug())) . '">' . esc_html__('Open Vendor Availability', 'backstage-venue-manager') . '</a>';
		echo '</div>';
		echo '</div>';
		echo '</div>';
		...
		if (empty($portal_interest_rows)) {
			...
			foreach ($portal_interest_rows as $row) {
				$assign_url = bvmgr_add_dispatch_assignment_review_url((int) ($row['id'] ?? 0), (int) ($row['event_plan_id'] ?? 0));
				$dispatch_url = bvmgr_add_dispatch_admin_url(array('event_plan_id' => (int) ($row['event_plan_id'] ?? 0)));
				echo '<tr>';
				echo '<td><strong>' . esc_html((string) ($row['vendor_title'] ?? '')) . '</strong><div class="description">' . wp_kses(bvmgr_add_dispatch_source_pill('portal_interest'), bvmgr_add_dispatch_pill_allowed_html()) . '</div></td>';
				$target_label = sanitize_key((string) ($row['target_mode'] ?? 'secondary')) === 'primary'
					? __('Primary vendor', 'backstage-venue-manager')
					: (string) bvmgr_add_dispatch_type_label((string) ($row['vendor_type'] ?? ''));
		...
		if (!empty($vendor_need_events)) {
			...
			foreach ($vendor_need_events as $context) {
				$dispatch_url = bvmgr_add_dispatch_admin_url(array('event_plan_id' => (int) ($context['event_plan_id'] ?? 0)));
				$need_rows = bvmgr_add_dispatch_dashboard_need_rows((array) $context, $dashboard_filters);
				foreach ($need_rows as $group) {
					...
					echo '<td>' . esc_html(bvmgr_add_dispatch_format_date((string) ($context['event_date'] ?? ''))) . '</td>';
					echo '<td>' . esc_html((string) ($group['label'] ?? __('Vendor', 'backstage-venue-manager'))) . '</td>';
		...
		if ($need_rows_rendered <= 0) {
			...
			if (!empty($excluded_need_events)) {
				...
				foreach ($excluded_need_events as $excluded) {
					...
					echo '<td>' . esc_html((string) (($excluded['event_date'] ?? '') !== '' ? bvmgr_add_dispatch_format_date((string) $excluded['event_date']) : __('Missing date', 'backstage-venue-manager'))) . '</td>';
		...
		if (empty($open_events)) {
			...
			foreach ($open_events as $context) {
				...
				/* translators: %s: event. */
				echo '<div class="vms-add-quickstart-event__head"><strong>' . esc_html(sprintf(__('Event: %s', 'backstage-venue-manager'), (string) ($context['event_title'] ?? ''))) . '</strong><span>' . esc_html(bvmgr_add_dispatch_format_date((string) ($context['event_date'] ?? ''))) . '</span></div>';
				...
				foreach ($need_rows as $need_row) {
					echo '<li>' . esc_html(bvmgr_add_dispatch_need_row_summary_label((array) $need_row)) . '</li>';
				}
				echo '</ul>';
		...
		if (empty($responses)) {
			...
			foreach ($responses as $response) {
				...
				$target_label = sanitize_key((string) ($response['target_mode'] ?? 'secondary')) === 'primary'
					? __('Primary vendor', 'backstage-venue-manager')
					: (string) bvmgr_add_dispatch_type_label((string) ($response['vendor_type'] ?? ''));
				echo '<td>' . esc_html($target_label !== '' ? $target_label : __('Vendor', 'backstage-venue-manager')) . '</td>';
				echo '<td>' . esc_html((string) ($response['event_title'] ?? '')) . '</td>';
				echo '<td>' . wp_kses(bvmgr_add_dispatch_source_pill((string) ($response['response_source'] ?? '')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
				echo '<td>' . wp_kses(bvmgr_add_dispatch_status_pill((string) ($response['response_status'] ?? 'requested')), bvmgr_add_dispatch_pill_allowed_html()) . '</td>';
		...
		bvmgr_add_dispatch_render_request_history($requests);
	}
}

if (!function_exists('bvmgr_add_dispatch_render_admin_page_content')) {
	function bvmgr_add_dispatch_render_admin_page_content(): void
	{
		...
		$context = $event_plan_id > 0 ? bvmgr_add_dispatch_get_event_plan_context($event_plan_id) : null;

		if (!$context) {
			bvmgr_add_dispatch_render_dashboard_home();
			return;
		}

		$builder_args = bvmgr_add_dispatch_parse_builder_args($_GET, $context);
		$recipients = bvmgr_add_dispatch_collect_eligible_recipients($context, $builder_args);
		...
		echo '<div><span class="vms-add-label">' . esc_html__('Date', 'backstage-venue-manager') . '</span><strong>' . esc_html(bvmgr_add_dispatch_format_date((string) ($context['event_date'] ?? ''))) . '</strong></div>';
		...
		bvmgr_add_dispatch_render_request_builder($context, $builder_args, $recipients);
		bvmgr_add_dispatch_render_request_history(bvmgr_add_dispatch_get_requests_for_event_plan((int) $context['event_plan_id'], 12), (int) $context['event_plan_id']);
	}
}

if (!function_exists('bvmgr_add_dispatch_render_admin_page')) {
	function bvmgr_add_dispatch_render_admin_page(): void
	{
		...
		if (function_exists('bvmgr_admin_ui_render_shell')) {
			bvmgr_admin_ui_render_shell(
				...
				'bvmgr_add_dispatch_render_admin_page_content'
			);
			return;
		}
...
if (!function_exists('bvmgr_add_dispatch_handle_send_request')) {
	function bvmgr_add_dispatch_handle_send_request(): void
	{
		if (!current_user_can('manage_options')) {
		...
		if (!$context) {
			...
			wp_safe_redirect(bvmgr_add_dispatch_admin_url());
			exit;
		}

		$builder_args = bvmgr_add_dispatch_parse_builder_args($_POST, $context);
		$eligible = bvmgr_add_dispatch_collect_eligible_recipients($context, $builder_args);
		$eligible_map = array();
		foreach ($eligible as $recipient) {
			$eligible_map[(int) ($recipient['vendor_id'] ?? 0)] = $recipient;
		}

		$selected_vendor_ids = bvmgr_add_dispatch_selected_vendor_ids($_POST);
		...
		$created = bvmgr_add_dispatch_create_request($event_plan_id, $builder_args, $selected_recipients);
		if (is_wp_error($created)) {
			if (function_exists('bvmgr_add_admin_notice')) {
				bvmgr_add_admin_notice($created->get_error_message(), 'error');
			}
			wp_safe_redirect(bvmgr_add_dispatch_admin_url(array('event_plan_id' => $event_plan_id)));
			exit;
		}
		...
		$failed = 0;
		foreach ($responses as $response) {
			$result = bvmgr_add_dispatch_send_response_email($request, (array) $response, $context);
...
add_action('admin_post_vms_add_dispatch_send_request', 'bvmgr_add_dispatch_handle_send_request');

if (!function_exists('bvmgr_add_dispatch_handle_assign_vendor')) {
	function bvmgr_add_dispatch_handle_assign_vendor(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You do not have permission to perform this action.', 'backstage-venue-manager'));
		}
...
add_action('admin_post_vms_add_dispatch_assign_vendor', 'bvmgr_add_dispatch_handle_assign_vendor');

if (!function_exists('bvmgr_add_dispatch_handle_confirm_assignment')) {
	function bvmgr_add_dispatch_handle_confirm_assignment(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You do not have permission to perform this action.', 'backstage-venue-manager'));
		}
		...
		$result = bvmgr_add_dispatch_apply_assignment_review($response_id, $target_type, $allow_over_capacity);
		if (function_exists('bvmgr_add_admin_notice')) {
...
add_action('admin_post_vms_add_dispatch_confirm_assignment', 'bvmgr_add_dispatch_handle_confirm_assignment');

if (!function_exists('bvmgr_add_dispatch_handle_close_request')) {
	function bvmgr_add_dispatch_handle_close_request(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You do not have permission to perform this action.', 'backstage-venue-manager'));
		}
...
add_action('admin_post_vms_add_dispatch_close_request', 'bvmgr_add_dispatch_handle_close_request');

if (!function_exists('bvmgr_add_dispatch_handle_resend_response')) {
	function bvmgr_add_dispatch_handle_resend_response(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('You do not have permission to perform this action.', 'backstage-venue-manager'));
		}
		...
		if (is_wp_error($prepared)) {
			...
			$result = bvmgr_add_dispatch_send_response_email(
...
add_action('admin_post_vms_add_dispatch_resend_response', 'bvmgr_add_dispatch_handle_resend_response');
