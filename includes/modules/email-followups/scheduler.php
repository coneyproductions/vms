...
if (!function_exists('bvmgr_email_followups_cron_hook')) {
	function bvmgr_email_followups_cron_hook(): string
	{
...
if (!function_exists('bvmgr_email_followups_schedule_cron')) {
	function bvmgr_email_followups_schedule_cron(): void
	{
		if (function_exists('bvmgr_should_run_runtime_maintenance') && !bvmgr_should_run_runtime_maintenance()) {
			return;
		}
		if (!wp_next_scheduled(bvmgr_email_followups_cron_hook())) {
			wp_schedule_event(time() + HOUR_IN_SECONDS, 'hourly', bvmgr_email_followups_cron_hook());
		}
	}
}
add_action('init', 'bvmgr_email_followups_schedule_cron');
...
if (!function_exists('bvmgr_email_followups_due_items')) {
	function bvmgr_email_followups_due_items(): array
	{
		...
		foreach ($plans as $plan_id_raw) {
			...
			$context = bvmgr_email_followups_event_context($plan_id);
			list($allowed) = bvmgr_email_followups_context_allows_send($context);
			if (!$allowed) {
				continue;
			}
			foreach (bvmgr_email_followups_template_definitions() as $email_key => $def) {
				...
				$scheduled_ts = bvmgr_email_followups_scheduled_timestamp($plan_id, $email_key);
				if ($scheduled_ts <= 0 || $scheduled_ts > $now || $scheduled_ts < ($now - $window)) {
					continue;
				}
				if (bvmgr_email_followups_was_sent($plan_id, $email_key)) {
					continue;
				}
				$items[] = array(
...
if (!function_exists('bvmgr_email_followups_cron_run')) {
	function bvmgr_email_followups_cron_run(): void
	{
		foreach (bvmgr_email_followups_due_items() as $item) {
			bvmgr_email_followups_send_event_email((string) ($item['email_key'] ?? ''), absint($item['event_plan_id'] ?? 0), 'auto');
		}
	}
}
add_action(bvmgr_email_followups_cron_hook(), 'bvmgr_email_followups_cron_run');
