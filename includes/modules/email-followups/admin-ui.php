...
if (!function_exists('vms_email_followups_render_tabs')) {
	...
	{
		$tabs = array(
			'overview' => __('Overview', 'vms'),
			'templates' => __('Templates', 'vms'),
			'preview' => __('Preview & Test', 'vms'),
			'logs' => __('Logs', 'vms'),
		);
	...
	function vms_email_followups_render_admin_page(): void
	{
		...
		if (function_exists('vms_admin_ui_render_shell')) {
			vms_admin_ui_render_shell(array(
				'title' => __('Email Follow-Ups', 'vms'),
				'subtitle' => __('Event-aware buyer reminders, day-of updates, and post-show follow-ups.', 'vms'),
				'shell_id' => 'vms-email-followups-admin',
			), $render);
			return;
		}
		echo '<div class="wrap" id="vms-email-followups-admin"><h1>' . esc_html__('Email Follow-Ups', 'vms') . '</h1>';
		$render();
		echo '</div>';
	}
}
...
if (!function_exists('vms_email_followups_render_overview_tab')) {
	function vms_email_followups_render_overview_tab(): void
	{
		...
		echo '<article class="vms-efu-card"><h2>' . esc_html__('Status', 'vms') . '</h2>';
		echo '<p><strong>' . esc_html__('Module:', 'vms') . '</strong> ' . esc_html(!empty($settings['enabled']) ? __('Enabled', 'vms') : __('Disabled', 'vms')) . '</p>';
		echo '<p><strong>' . esc_html__('Automatic scheduled sends:', 'vms') . '</strong> ' . esc_html(!empty($settings['auto_send_enabled']) ? __('On', 'vms') : __('Off — safest first-pass default', 'vms')) . '</p>';
		echo '<p><strong>' . esc_html__('Due now:', 'vms') . '</strong> ' . esc_html((string) count($due)) . '</p>';
		echo '<p class="description">' . esc_html__('Template timing now lives on each template so it is clear what sends before, day-of, after, or manually.', 'vms') . '</p>';
		echo '</article>';

		echo '<article class="vms-efu-card"><h2>' . esc_html__('MailPoet', 'vms') . '</h2>';
		echo '<p>' . esc_html((string) ($mailpoet['message'] ?? '')) . '</p>';
		if (($mailpoet['setup_complete'] ?? null) !== null) {
			echo '<p><strong>' . esc_html__('Setup complete:', 'vms') . '</strong> ' . esc_html(!empty($mailpoet['setup_complete']) ? __('Yes', 'vms') : __('No', 'vms')) . '</p>';
		}
		if (!empty($settings['mailpoet_list_id'])) {
			echo '<p><strong>' . esc_html__('Configured list ID:', 'vms') . '</strong> ' . esc_html((string) $settings['mailpoet_list_id']) . '</p>';
		}
		echo '<p class="description">' . esc_html__('VMS sends through WordPress email. If MailPoet is configured to send site emails, MailPoet handles delivery. Optional subscriber/list sync can also be enabled below.', 'vms') . '</p>';
		echo '</article>';
		echo '</section>';
		...
		echo '<h2>' . esc_html__('Global Settings', 'vms') . '</h2>';
		echo '<p class="description">' . esc_html__('These are the shared safety, delivery, and signature settings. The “when should this send?” settings are on each template.', 'vms') . '</p>';
		echo '<div class="vms-efu-form-grid">';
		vms_email_followups_checkbox_field('vms_email_followups[enabled]', __('Enable Email Follow-Ups module', 'vms'), !empty($settings['enabled']));
		vms_email_followups_checkbox_field('vms_email_followups[auto_send_enabled]', __('Enable automatic scheduled sends', 'vms'), !empty($settings['auto_send_enabled']), __('Leave off until staging tests prove recipient discovery and templates are correct.', 'vms'));
		vms_email_followups_checkbox_field('vms_email_followups[mailpoet_sync_enabled]', __('Sync recipients to MailPoet list/tags before sending', 'vms'), !empty($settings['mailpoet_sync_enabled']));
		echo '<label><span>' . esc_html__('MailPoet list ID', 'vms') . '</span><input type="text" name="vms_email_followups[mailpoet_list_id]" value="' . esc_attr((string) $settings['mailpoet_list_id']) . '" /></label>';
		echo '<label><span>' . esc_html__('From name', 'vms') . '</span><input type="text" name="vms_email_followups[from_name]" value="' . esc_attr((string) $settings['from_name']) . '" /></label>';
		echo '<label><span>' . esc_html__('From email', 'vms') . '</span><input type="email" name="vms_email_followups[from_email]" value="' . esc_attr((string) $settings['from_email']) . '" /></label>';
		echo '<label><span>' . esc_html__('Reply-to email', 'vms') . '</span><input type="email" name="vms_email_followups[reply_to_email]" value="' . esc_attr((string) $settings['reply_to_email']) . '" /></label>';
		echo '<label><span>' . esc_html__('Default test recipient', 'vms') . '</span><input type="email" name="vms_email_followups[test_recipient]" value="' . esc_attr((string) $settings['test_recipient']) . '" /></label>';
		echo '<label><span>' . esc_html__('Due-window hours', 'vms') . '</span><input type="number" min="1" max="72" name="vms_email_followups[reminder_window_hours]" value="' . esc_attr((string) $settings['reminder_window_hours']) . '" /></label>';
		echo '</div>';
		echo '<label class="vms-efu-signature-field"><span>' . esc_html__('Default signature', 'vms') . '</span><textarea name="vms_email_followups[signature]" rows="4">' . esc_textarea((string) ($settings['signature'] ?? '')) . '</textarea><em>' . esc_html__('Use {signature} in any template. Leave this blank if you do not want a signature.', 'vms') . '</em></label>';
...
if (!function_exists('vms_email_followups_render_template_schedule_fields')) {
	function vms_email_followups_render_template_schedule_fields(string $key, array $def, string $base_name): void
	{
		...
		echo '<label><span>' . esc_html__('Send timing', 'vms') . '</span><select name="' . esc_attr($base_name . '[schedule_mode]') . '">';
		$options = array(
			'manual' => __('Manual only', 'vms'),
			'before' => __('Before event', 'vms'),
			'day_of' => __('Day of event', 'vms'),
			'after' => __('After event', 'vms'),
		);
		foreach ($options as $value => $label) {
			echo '<option value="' . esc_attr($value) . '" ' . selected($mode, $value, false) . '>' . esc_html($label) . '</option>';
		}
		echo '</select></label>';
		echo '<label><span>' . esc_html__('Days', 'vms') . '</span><input type="number" min="0" max="60" name="' . esc_attr($base_name . '[schedule_days]') . '" value="' . esc_attr((string) $days) . '" /><em>' . esc_html__('Ignored for manual/day-of templates.', 'vms') . '</em></label>';
		echo '<label><span>' . esc_html__('Send hour', 'vms') . '</span><input type="number" min="0" max="23" name="' . esc_attr($base_name . '[send_hour]') . '" value="' . esc_attr((string) $hour) . '" /><em>' . esc_html__('24-hour clock; 9 means 9:00 AM.', 'vms') . '</em></label>';
		echo '</div>';
		if (function_exists('vms_email_followups_template_timing_label')) {
			echo '<p class="description"><strong>' . esc_html__('Current timing:', 'vms') . '</strong> ' . esc_html(vms_email_followups_template_timing_label($def)) . '</p>';
		}
	}
}
...
if (!function_exists('vms_email_followups_render_templates_tab')) {
	function vms_email_followups_render_templates_tab(): void
	{
		...
		echo '<div class="vms-efu-token-box"><h2>' . esc_html__('Available Tokens', 'vms') . '</h2><p class="description">' . esc_html__('Use {customer_greeting} and {signature} for the safest human-feeling opening and closing.', 'vms') . '</p><div class="vms-efu-token-list">';
		...
		foreach (vms_email_followups_template_definitions() as $key => $def) {
			...
			echo '</div>';
			vms_email_followups_checkbox_field('vms_email_followups[templates_enabled][' . $key . ']', __('Enable this template', 'vms'), !empty($enabled[$key]));
			echo '</div>';
			if ($is_custom) {
				echo '<div class="vms-efu-form-grid">';
				echo '<label><span>' . esc_html__('Template name', 'vms') . '</span><input type="text" name="vms_email_followups[template_meta][' . esc_attr($key) . '][label]" value="' . esc_attr((string) ($def['label'] ?? '')) . '" /></label>';
				echo '<label><span>' . esc_html__('Description', 'vms') . '</span><input type="text" name="vms_email_followups[template_meta][' . esc_attr($key) . '][description]" value="' . esc_attr((string) ($def['description'] ?? '')) . '" /></label>';
				echo '</div>';
			}
			vms_email_followups_render_template_schedule_fields($key, (array) $def, 'vms_email_followups[template_meta][' . $key . ']');
			echo '<label><span>' . esc_html__('Subject', 'vms') . '</span><input type="text" name="vms_email_followups[templates][' . esc_attr($key) . '][subject]" value="' . esc_attr((string) ($template['subject'] ?? '')) . '" /></label>';
			echo '<label><span>' . esc_html__('Body', 'vms') . '</span><textarea name="vms_email_followups[templates][' . esc_attr($key) . '][body]" rows="10">' . esc_textarea((string) ($template['body'] ?? '')) . '</textarea></label>';
			echo '<p class="vms-efu-template-save"><button type="submit" class="button button-primary">' . esc_html__('Save Template Changes', 'vms') . '</button> <span class="description">' . esc_html__('Saves all template changes on this page.', 'vms') . '</span></p>';
			if ($is_custom) {
				echo '<label class="vms-efu-check vms-efu-delete-template"><input type="checkbox" name="vms_email_followups[delete_custom_templates][' . esc_attr($key) . ']" value="1" /> <span>' . esc_html__('Delete this custom template when saving', 'vms') . '</span></label>';
			}
			echo '</section>';
		}

		echo '<section class="vms-efu-template-card vms-efu-new-template">';
		echo '<h2>' . esc_html__('Add Template', 'vms') . '</h2>';
		echo '<p class="description">' . esc_html__('Create another reusable follow-up, such as a food-truck apology note, rain update, sponsor thank-you, VIP reminder, or special post-show offer.', 'vms') . '</p>';
		echo '<div class="vms-efu-form-grid">';
		echo '<label><span>' . esc_html__('Template name', 'vms') . '</span><input type="text" name="vms_email_followups[new_template][label]" value="" /></label>';
		echo '<label><span>' . esc_html__('Description', 'vms') . '</span><input type="text" name="vms_email_followups[new_template][description]" value="" /></label>';
		echo '</div>';
		...
		echo '<label><span>' . esc_html__('Subject', 'vms') . '</span><input type="text" name="vms_email_followups[new_template][subject]" value="" /></label>';
		echo '<label><span>' . esc_html__('Body', 'vms') . '</span><textarea name="vms_email_followups[new_template][body]" rows="8">' . esc_textarea("{customer_greeting}\n\n\n\n{signature}") . '</textarea></label>';
		echo '<p class="vms-efu-template-save"><button type="submit" name="vms_email_followups[create_new_template]" value="1" class="button button-primary">' . esc_html__('Save New Template', 'vms') . '</button> <span class="description">' . esc_html__('Creates the template and saves all template changes on this page.', 'vms') . '</span></p>';
		echo '</section>';

		echo '<p><button type="submit" class="button button-primary">' . esc_html__('Save Templates', 'vms') . '</button></p>';
		echo '</form>';
	}
}
...
if (!function_exists('vms_email_followups_render_preview_tab')) {
	function vms_email_followups_render_preview_tab(): void
	{
		...
		echo '<label><span>' . esc_html__('Event', 'vms') . '</span><select name="event_plan_id">';
		...
		echo '</select></label>';
		echo '<label><span>' . esc_html__('Template', 'vms') . '</span><select name="email_key">';
		foreach (vms_email_followups_template_definitions() as $key => $def) {
			echo '<option value="' . esc_attr($key) . '" ' . selected($email_key, $key, false) . '>' . esc_html((string) ($def['label'] ?? $key)) . '</option>';
		}
		echo '</select></label>';
		echo '<button type="submit" class="button">' . esc_html__('Preview', 'vms') . '</button>';
		echo '</form>';

		if ($event_plan_id <= 0) {
			echo '<div class="notice notice-warning inline"><p>' . esc_html__('No Event Plans found for preview/testing.', 'vms') . '</p></div>';
			return;
		}
		...
		echo '<article class="vms-efu-card"><h2>' . esc_html__('Recipient Preview', 'vms') . '</h2>';
		echo '<p><strong>' . esc_html__('Eligible recipients:', 'vms') . '</strong> ' . esc_html((string) count($recipients)) . '</p>';
		echo '<p><strong>' . esc_html__('Net tickets represented:', 'vms') . '</strong> ' . esc_html((string) (($recipient_result['counts']['tickets_net'] ?? 0))) . '</p>';
		echo '<p><strong>' . esc_html__('Send allowed:', 'vms') . '</strong> ' . esc_html($allowed ? __('Yes', 'vms') : __('No', 'vms')) . '</p>';
		if ($email_key === 'post_event') {
			$feedback_url = (string) ($rendered['tokens']['{feedback_url}'] ?? '');
			echo '<p><strong>' . esc_html__('Feedback link:', 'vms') . '</strong> ' . esc_html($feedback_url !== '' ? __('Included', 'vms') : __('Unavailable', 'vms')) . '</p>';
		}
		if (!$allowed) {
			echo '<p class="vms-efu-warning">' . esc_html($reason) . '</p>';
		}
		if ($scheduled_ts > 0) {
			echo '<p><strong>' . esc_html__('Scheduled timing:', 'vms') . '</strong> ' . esc_html(wp_date('M j, Y g:ia', $scheduled_ts, wp_timezone())) . '</p>';
		}
		echo '</article>';
		echo '<article class="vms-efu-card"><h2>' . esc_html__('Rendered Email', 'vms') . '</h2>';
		echo '<p><strong>' . esc_html__('Subject:', 'vms') . '</strong> ' . esc_html((string) $rendered['subject']) . '</p>';
		...
		if (!empty($batch) && (int) ($batch['event_plan_id'] ?? 0) === $event_plan_id && sanitize_key((string) ($batch['email_key'] ?? '')) === $email_key) {
			...
			echo '<h2>' . esc_html__('Manual send in progress', 'vms') . '</h2>';
			echo '<p>' . esc_html(sprintf(_n('%d selected recipient is waiting for the next send step.', '%d selected recipients are waiting for the next send step.', $remaining_count, 'vms'), $remaining_count)) . '</p>';
			...
			echo '<button type="submit" class="button button-primary">' . esc_html__('Continue Sending Next Batch', 'vms') . '</button>';
			echo '</form>';
			echo '</section>';
		}
		...
		echo '<input type="hidden" name="event_plan_id" value="' . esc_attr((string) $event_plan_id) . '" />';
		echo '<input type="hidden" name="email_key" value="' . esc_attr($email_key) . '" />';
		echo '<label><span>' . esc_html__('Send test to', 'vms') . '</span><input type="email" name="test_recipient" value="' . esc_attr((string) $settings['test_recipient']) . '" /></label> ';
		echo '<button type="submit" class="button button-primary">' . esc_html__('Send Test Email', 'vms') . '</button>';
		echo '</form>';
		...
		if (!empty($recipients)) {
			echo '<details class="vms-efu-recipient-list vms-efu-recipient-picker" open><summary>' . esc_html(sprintf(_n('Choose recipients (%d eligible; all selected by default)', 'Choose recipients (%d eligible; all selected by default)', count($recipients), 'vms'), count($recipients))) . '</summary>';
			echo '<div class="vms-efu-recipient-tools"><button type="button" class="button" data-vms-efu-select="all">' . esc_html__('Select all', 'vms') . '</button> <button type="button" class="button" data-vms-efu-select="none">' . esc_html__('Select none', 'vms') . '</button> <span class="vms-efu-selected-count" aria-live="polite"></span></div>';
			echo '<table class="widefat striped"><thead><tr><th>' . esc_html__('Send', 'vms') . '</th><th>' . esc_html__('Email', 'vms') . '</th><th>' . esc_html__('Name', 'vms') . '</th><th>' . esc_html__('Tickets', 'vms') . '</th><th>' . esc_html__('Orders', 'vms') . '</th></tr></thead><tbody>';
			...
			echo '<p class="vms-efu-warning">' . esc_html__('No eligible recipients were found for this event/template preview.', 'vms') . '</p>';
		}
		echo '<label class="vms-efu-check"><input type="checkbox" name="confirm_send" value="1" /> <span>' . esc_html__('I understand this sends to the selected eligible recipients above.', 'vms') . '</span></label>';
		echo '<p class="vms-efu-send-note">' . esc_html(sprintf(__('Manual sends are processed in batches of up to %d recipients to reduce timeout risk. If more remain, VMS will show a Continue Sending button after the page returns.', 'vms'), vms_email_followups_manual_batch_size())) . '</p>';
		echo '<p class="vms-efu-send-progress" aria-live="polite"></p>';
		echo '<button type="submit" class="button">' . esc_html__('Send to Selected Recipients', 'vms') . '</button>';
		echo '</form>';
		echo '</section>';
	}
}
...
if (!function_exists('vms_email_followups_render_logs_tab')) {
	function vms_email_followups_render_logs_tab(): void
	{
		...
		echo '<button type="submit" class="button">' . esc_html__('Clear Logs', 'vms') . '</button>';
		echo '</form>';
		if (empty($logs)) {
			echo '<p>' . esc_html__('No email follow-up logs yet.', 'vms') . '</p></section>';
			return;
		}
		echo '<table class="widefat striped"><thead><tr><th>' . esc_html__('Time', 'vms') . '</th><th>' . esc_html__('Status', 'vms') . '</th><th>' . esc_html__('Action', 'vms') . '</th><th>' . esc_html__('Template', 'vms') . '</th><th>' . esc_html__('Event', 'vms') . '</th><th>' . esc_html__('Recipient', 'vms') . '</th><th>' . esc_html__('Message', 'vms') . '</th></tr></thead><tbody>';
		foreach ($logs as $log) {
...
if (!function_exists('vms_email_followups_save_settings_post')) {
	function vms_email_followups_save_settings_post(): void
	{
		...
		vms_email_followups_redirect_notice($tab, __('Email follow-up settings saved.', 'vms'));
	}
}
...
if (!function_exists('vms_email_followups_manual_send_post')) {
	function vms_email_followups_manual_send_post(): void
	{
		...
		if (empty($_POST['confirm_send'])) {
			vms_email_followups_redirect_notice('preview', __('Manual send was not confirmed, so no recipient emails were sent.', 'vms'), 'warning', array('event_plan_id' => $event_plan_id, 'email_key' => $email_key));
		}
		...
		if ($batch_token !== '') {
			...
			if (empty($batch) || (int) ($batch['event_plan_id'] ?? 0) !== $event_plan_id || sanitize_key((string) ($batch['email_key'] ?? '')) !== $email_key) {
				vms_email_followups_redirect_notice('preview', __('The saved send batch expired or no longer matches this event/template. No emails were sent.', 'vms'), 'warning', array('event_plan_id' => $event_plan_id, 'email_key' => $email_key));
			}
			...
			if (empty($recipient_emails)) {
				vms_email_followups_redirect_notice('preview', __('No recipients were selected, so no emails were sent.', 'vms'), 'warning', array('event_plan_id' => $event_plan_id, 'email_key' => $email_key));
			}
		}
		...
		$message = sprintf(__('Manual send step complete: %d sent, %d skipped, %d errors.', 'vms'), (int) ($result['sent'] ?? 0), (int) ($result['skipped'] ?? 0), (int) ($result['errors'] ?? 0));
		if (!empty($remaining)) {
			$message .= ' ' . sprintf(_n('%d recipient remains for the next batch.', '%d recipients remain for the next batch.', count($remaining), 'vms'), count($remaining));
		}
...
if (!function_exists('vms_email_followups_clear_logs_post')) {
	function vms_email_followups_clear_logs_post(): void
	{
		...
		vms_email_followups_redirect_notice('logs', __('Email follow-up logs cleared.', 'vms'));
	}
}
		...
		$tours[] = array(
			...
			'steps' => array(
				array(
					...
					'title' => __('Start with status', 'vms'),
					'body' => __('This screen checks whether the routine is enabled, whether automatic sends are still guarded, and whether MailPoet is available for delivery/list sync.', 'vms'),
					'position' => 'bottom',
				),
				array(
					'id' => 'settings',
					'selector' => '[data-vms-tour="email-followups.settings"]',
					'title' => __('Keep the first pass safe', 'vms'),
					'body' => __('Automatic sends are intentionally separate from the module toggle. Preview recipients and send tests before allowing scheduled customer emails.', 'vms'),
					'position' => 'top',
				),
				array(
					'id' => 'preview',
					'selector' => '[data-vms-tour="email-followups.preview-controls"]',
					'title' => __('Preview each event', 'vms'),
					'body' => __('Pick an event and template to see the resolved copy, eligible buyers, skipped conditions, and the event-date-based timing before anything goes out.', 'vms'),
					'position' => 'bottom',
				),
			),
		);
