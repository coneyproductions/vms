...
if (!function_exists('bvmgr_email_followups_admin_slug')) {
	function bvmgr_email_followups_admin_slug(): string
	{
		return 'vms-email-followups';
	}
}

if (!function_exists('bvmgr_email_followups_admin_url')) {
	function bvmgr_email_followups_admin_url(array $args = array()): string
	{
		$url = admin_url('admin.php?page=' . bvmgr_email_followups_admin_slug());
		return empty($args) ? $url : add_query_arg($args, $url);
	}
}

if (!function_exists('bvmgr_email_followups_query_arg')) {
	function bvmgr_email_followups_query_arg(string $key): string
	{
...
if (!function_exists('bvmgr_email_followups_register_menu')) {
	function bvmgr_email_followups_register_menu(): void
	{
		add_submenu_page(
			'vms-dashboard',
			...
			__('Email Follow-Ups', 'backstage-venue-manager'),
			'manage_options',
			bvmgr_email_followups_admin_slug(),
			'bvmgr_email_followups_render_admin_page'
		);
	}
}
add_action('admin_menu', 'bvmgr_email_followups_register_menu', 45);

if (!function_exists('bvmgr_email_followups_register_shell_page')) {
	function bvmgr_email_followups_register_shell_page(array $pages): array
	{
		$pages[] = bvmgr_email_followups_admin_slug();
		return array_values(array_unique($pages));
	}
}
add_filter('vms_admin_ui_shell_pages', 'bvmgr_email_followups_register_shell_page');

if (!function_exists('bvmgr_email_followups_enqueue_admin_assets')) {
	function bvmgr_email_followups_enqueue_admin_assets(): void
	{
		$page = sanitize_key(bvmgr_email_followups_query_arg('page'));
		if ($page !== bvmgr_email_followups_admin_slug()) {
			return;
		}
		wp_enqueue_style(
...
add_action('admin_enqueue_scripts', 'bvmgr_email_followups_enqueue_admin_assets', 35);

if (!function_exists('bvmgr_email_followups_current_tab')) {
	function bvmgr_email_followups_current_tab(): string
	{
		$tab = sanitize_key(bvmgr_email_followups_query_arg('tab'));
		$tabs = array('overview', 'templates', 'preview', 'logs');
		return in_array($tab, $tabs, true) ? $tab : 'overview';
	}
}

if (!function_exists('bvmgr_email_followups_redirect_notice')) {
	function bvmgr_email_followups_redirect_notice(string $tab, string $notice, string $type = 'success', array $args = array()): void
	{
		...
		wp_safe_redirect(bvmgr_email_followups_admin_url($args));
		exit;
	}
}

if (!function_exists('bvmgr_email_followups_render_notices')) {
	function bvmgr_email_followups_render_notices(): void
	{
		$notice = sanitize_text_field(bvmgr_email_followups_query_arg('vms_efu_notice'));
		if ($notice === '') {
			return;
		}
		$type = sanitize_key(bvmgr_email_followups_query_arg('vms_efu_notice_type'));
...
if (!function_exists('bvmgr_email_followups_render_tabs')) {
	function bvmgr_email_followups_render_tabs(string $active): void
	{
		$tabs = array(
		...
		foreach ($tabs as $tab => $label) {
			...
			echo '<a class="nav-tab' . esc_attr($class) . '" href="' . esc_url(bvmgr_email_followups_admin_url(array('tab' => $tab))) . '">' . esc_html($label) . '</a>';
		}
		echo '</nav>';
	}
}

if (!function_exists('bvmgr_email_followups_render_admin_page')) {
	function bvmgr_email_followups_render_admin_page(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
		$tab = bvmgr_email_followups_current_tab();
		$preview_state = array();
		if ($tab === 'preview') {
			$preview_state = bvmgr_email_followups_resolve_preview_state();
		}
		$render_notices = static function () use ($tab, $preview_state): void {
			bvmgr_email_followups_render_page_notices($tab, $preview_state);
		};
		$render = static function () use ($tab, $preview_state): void {
			bvmgr_email_followups_render_tabs($tab);
			if ($tab === 'templates') {
				bvmgr_email_followups_render_templates_tab();
			} elseif ($tab === 'preview') {
				bvmgr_email_followups_render_preview_tab($preview_state);
			} elseif ($tab === 'logs') {
				bvmgr_email_followups_render_logs_tab();
			} else {
				bvmgr_email_followups_render_overview_tab();
			}
		};
...
if (!function_exists('bvmgr_email_followups_render_overview_tab')) {
	function bvmgr_email_followups_render_overview_tab(): void
	{
		$settings = bvmgr_email_followups_settings();
		$mailpoet = bvmgr_email_followups_mailpoet_status();
		$due = bvmgr_email_followups_due_items();
		...
		bvmgr_email_followups_checkbox_field('vms_email_followups[enabled]', __('Enable Email Follow-Ups module', 'backstage-venue-manager'), !empty($settings['enabled']));
		bvmgr_email_followups_checkbox_field('vms_email_followups[auto_send_enabled]', __('Enable automatic scheduled sends', 'backstage-venue-manager'), !empty($settings['auto_send_enabled']), __('Leave off until staging tests prove recipient discovery and templates are correct.', 'backstage-venue-manager'));
		bvmgr_email_followups_checkbox_field('vms_email_followups[mailpoet_sync_enabled]', __('Sync recipients to MailPoet list/tags before sending', 'backstage-venue-manager'), !empty($settings['mailpoet_sync_enabled']));
...
if (!function_exists('bvmgr_email_followups_checkbox_field')) {
	function bvmgr_email_followups_checkbox_field(string $name, string $label, bool $checked, string $description = ''): void
	{
...
if (!function_exists('bvmgr_email_followups_render_template_schedule_fields')) {
	function bvmgr_email_followups_render_template_schedule_fields(string $key, array $def, string $base_name): void
	{
		$mode = function_exists('bvmgr_email_followups_template_schedule_mode') ? bvmgr_email_followups_template_schedule_mode($def) : 'manual';
		...
		if (function_exists('bvmgr_email_followups_template_timing_label')) {
			echo '<p class="description"><strong>' . esc_html__('Current timing:', 'backstage-venue-manager') . '</strong> ' . esc_html(bvmgr_email_followups_template_timing_label($def)) . '</p>';
		}
	}
}

if (!function_exists('bvmgr_email_followups_render_templates_tab')) {
	function bvmgr_email_followups_render_templates_tab(): void
	{
		$settings = bvmgr_email_followups_settings();
		...
		foreach (bvmgr_email_followups_tokens_help() as $token => $desc) {
			echo '<code>' . esc_html($token) . '</code><span>' . esc_html($desc) . '</span>';
		}
		echo '</div></div>';

		foreach (bvmgr_email_followups_template_definitions() as $key => $def) {
			...
			echo '</div>';
			bvmgr_email_followups_checkbox_field('vms_email_followups[templates_enabled][' . $key . ']', __('Enable this template', 'backstage-venue-manager'), !empty($enabled[$key]));
			echo '</div>';
			if ($is_custom) {
				echo '<div class="vms-efu-form-grid">';
			...
			bvmgr_email_followups_render_template_schedule_fields($key, (array) $def, 'vms_email_followups[template_meta][' . $key . ']');
		...
		echo '</div>';
		bvmgr_email_followups_render_template_schedule_fields('new_template', array('kind' => 'manual', 'offset_days' => 0, 'send_hour' => 9), 'vms_email_followups[new_template]');
...
if (!function_exists('bvmgr_email_followups_selected_plan_id')) {
	/**
	 * @param array<int,WP_Post>|null $event_choices
	 */
	function bvmgr_email_followups_selected_plan_id(?array $event_choices = null): int
	{
		$raw = absint(bvmgr_email_followups_query_arg('event_plan_id'));
		if ($raw > 0) {
			return $raw;
		}
		if ($event_choices === null) {
			$event_choices = function_exists('bvmgr_email_followups_event_choices') ? bvmgr_email_followups_event_choices(1) : bvmgr_email_followups_upcoming_event_choices(1);
		}
		return !empty($event_choices[0]) && $event_choices[0] instanceof WP_Post ? (int) $event_choices[0]->ID : 0;
	}
}

if (!function_exists('bvmgr_email_followups_resolve_preview_state')) {
	/**
	 * @return array{event_plan_id:int,email_key:string,event_choices:array<int,WP_Post>,template_definitions:array<string,mixed>}
	 */
	function bvmgr_email_followups_resolve_preview_state(): array
	{
		$selected_event_plan_id = absint(bvmgr_email_followups_query_arg('event_plan_id'));
		$event_choices = function_exists('bvmgr_email_followups_event_choices')
			? bvmgr_email_followups_event_choices(120, $selected_event_plan_id)
			: bvmgr_email_followups_upcoming_event_choices(80);
		$event_plan_id = bvmgr_email_followups_selected_plan_id($event_choices);
		$email_key = sanitize_key(bvmgr_email_followups_query_arg('email_key'));
		$template_definitions = bvmgr_email_followups_template_definitions();
...
if (!function_exists('bvmgr_email_followups_render_preview_empty_state_notice')) {
	/**
	 * @param array<string,mixed> $preview_state
	 */
	function bvmgr_email_followups_render_preview_empty_state_notice(array $preview_state): void
	{
...
if (!function_exists('bvmgr_email_followups_render_page_notices')) {
	/**
	 * @param array<string,mixed> $preview_state
	 */
	function bvmgr_email_followups_render_page_notices(string $tab, array $preview_state = array()): void
	{
		...
		bvmgr_email_followups_render_preview_empty_state_notice($preview_state);
	}
}

if (!function_exists('bvmgr_email_followups_render_preview_tab')) {
	/**
	 * @param array<string,mixed> $preview_state
	 */
	function bvmgr_email_followups_render_preview_tab(array $preview_state = array()): void
	{
		...
		$event_choices = isset($preview_state['event_choices']) && is_array($preview_state['event_choices'])
			...
			: (function_exists('bvmgr_email_followups_event_choices') ? bvmgr_email_followups_event_choices(120, $event_plan_id) : bvmgr_email_followups_upcoming_event_choices(80));
		$template_definitions = isset($preview_state['template_definitions']) && is_array($preview_state['template_definitions'])
			...
			: bvmgr_email_followups_template_definitions();
		if (!isset($template_definitions[$email_key])) {
			$email_key = 'know_before';
		}
		$settings = bvmgr_email_followups_settings();
		...
		echo '<input type="hidden" name="page" value="' . esc_attr(bvmgr_email_followups_admin_slug()) . '" />';
		...
		foreach ($event_choices as $plan) {
			...
			$label = function_exists('bvmgr_email_followups_event_choice_label') ? bvmgr_email_followups_event_choice_label($plan) : ((string) get_post_meta($plan->ID, '_vms_event_date', true) . ' — ' . get_the_title($plan));
		...
		$batch_token = sanitize_key(bvmgr_email_followups_query_arg('batch_token'));
		$batch = $batch_token !== '' ? bvmgr_email_followups_batch_get($batch_token) : array();
		...
		/* translators: %d: number used in this message. */
		echo '<p class="vms-efu-send-note">' . esc_html(sprintf(__('Manual sends are processed in batches of up to %d recipients to reduce timeout risk. If more remain, Backstage Venue Manager will show a Continue Sending button after the page returns.', 'backstage-venue-manager'), bvmgr_email_followups_manual_batch_size())) . '</p>';
		echo '<p class="vms-efu-send-progress" aria-live="polite"></p>';
...
if (!function_exists('bvmgr_email_followups_render_logs_tab')) {
	function bvmgr_email_followups_render_logs_tab(): void
	{
...
if (!function_exists('bvmgr_email_followups_save_settings_post')) {
	function bvmgr_email_followups_save_settings_post(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
		...
		$input['_tab'] = $tab;
		update_option(bvmgr_email_followups_option_key(), bvmgr_email_followups_sanitize_settings($input), false);
		bvmgr_email_followups_redirect_notice($tab, __('Email follow-up settings saved.', 'backstage-venue-manager'));
	}
}
add_action('admin_post_vms_email_followups_save_settings', 'bvmgr_email_followups_save_settings_post');
...
if (!function_exists('bvmgr_email_followups_batch_transient_key')) {
	function bvmgr_email_followups_batch_transient_key(string $token): string
	{
...
if (!function_exists('bvmgr_email_followups_batch_create')) {
	function bvmgr_email_followups_batch_create(int $event_plan_id, string $email_key, array $emails): string
	{
		...
		set_transient(bvmgr_email_followups_batch_transient_key($token), array(
...
if (!function_exists('bvmgr_email_followups_batch_get')) {
	function bvmgr_email_followups_batch_get(string $token): array
	{
		$token = sanitize_key($token);
		if ($token === '') {
			return array();
		}
		$data = get_transient(bvmgr_email_followups_batch_transient_key($token));
		return is_array($data) ? $data : array();
	}
}

if (!function_exists('bvmgr_email_followups_batch_clear')) {
	function bvmgr_email_followups_batch_clear(string $token): void
	{
		$token = sanitize_key($token);
		if ($token !== '') {
			delete_transient(bvmgr_email_followups_batch_transient_key($token));
		}
	}
}
...
if (!function_exists('bvmgr_email_followups_manual_send_post')) {
	function bvmgr_email_followups_manual_send_post(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
		...
		if ($batch_token !== '') {
			...
			$selected = isset($_POST['selected_recipients']) && is_array($_POST['selected_recipients'])
				...
				: array();
			$recipient_emails = function_exists('bvmgr_email_followups_normalize_recipient_emails') ? bvmgr_email_followups_normalize_recipient_emails($selected) : array();
			if (empty($recipient_emails)) {
				bvmgr_email_followups_redirect_notice('preview', __('No recipients were selected, so no emails were sent.', 'backstage-venue-manager'), 'warning', array('event_plan_id' => $event_plan_id, 'email_key' => $email_key));
			}
		}
		...
		bvmgr_email_followups_redirect_notice('preview', $message, !empty($result['ok']) ? 'success' : 'warning', $args);
	}
}
add_action('admin_post_vms_email_followups_manual_send', 'bvmgr_email_followups_manual_send_post');

if (!function_exists('bvmgr_email_followups_clear_logs_post')) {
	function bvmgr_email_followups_clear_logs_post(): void
	{
		if (!current_user_can('manage_options')) {
			wp_die(esc_html__('Insufficient permissions.', 'backstage-venue-manager'));
		}
...
add_action('admin_post_vms_email_followups_clear_logs', 'bvmgr_email_followups_clear_logs_post');

if (!function_exists('bvmgr_email_followups_register_tours')) {
	function bvmgr_email_followups_register_tours(array $tours): array
	{
		$tours[] = array(
			'id' => 'vms.email_followups.basics',
			'title' => __('Email Follow-Ups', 'backstage-venue-manager'),
			'screen' => 'admin:' . bvmgr_email_followups_admin_slug(),
			'version' => '1.0.0',
			'level' => 'beginner',
...
add_filter('vms_tours_register', 'bvmgr_email_followups_register_tours');
