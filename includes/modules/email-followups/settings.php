...
if (!function_exists('vms_email_followups_option_key')) {
	function vms_email_followups_option_key(): string
	{
		return 'vms_email_followups_settings';
	}
}

if (!function_exists('vms_email_followups_default_signature')) {
	function vms_email_followups_default_signature(): string
	{
		return "See you at the show,\nSerenade Range";
	}
}

if (!function_exists('vms_email_followups_default_template_meta')) {
	function vms_email_followups_default_template_meta(): array
	{
		return array(
...
if (!function_exists('vms_email_followups_default_settings')) {
	function vms_email_followups_default_settings(): array
	{
		...
		return array(
			...
			'signature' => vms_email_followups_default_signature(),
			...
			'template_meta' => vms_email_followups_default_template_meta(),
			'custom_templates' => array(),
			'templates' => function_exists('vms_email_followups_default_templates') ? vms_email_followups_default_templates() : array(),
		);
	}
}
...
if (!function_exists('vms_email_followups_settings')) {
	...
	{
		$defaults = vms_email_followups_default_settings();
		$stored = get_option(vms_email_followups_option_key(), array());
		if (!is_array($stored)) {
			$stored = array();
		}
...
if (!function_exists('vms_email_followups_unique_custom_key')) {
	function vms_email_followups_unique_custom_key(string $label, array $existing_keys): string
	{
...
if (!function_exists('vms_email_followups_sanitize_template_schedule')) {
	function vms_email_followups_sanitize_template_schedule(array $input, array $existing = array()): array
	{
...
if (!function_exists('vms_email_followups_clean_template_text')) {
	function vms_email_followups_clean_template_text(string $text): string
	{
...
if (!function_exists('vms_email_followups_sanitize_settings')) {
	function vms_email_followups_sanitize_settings(array $input): array
	{
		$defaults = vms_email_followups_default_settings();
		$current = vms_email_followups_settings();
		$out = $current;
		...
		if ($updating_global) {
			...
			$out['mailpoet_list_id'] = isset($input['mailpoet_list_id']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($input['mailpoet_list_id']))) : '';
			$out['from_name'] = isset($input['from_name']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($input['from_name']))) : (string) $defaults['from_name'];
			...
			$out['signature'] = isset($input['signature']) ? wp_kses_post(vms_email_followups_clean_template_text((string) wp_unslash($input['signature']))) : (string) $defaults['signature'];
		}

		if ($updating_templates) {
			$builtin_defs = function_exists('vms_email_followups_builtin_template_definitions') ? vms_email_followups_builtin_template_definitions() : array();
			$current_defs = function_exists('vms_email_followups_template_definitions') ? vms_email_followups_template_definitions() : $builtin_defs;
			...
			foreach ($current_defs as $key => $def) {
				...
				$out['template_meta'][$key] = vms_email_followups_sanitize_template_schedule(isset($meta_in[$key]) && is_array($meta_in[$key]) ? (array) $meta_in[$key] : array(), $existing_schedule);
				...
				$out['templates'][$key] = array(
					'subject' => isset($incoming['subject']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($incoming['subject']))) : (string) ($existing['subject'] ?? ''),
					'body' => isset($incoming['body']) ? wp_kses_post(vms_email_followups_clean_template_text((string) wp_unslash($incoming['body']))) : (string) ($existing['body'] ?? ''),
				);
			}
			...
			$new_label = isset($new['label']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($new['label']))) : '';
			$new_subject = isset($new['subject']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($new['subject']))) : '';
			...
			if ($new_requested && ($new_label !== '' || $new_subject !== '' || $new_body !== '')) {
				...
				$description = isset($new['description']) ? sanitize_text_field(vms_email_followups_clean_template_text((string) wp_unslash($new['description']))) : '';
...
if (!function_exists('vms_email_followups_migrate_template_encoding_609')) {
	function vms_email_followups_migrate_template_encoding_609(): void
	{
		...
		$settings = get_option(vms_email_followups_option_key(), array());
		...
		$clean_value = static function ($value) use (&$changed) {
			...
			$cleaned = vms_email_followups_clean_template_text($value);
			if ($cleaned !== $value) {
				$changed = true;
			}
			return $cleaned;
		...
		if ($changed) {
			update_option(vms_email_followups_option_key(), $settings, false);
		}
		update_option($flag, '1', false);
	}
}
add_action('init', 'vms_email_followups_migrate_template_encoding_609', 24);

if (!function_exists('vms_email_followups_migrate_feedback_template_605')) {
	function vms_email_followups_migrate_feedback_template_605(): void
	{
...
add_action('init', 'vms_email_followups_migrate_feedback_template_605', 25);

if (!function_exists('vms_email_followups_migrate_customer_greeting_template_607')) {
	function vms_email_followups_migrate_customer_greeting_template_607(): void
	{
		...
		$templates = is_array($settings['templates'] ?? null) ? (array) $settings['templates'] : array();
		$changed = false;
		foreach (array_keys(function_exists('vms_email_followups_template_definitions') ? vms_email_followups_template_definitions() : array()) as $key) {
...
add_action('init', 'vms_email_followups_migrate_customer_greeting_template_607', 26);

if (!function_exists('vms_email_followups_migrate_signature_template_608')) {
	function vms_email_followups_migrate_signature_template_608(): void
	{
		...
		if (!isset($settings['signature']) || trim((string) $settings['signature']) === '') {
			$settings['signature'] = vms_email_followups_default_signature();
		}
		$templates = is_array($settings['templates'] ?? null) ? (array) $settings['templates'] : array();
		$changed = false;
		foreach (array_keys(function_exists('vms_email_followups_template_definitions') ? vms_email_followups_template_definitions() : array()) as $key) {
...
add_action('init', 'vms_email_followups_migrate_signature_template_608', 27);

if (!function_exists('vms_email_followups_migrate_duplicate_empty_custom_templates_610')) {
	function vms_email_followups_migrate_duplicate_empty_custom_templates_610(): void
	{
...
add_action('init', 'vms_email_followups_migrate_duplicate_empty_custom_templates_610', 28);
