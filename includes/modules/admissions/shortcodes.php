...
if (!function_exists('vms_door_checkin_shortcode')) {
	function vms_door_checkin_shortcode($atts = array()): string
	{
		if (!is_user_logged_in() || !vms_admission_current_user_can_checkin()) {
			return '<div class="vms-door-checkin-denied">' . esc_html__('Access denied.', 'vms') . '</div>';
		}
		...
		<div class="vms-door-checkin" id="vms-door-checkin-root">
			<div class="vms-door-toolbar">
				<label class="vms-door-field">
					<span><?php esc_html_e('Select Event Plan', 'vms'); ?></span>
					<select id="vms-door-event-plan"></select>
				</label>
				<input type="search" id="vms-door-search" placeholder="<?php echo esc_attr__('Scan QR or search name/phone', 'vms'); ?>" autocomplete="off">
				<button type="button" class="vms-door-scan-submit" id="vms-door-scan-submit"><?php esc_html_e('Scan / Check In', 'vms'); ?></button>
			</div>
			<div class="vms-door-filters" id="vms-door-filters">
				<button type="button" data-status="active" class="is-active"><?php esc_html_e('Active', 'vms'); ?></button>
				<button type="button" data-status="checked_in"><?php esc_html_e('Checked In', 'vms'); ?></button>
				<button type="button" data-status="all"><?php esc_html_e('All', 'vms'); ?></button>
			</div>
