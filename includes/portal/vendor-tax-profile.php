...
function vms_vendor_portal_render_tax_profile($vendor_id)
{
	...
	echo '<h3 class="vms-vtp-subhead vms-vtp-subhead-first">' . esc_html__('Payee & Entity', 'vms') . '</h3>';
	echo '<div class="vms-grid vms-vtp-grid">';

	echo '<div class="vms-field">';
	echo '<label for="vms_payee_legal_name">' . esc_html__('Legal / Payee Name', 'vms') . '</label>';
	echo '<input type="text" id="vms_payee_legal_name" name="vms_payee_legal_name" value="' . esc_attr($payee_legal) . '" required>';
	echo '</div>';

	echo '<div class="vms-field">';
	echo '<label for="vms_payee_dba">' . esc_html__('DBA (Optional)', 'vms') . '</label>';
	...
	echo '<h3 class="vms-vtp-subhead">' . esc_html__('Mailing Address', 'vms') . '</h3>';
	echo '<div class="vms-grid vms-vtp-grid">';
	...
	echo '<h3 class="vms-vtp-subhead">' . esc_html__('W-9', 'vms') . ' (' . esc_html($provider_label) . ')</h3>';
	...
	if ($provider === 'upload') {
		...
		if ($w9_url) {
			echo '<p class="description vms-vtp-upload-note">' .
				esc_html__('W-9 on file:', 'vms') . ' <a href="' . esc_url($w9_url) . '" target="_blank" rel="noopener">' . esc_html__('View', 'vms') . '</a>' .
				'</p>';
		}

		echo '<div class="vms-field vms-vtp-field-wide">';
		echo '<label for="vms_w9_upload">' . esc_html__('W-9 Upload (PDF/image)', 'vms') . '</label>';
		...
		echo '<span>' . esc_html__('I have completed my W-9/tax information using the secure email request.', 'vms') . '</span>';
		echo '</label>';
		echo '</div>';
	}
