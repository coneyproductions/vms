...
if (!function_exists('bvmgr_square_firewall_square_meta_keys')) {
    /**
     * Square product-link metadata used by WooCommerce Square across recent builds.
     *
     * @return string[]
     */
    function bvmgr_square_firewall_square_meta_keys(): array
    {
        return array(
            '_wc_square_synced',
            '_square_item_id',
...
if (!function_exists('bvmgr_square_firewall_protected_category_slugs')) {
    /**
     * @return string[]
     */
    function bvmgr_square_firewall_protected_category_slugs(): array
    {
...
if (!function_exists('bvmgr_square_firewall_product_meta_key')) {
    function bvmgr_square_firewall_product_meta_key(string $which): string
    {
...
if (!function_exists('bvmgr_square_firewall_product_id_from_value')) {
    /**
     * @param mixed $value
     */
    function bvmgr_square_firewall_product_id_from_value($value): int
    {
        if (is_numeric($value)) {
            return absint($value);
        }
...
if (!function_exists('bvmgr_square_firewall_extract_product_id')) {
    /**
     * @param mixed ...$values
     */
    function bvmgr_square_firewall_extract_product_id(...$values): int
    {
        foreach ($values as $value) {
            $id = bvmgr_square_firewall_product_id_from_value($value);
            if ($id > 0) {
                return $id;
            }
        }
...
if (!function_exists('bvmgr_square_firewall_product_category_slugs')) {
    /**
     * @return string[]
     */
    function bvmgr_square_firewall_product_category_slugs(int $product_id): array
    {
...
if (!function_exists('bvmgr_square_firewall_get_sku')) {
    function bvmgr_square_firewall_get_sku(int $product_id): string
    {
        $product_id = absint($product_id);
        if ($product_id <= 0) {
            return '';
...
if (!function_exists('bvmgr_square_firewall_is_protected_sku')) {
    function bvmgr_square_firewall_is_protected_sku(string $sku): bool
    {
        $sku = trim($sku);
        if ($sku === '') {
            return false;
...
if (!function_exists('bvmgr_square_firewall_is_protected_role')) {
    function bvmgr_square_firewall_is_protected_role(string $role): bool
    {
...
if (!function_exists('bvmgr_square_firewall_classify_product')) {
    /**
     * Returns a human-readable protection reason or an empty string when not protected.
     */
    function bvmgr_square_firewall_classify_product(int $product_id): string
    {
        $product_id = absint($product_id);
        if ($product_id <= 0) {
            return '';
        ...
        $sku = bvmgr_square_firewall_get_sku($product_id);
        if (bvmgr_square_firewall_is_protected_sku($sku)) {
            return 'vms_sku';
        }

        $role_key = bvmgr_square_firewall_product_meta_key('product_role');
        $role = $role_key !== '' ? sanitize_key((string) get_post_meta($product_id, $role_key, true)) : '';
        if (bvmgr_square_firewall_is_protected_role($role)) {
            return 'vms_product_role_' . $role;
        }

        $ent_key = bvmgr_square_firewall_product_meta_key('ticketing_entitlement_id');
        if ($ent_key !== '' && sanitize_key((string) get_post_meta($product_id, $ent_key, true)) !== '') {
            return 'vms_ticketing_entitlement';
        }

        $category_slugs = bvmgr_square_firewall_product_category_slugs($product_id);
        $protected_slugs = bvmgr_square_firewall_protected_category_slugs();
        ...
        $marker_key = bvmgr_square_firewall_product_meta_key('ticketing_marker_version');
        $provider_key = bvmgr_square_firewall_product_meta_key('ticketing_source_provider');
        $plan_key = bvmgr_square_firewall_product_meta_key('event_plan_id');
...
if (!function_exists('bvmgr_square_firewall_is_protected_product')) {
    function bvmgr_square_firewall_is_protected_product(int $product_id): bool
    {
        return bvmgr_square_firewall_classify_product($product_id) !== '';
    }
}

if (!function_exists('bvmgr_square_firewall_has_square_link')) {
    function bvmgr_square_firewall_has_square_link(int $product_id): bool
    {
        $product_id = absint($product_id);
        if ($product_id <= 0) {
            return false;
        ...
        foreach (bvmgr_square_firewall_square_meta_keys() as $meta_key) {
...
if (!function_exists('bvmgr_square_firewall_set_square_sync_no')) {
    function bvmgr_square_firewall_set_square_sync_no(int $product_id): bool
    {
        $product_id = absint($product_id);
        if ($product_id <= 0) {
            return false;
...
if (!function_exists('bvmgr_square_firewall_protect_product')) {
     ...
     */
    function bvmgr_square_firewall_protect_product(int $product_id, bool $clear_square_meta = true): array
    {
        $product_id = absint($product_id);
        $reason = bvmgr_square_firewall_classify_product($product_id);
        $result = array(
        ...
        $result['sync_changed'] = bvmgr_square_firewall_set_square_sync_no($product_id);
...
if (!function_exists('bvmgr_square_firewall_protect_product_and_children')) {
    /**
     * @return array<int,array<string,mixed>>
     */
    function bvmgr_square_firewall_protect_product_and_children(int $product_id, bool $clear_square_meta = true): array
    {
        $product_id = absint($product_id);
        if ($product_id <= 0) {
            return array();
        ...
        foreach (array_values(array_unique(array_filter($ids))) as $id) {
            $results[$id] = bvmgr_square_firewall_protect_product($id, $clear_square_meta);
        }
...
if (!function_exists('bvmgr_square_firewall_on_product_save')) {
    /**
     * @param mixed $post_or_product
     */
    function bvmgr_square_firewall_on_product_save($product_id, $post_or_product = null, $update = null): void
    {
        ...
        if (bvmgr_square_firewall_is_protected_product($product_id)) {
            bvmgr_square_firewall_protect_product_and_children($product_id, true);
        }
    }
}
add_action('save_post_product', 'bvmgr_square_firewall_on_product_save', 99, 3);
add_action('woocommerce_update_product', 'bvmgr_square_firewall_on_product_save', 99, 1);

if (!function_exists('bvmgr_square_firewall_on_marker_meta_change')) {
    /**
     * @param mixed $meta_id
     * @param mixed $meta_value
     */
    function bvmgr_square_firewall_on_marker_meta_change($meta_id, int $object_id, string $meta_key, $meta_value = null): void
    {
        ...
        $watched = array_filter(array(
            bvmgr_square_firewall_product_meta_key('product_role'),
            bvmgr_square_firewall_product_meta_key('ticketing_entitlement_id'),
            bvmgr_square_firewall_product_meta_key('ticketing_marker_version'),
            bvmgr_square_firewall_product_meta_key('ticketing_source_provider'),
        ...
        if (bvmgr_square_firewall_is_protected_product($object_id)) {
            bvmgr_square_firewall_protect_product_and_children($object_id, true);
        }
    }
}
add_action('added_post_meta', 'bvmgr_square_firewall_on_marker_meta_change', 99, 4);
add_action('updated_post_meta', 'bvmgr_square_firewall_on_marker_meta_change', 99, 4);

if (!function_exists('bvmgr_square_firewall_filter_should_sync')) {
    /**
     ...
     * @return mixed
     */
    function bvmgr_square_firewall_filter_should_sync($should_sync, ...$args)
    {
        $product_id = bvmgr_square_firewall_extract_product_id($should_sync, ...$args);
        if ($product_id > 0 && bvmgr_square_firewall_is_protected_product($product_id)) {
...
foreach (array(
    ...
    add_filter($bvmgr_square_firewall_filter_name, 'bvmgr_square_firewall_filter_should_sync', 99, 10);
}
unset($bvmgr_square_firewall_filter_name);

if (!function_exists('bvmgr_square_firewall_query_product_ids')) {
    /**
     * @return int[]
     */
    function bvmgr_square_firewall_query_product_ids(int $after_id = 0, int $limit = 250): array
    {
        global $wpdb;
...
if (!function_exists('bvmgr_square_firewall_scan_products')) {
     ...
     * @return array<string,mixed>
     */
    function bvmgr_square_firewall_scan_products(bool $repair = false, int $limit = 5000): array
    {
        ...
        while ($checked < $limit) {
            $batch = bvmgr_square_firewall_query_product_ids($after_id, min(250, $limit - $checked));
            if (empty($batch)) {
                break;
            }
            ...
            foreach ($batch as $product_id) {
                ...
                $has_link = bvmgr_square_firewall_has_square_link($product_id);
                if ($has_link) {
                ...
                $repair_result = null;
                if ($repair) {
                    $repair_result = bvmgr_square_firewall_protect_product($product_id, true);
...
if (!function_exists('bvmgr_square_firewall_auto_backfill_once')) {
    function bvmgr_square_firewall_auto_backfill_once(): void
    {
        if (!is_admin()) {
            return;
        }
        ...
        try {
            ...
            $ids = bvmgr_square_firewall_query_product_ids($cursor, 100);
            if (empty($ids)) {
...
add_action('admin_init', 'bvmgr_square_firewall_auto_backfill_once', 57);
