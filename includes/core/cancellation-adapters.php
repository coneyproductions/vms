...
require_once __DIR__ . '/cancellation-purchases.php';
...
if (!function_exists('bvmgr_cancellation_refund_match_order_item')) {
	...
	{
		$identity = bvmgr_cancel_purchase_identity($item, array('event_plan_id' => $event_plan_id, 'tec_event_id' => $tec_event_id, 'product_lookup' => $product_lookup), bvmgr_cancel_purchase_providers());
		return array('matched' => $identity['state'] === 'eligible', 'source' => $identity['provider'], 'product_id' => (int) $item->get_product_id(), 'product_role' => bvmgr_cancellation_refund_product_role((int) $item->get_product_id()), 'eligibility' => $identity['state']);
	}
}
...
if (!empty($data['failed_products']) || !empty($data['failed_rsvp_tickets'])) {
	...
	return array('status' => 'done', 'message' => 'purchase_preflight_ready', 'data' => $scope);
}, 10, 5);
...
add_filter('vms_cancellation_run_step', function ($result, $event_plan_id, $policy, $step_key, $summary) {
	...
	if (empty($summary['purchase_preflight']['accepted_by'])) {
		return array('status' => 'blocked', 'message' => 'purchase_preflight_acceptance_required', 'data' => array('requires_operator_review' => true));
	}
	$candidates = (array) ($summary['purchase_preflight']['scope']['candidates'] ?? array());
	...
	return bvmgr_cancel_execute_scope($event_plan_id, $summary);
}, 10, 5);
