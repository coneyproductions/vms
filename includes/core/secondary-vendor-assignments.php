...
if (!function_exists('vms_event_plan_secondary_vendor_assignment_meta_key')) {
	function vms_event_plan_secondary_vendor_assignment_meta_key(): string
	{
		if (function_exists('vms_meta_key')) {
			$key = (string) vms_meta_key('event_plan', 'secondary_vendor_assignments_v1');
			if ($key !== '') {
				return $key;
			}
		}
...
if (!function_exists('vms_event_plan_secondary_vendor_type_options')) {
	function vms_event_plan_secondary_vendor_type_options(): array
	{
		$options = array();

		if (function_exists('vms_vendor_type_select_options')) {
			foreach ((array) vms_vendor_type_select_options() as $slug => $label) {
				$slug = vms_event_plan_normalize_secondary_vendor_type_slug((string) $slug);
...
if (!function_exists('vms_event_plan_additional_vendor_type_options')) {
	...
	{
		$options = vms_event_plan_secondary_vendor_type_options();
		unset($options['band']);
		return $options;
	}
}

if (!function_exists('vms_event_plan_secondary_vendor_mode_options')) {
...
if (!function_exists('vms_event_plan_secondary_vendor_default_slot_limit')) {
	function vms_event_plan_secondary_vendor_default_slot_limit(string $type_slug, string $mode = 'standard')
	{
		$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug($type_slug);
...
if (!function_exists('vms_event_plan_secondary_vendor_effective_slot_limit')) {
	function vms_event_plan_secondary_vendor_effective_slot_limit(string $type_slug, string $mode, $slot_limit): ?int
	{
		$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug($type_slug);
		$mode = sanitize_key($mode);
		if (!in_array($mode, array('standard', 'market'), true)) {
			$mode = vms_event_plan_secondary_vendor_default_mode($type_slug);
		}
		...
		$default = vms_event_plan_secondary_vendor_default_slot_limit($type_slug, $mode);
...
if (!function_exists('vms_event_plan_secondary_vendor_compare_value')) {
	function vms_event_plan_secondary_vendor_compare_value($value)
	{
		if (!is_array($value)) {
			return $value;
		}
		...
		foreach ($value as $key => $child) {
			$normalized[$key] = vms_event_plan_secondary_vendor_compare_value($child);
		}
...
if (!function_exists('vms_event_plan_secondary_vendor_terms_for_vendor')) {
	function vms_event_plan_secondary_vendor_terms_for_vendor(int $vendor_id): array
	{
		...
		foreach ($terms as $term) {
			...
			$slugs[] = vms_event_plan_normalize_secondary_vendor_type_slug((string) $term->slug);
		}
...
if (!function_exists('vms_event_plan_normalize_secondary_vendor_assignment_map')) {
	function vms_event_plan_normalize_secondary_vendor_assignment_map(int $post_id, array $assignments, int $primary_vendor_id = 0, array $args = array()): array
	{
		$post_id = absint($post_id);
		$primary_vendor_id = absint($primary_vendor_id);
		...
		foreach ($assignments as $raw_key => $raw_row) {
			...
			$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug($candidate_type);
			if ($type_slug === '') {
				continue;
			}
			...
			if (array_key_exists('slot_limit', $row)) {
				...
				if ($row['slot_limit'] === '' || $row['slot_limit'] === null) {
					$slot_limit = vms_event_plan_secondary_vendor_default_slot_limit($type_slug, $mode);
				} else {
			...
			foreach (array('allow_over_capacity', 'over_capacity_override', 'capacity_override') as $override_key) {
				if (array_key_exists($override_key, $row) && vms_event_plan_parse_secondary_vendor_over_capacity_override($row[$override_key])) {
			...
			if (array_key_exists('open_for_dispatch', $row)) {
				...
				$open_for_dispatch = vms_event_plan_parse_secondary_vendor_over_capacity_override($row['open_for_dispatch']);
			}
			...
			$vendor_ids = vms_event_plan_normalize_secondary_vendor_ids($post_id, $type_slug, $raw_vendor_ids, $primary_vendor_id);
			...
			if (isset($normalized[$type_slug])) {
				...
				$existing['mode'] = $mode !== '' ? $mode : (string) ($existing['mode'] ?? vms_event_plan_secondary_vendor_default_mode($type_slug));
...
if (!function_exists('vms_event_plan_secondary_vendor_over_capacity_rows')) {
	function vms_event_plan_secondary_vendor_over_capacity_rows(array $assignments): array
	{
		$rows = array();
		$type_options = function_exists('vms_event_plan_secondary_vendor_type_options')
			? (array) vms_event_plan_secondary_vendor_type_options()
			: array();

		foreach ($assignments as $type_slug => $assignment) {
			$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug((string) $type_slug);
			...
			$slot_limit = vms_event_plan_secondary_vendor_effective_slot_limit($type_slug, $mode, $assignment['slot_limit'] ?? null);
			if ($slot_limit === null) {
				continue;
			}
			...
			$allow_over_capacity = vms_event_plan_parse_secondary_vendor_over_capacity_override($assignment['allow_over_capacity'] ?? false);
...
if (!function_exists('vms_event_plan_validate_secondary_vendor_assignment_capacity')) {
	function vms_event_plan_validate_secondary_vendor_assignment_capacity(array $assignments)
	{
		$over_capacity_rows = vms_event_plan_secondary_vendor_over_capacity_rows($assignments);
...
if (!function_exists('vms_event_plan_get_secondary_vendor_flat_ids_from_assignments')) {
	function vms_event_plan_get_secondary_vendor_flat_ids_from_assignments(array $assignments, int $primary_vendor_id = 0): array
	{
		$primary_vendor_id = absint($primary_vendor_id);
		$secondary_ids = array();
...
if (!function_exists('vms_event_plan_get_secondary_vendor_ids_for_type')) {
	function vms_event_plan_get_secondary_vendor_ids_for_type($source, string $type_slug): array
	{
		$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug($type_slug);
		if ($type_slug === '') {
			return array();
		...
		$assignments = is_numeric($source)
			? vms_event_plan_get_secondary_vendor_assignments((int) $source)
...
if (!function_exists('vms_event_plan_legacy_secondary_vendor_type_from_assignments')) {
	function vms_event_plan_legacy_secondary_vendor_type_from_assignments(array $assignments): string
	{
		$first = '';
		foreach ($assignments as $type_slug => $assignment) {
			unset($assignment);
...
if (!function_exists('vms_event_plan_guess_secondary_vendor_assignments_from_flat_ids')) {
	function vms_event_plan_guess_secondary_vendor_assignments_from_flat_ids(int $post_id, array $secondary_ids, int $primary_vendor_id = 0): array
	{
		$post_id = absint($post_id);
		$primary_vendor_id = absint($primary_vendor_id);
		$guessed = array();
		...
		foreach (array_values(array_unique(array_filter(array_map('absint', $secondary_ids)))) as $vendor_id) {
			...
			$type_slugs = vms_event_plan_secondary_vendor_terms_for_vendor($vendor_id);
			$type_slug = '';
		...
		return vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, $guessed, $primary_vendor_id, array(
			'preserve_empty' => true,
		));
	}
}

if (!function_exists('vms_event_plan_read_legacy_secondary_vendor_assignments')) {
	function vms_event_plan_read_legacy_secondary_vendor_assignments(int $post_id, int $primary_vendor_id = 0): array
	{
		$post_id = absint($post_id);
		$primary_vendor_id = absint($primary_vendor_id);
		...
		if ($secondary_type !== '') {
			return vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, array(
		...
		return vms_event_plan_guess_secondary_vendor_assignments_from_flat_ids($post_id, $secondary_ids, $primary_vendor_id);
	}
}
...
if (!function_exists('vms_event_plan_get_secondary_vendor_assignments')) {
	function vms_event_plan_get_secondary_vendor_assignments(int $post_id, array $args = array()): array
	{
		...
		$raw = get_post_meta($post_id, vms_event_plan_secondary_vendor_assignment_meta_key(), true);
		$has_canonical_meta = function_exists('metadata_exists')
			? metadata_exists('post', $post_id, vms_event_plan_secondary_vendor_assignment_meta_key())
			: ($raw !== '' && $raw !== null);

		if (is_array($raw)) {
			$normalized = vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, $raw, $primary_vendor_id, array(
				'preserve_empty' => true,
			));
		...
		return vms_event_plan_read_legacy_secondary_vendor_assignments($post_id, $primary_vendor_id);
	}
}

if (!function_exists('vms_event_plan_write_secondary_vendor_assignments')) {
	function vms_event_plan_write_secondary_vendor_assignments(int $post_id, array $assignments)
	{
		$post_id = absint($post_id);
		...
		$k_secondary_idx = function_exists('vms_meta_key') ? (vms_meta_key('event_plan', 'secondary_vendor_id') ?: '_vms_secondary_vendor_id') : '_vms_secondary_vendor_id';
		$k_secondary_type = function_exists('vms_meta_key') ? (vms_meta_key('event_plan', 'secondary_vendor_type') ?: '_vms_secondary_vendor_type') : '_vms_secondary_vendor_type';
		$k_secondary_unq = function_exists('vms_meta_key') ? (vms_meta_key('event_plan', 'secondary_vendor_unqualified') ?: '_vms_secondary_vendor_unqualified') : '_vms_secondary_vendor_unqualified';
		$k_secondary_unq_ids = function_exists('vms_meta_key') ? (vms_meta_key('event_plan', 'secondary_vendor_unqualified_ids') ?: '_vms_secondary_vendor_unqualified_ids') : '_vms_secondary_vendor_unqualified_ids';
		$primary_vendor_id = (int) get_post_meta($post_id, '_vms_band_vendor_id', true);

		$normalized = vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, $assignments, $primary_vendor_id, array(
			'preserve_empty' => true,
		));
		$capacity_validation = vms_event_plan_validate_secondary_vendor_assignment_capacity($normalized);
		if (is_wp_error($capacity_validation)) {
			return $capacity_validation;
		}

		$flat_ids = vms_event_plan_get_secondary_vendor_flat_ids_from_assignments($normalized, $primary_vendor_id);
		$legacy_type = vms_event_plan_legacy_secondary_vendor_type_from_assignments($normalized);

		if (!empty($normalized)) {
			update_post_meta($post_id, vms_event_plan_secondary_vendor_assignment_meta_key(), $normalized);
		} else {
			delete_post_meta($post_id, vms_event_plan_secondary_vendor_assignment_meta_key());
		}
		...
		if (function_exists('vms_secondary_vendor_is_qualified')) {
			foreach ($normalized as $type_slug => $assignment) {
				foreach ((array) ($assignment['vendor_ids'] ?? array()) as $vendor_id) {
					...
					$ok = vms_secondary_vendor_is_qualified($vendor_id, array(
...
if (!function_exists('vms_event_plan_resolve_secondary_vendor_submission')) {
	function vms_event_plan_resolve_secondary_vendor_submission(int $post_id, array $request): array
	{
		...
		if ($clear_requested) {
			...
			$secondary_vendor_assignments = vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, (array) $raw_assignments, $primary_vendor_id, array(
				'preserve_empty' => true,
			));
			...
			$secondary_vendor_assignments = $type_slug !== '' || !empty($secondary_ids)
				? vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, array(
		...
		$type_slug = vms_event_plan_legacy_secondary_vendor_type_from_assignments($secondary_vendor_assignments);
		$secondary_ids = vms_event_plan_get_secondary_vendor_flat_ids_from_assignments($secondary_vendor_assignments, $primary_vendor_id);
...
if (!function_exists('vms_event_plan_get_secondary_vendor_state')) {
	function vms_event_plan_get_secondary_vendor_state(int $post_id): array
	{
		...
		$secondary_vendor_assignments = vms_event_plan_get_secondary_vendor_assignments($post_id, array(
		...
		return array(
			'primary_vendor_id' => $primary_vendor_id,
			'secondary_vendor_assignments' => $secondary_vendor_assignments,
			'secondary_vendor_type' => vms_event_plan_legacy_secondary_vendor_type_from_assignments($secondary_vendor_assignments),
	...
	function vms_event_plan_secondary_vendor_state_diff_fields(array $before, array $after, array $fields = array()): array
	{
		...
		foreach ($fields as $field) {
			...
			if (is_array($before_value) || is_array($after_value)) {
				$before_encoded = wp_json_encode(vms_event_plan_secondary_vendor_compare_value((array) $before_value));
				$after_encoded = wp_json_encode(vms_event_plan_secondary_vendor_compare_value((array) $after_value));
...
if (!function_exists('vms_event_plan_secondary_vendor_rebuild_repair_reasons')) {
	function vms_event_plan_secondary_vendor_rebuild_repair_reasons(int $post_id, array $current_state = array()): array
	{
		...
		$expected_type = vms_event_plan_legacy_secondary_vendor_type_from_assignments($expected_assignments);

		$stored_canonical = get_post_meta($post_id, vms_event_plan_secondary_vendor_assignment_meta_key(), true);
		$stored_canonical = is_array($stored_canonical)
			? vms_event_plan_normalize_secondary_vendor_assignment_map($post_id, $stored_canonical, (int) ($current_state['primary_vendor_id'] ?? 0), array(
				'preserve_empty' => true,
			))
			: array();
		...
		$stored_type = vms_event_plan_normalize_secondary_vendor_type_slug((string) get_post_meta($post_id, $k_secondary_type, true));

		$repair_reasons = array();
		if ((empty($stored_canonical) && !empty($expected_assignments)) || ($has_canonical && wp_json_encode(vms_event_plan_secondary_vendor_compare_value($stored_canonical)) !== wp_json_encode(vms_event_plan_secondary_vendor_compare_value($expected_assignments)))) {
...
if (!function_exists('vms_event_plan_set_secondary_vendors')) {
	function vms_event_plan_set_secondary_vendors(int $post_id, string $type_slug, array $secondary_ids)
	{
		...
		$type_slug = vms_event_plan_normalize_secondary_vendor_type_slug($type_slug);
		$current_assignments = vms_event_plan_get_secondary_vendor_assignments($post_id);
		...
		if ($type_slug === '') {
			...
			$current_assignments[$type_slug] = array(
				'mode' => (string) ($current_assignment['mode'] ?? vms_event_plan_secondary_vendor_default_mode($type_slug)),
				'slot_limit' => array_key_exists($type_slug, $current_assignments)
					? ($current_assignment['slot_limit'] ?? vms_event_plan_secondary_vendor_default_slot_limit($type_slug, (string) ($current_assignment['mode'] ?? 'standard')))
					: vms_event_plan_secondary_vendor_default_slot_limit($type_slug, vms_event_plan_secondary_vendor_default_mode($type_slug)),
				'vendor_ids' => array_values(array_unique(array_filter(array_map('absint', $secondary_ids)))),
			);
			if (vms_event_plan_parse_secondary_vendor_over_capacity_override($current_assignment['allow_over_capacity'] ?? false)) {
			...
			if (array_key_exists('open_for_dispatch', $current_assignment)) {
				$current_assignments[$type_slug]['open_for_dispatch'] = vms_event_plan_parse_secondary_vendor_over_capacity_override($current_assignment['open_for_dispatch']);
			}
		}

		$write_result = vms_event_plan_write_secondary_vendor_assignments($post_id, $current_assignments);
...
if (!function_exists('vms_event_plan_save_secondary_vendors_module')) {
	function vms_event_plan_save_secondary_vendors_module(int $post_id, array $request)
	{
		...
		$secondary_vendor_submission = vms_event_plan_resolve_secondary_vendor_submission($post_id, $request);
		...
		$capacity_validation = vms_event_plan_validate_secondary_vendor_assignment_capacity($proposed_assignments);
		if (is_wp_error($capacity_validation)) {
			return $capacity_validation;
		}
		...
		$proposed_vendor_state = array(
			...
			'secondary_vendor_type' => vms_event_plan_legacy_secondary_vendor_type_from_assignments($proposed_assignments),
			'secondary_vendor_ids' => vms_event_plan_get_secondary_vendor_flat_ids_from_assignments($proposed_assignments, (int) ($current_vendor_state['primary_vendor_id'] ?? 0)),
			'linked_tec_event_id' => $linked_tec_event_id,
		);

		$vendor_dirty_fields = vms_event_plan_secondary_vendor_state_diff_fields(
			$current_vendor_state,
			$proposed_vendor_state,
			array('secondary_vendor_assignments', 'secondary_vendor_type', 'secondary_vendor_ids')
		);
		$repair_reasons = vms_event_plan_secondary_vendor_rebuild_repair_reasons($post_id, $current_vendor_state);
		$changed = !empty($vendor_dirty_fields) || !empty($repair_reasons);
		$queued_calendar_maintenance = false;

		if ($changed) {
			$write_result = vms_event_plan_write_secondary_vendor_assignments($post_id, $proposed_assignments);
			if (is_wp_error($write_result)) {
				return $write_result;
			}
...
if (!function_exists('vms_event_plan_secondary_vendor_group_available_slots')) {
	function vms_event_plan_secondary_vendor_group_available_slots(array $assignment): ?int
	{
