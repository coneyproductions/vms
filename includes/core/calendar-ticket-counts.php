...
if (!function_exists('bvmgr_calendar_ticket_counts_meta_key')) {
	function bvmgr_calendar_ticket_counts_meta_key(): string
	{
...
if (!function_exists('bvmgr_calendar_ticket_counts_tec_key')) {
	function bvmgr_calendar_ticket_counts_tec_key(): string
	{
...
if (!function_exists('bvmgr_calendar_ticket_counts_date_key')) {
	function bvmgr_calendar_ticket_counts_date_key(): string
	{
...
if (!function_exists('bvmgr_calendar_ticket_counts_get_tec_event_id_for_plan')) {
	function bvmgr_calendar_ticket_counts_get_tec_event_id_for_plan(int $plan_id): int
	{
		...
		return absint(get_post_meta($plan_id, bvmgr_calendar_ticket_counts_tec_key(), true));
	}
}

if (!function_exists('bvmgr_calendar_ticket_counts_get_product_ids_for_tec_event')) {
	/**
	 * @return int[]
	 */
	function bvmgr_calendar_ticket_counts_get_product_ids_for_tec_event(int $tec_event_id): array
	{
...
if (!function_exists('bvmgr_calendar_ticket_counts_compute_qty')) {
	function bvmgr_calendar_ticket_counts_compute_qty(array $product_ids): int
	{
...
if (!function_exists('bvmgr_calendar_ticket_counts_set_plan_qty')) {
	function bvmgr_calendar_ticket_counts_set_plan_qty(int $plan_id, int $qty): void
	{
		$plan_id = absint($plan_id);
		if ($plan_id <= 0) {
			return;
		...
		$qty = max(0, (int) $qty);
		$key = bvmgr_calendar_ticket_counts_meta_key();
...
if (!function_exists('bvmgr_calendar_ticket_counts_refresh_plan')) {
	function bvmgr_calendar_ticket_counts_refresh_plan(int $plan_id): int
	{
		$plan_id = absint($plan_id);
		...
		$tec_event_id = bvmgr_calendar_ticket_counts_get_tec_event_id_for_plan($plan_id);
		if ($tec_event_id <= 0) {
			bvmgr_calendar_ticket_counts_set_plan_qty($plan_id, 0);
			return 0;
		}

		$product_ids = bvmgr_calendar_ticket_counts_get_product_ids_for_tec_event($tec_event_id);
		$qty = bvmgr_calendar_ticket_counts_compute_qty($product_ids);
		bvmgr_calendar_ticket_counts_set_plan_qty($plan_id, $qty);
		return $qty;
	}
}

if (!function_exists('bvmgr_calendar_ticket_counts_refresh_plans')) {
	/**
	 * @param int[] $plan_ids
	 */
	function bvmgr_calendar_ticket_counts_refresh_plans(array $plan_ids): void
	{
		$plan_ids = array_values(array_unique(array_filter(array_map('absint', $plan_ids))));
		foreach ($plan_ids as $plan_id) {
			bvmgr_calendar_ticket_counts_refresh_plan($plan_id);
		}
	}
}

if (!function_exists('bvmgr_calendar_ticket_counts_find_plan_ids_by_tec_event')) {
	/**
	 * @return int[]
	 */
	function bvmgr_calendar_ticket_counts_find_plan_ids_by_tec_event(int $tec_event_id): array
	{
		$tec_event_id = absint($tec_event_id);
		if ($tec_event_id <= 0) {
			return array();
		...
		$plan_ids = get_posts(array(
			...
			'meta_query' => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Ticket-count refresh intentionally retrieves every Event Plan ID linked to the exact TEC event so each affected plan count is recalculated.
				array(
					'key' => bvmgr_calendar_ticket_counts_tec_key(),
					'value' => $tec_event_id,
					'compare' => '=',
					'type' => 'NUMERIC',
				),
...
if (!function_exists('bvmgr_calendar_ticket_counts_collect_plan_ids_from_order')) {
	/**
	 * @return int[]
	 */
	function bvmgr_calendar_ticket_counts_collect_plan_ids_from_order(int $order_id): array
	{
		...
		$product_to_tec_keys = array(
			...
			bvmgr_calendar_ticket_counts_tec_key(),
		);
		...
		foreach ((array) $order->get_items('line_item') as $item) {
			...
			foreach (array_values(array_unique(array_filter(array_map('absint', $tec_event_ids)))) as $tec_event_id) {
				$plan_ids = array_merge($plan_ids, bvmgr_calendar_ticket_counts_find_plan_ids_by_tec_event($tec_event_id));
			}
		}
...
if (!function_exists('bvmgr_calendar_ticket_counts_order_status_changed')) {
	function bvmgr_calendar_ticket_counts_order_status_changed($order_id, $old_status = '', $new_status = '', $order = null): void
	{
		$plan_ids = bvmgr_calendar_ticket_counts_collect_plan_ids_from_order(absint($order_id));
		if (empty($plan_ids)) {
			return;
		}
		bvmgr_calendar_ticket_counts_refresh_plans($plan_ids);
	}
}
add_action('woocommerce_order_status_changed', 'bvmgr_calendar_ticket_counts_order_status_changed', 20, 4);

if (!function_exists('bvmgr_calendar_ticket_counts_nightly_scan')) {
	function bvmgr_calendar_ticket_counts_nightly_scan(): void
	{
		...
		$plan_ids = get_posts(array(
			...
			'meta_query' => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_query -- Nightly ticket-count maintenance intentionally retrieves the complete Event Plan ID set in its configured future date window before recalculation.
				'relation' => 'AND',
				array(
					'key' => bvmgr_calendar_ticket_counts_date_key(),
					'value' => $today,
					'compare' => '>=',
					'type' => 'DATE',
				),
		...
		bvmgr_calendar_ticket_counts_refresh_plans((array) $plan_ids);
	}
}
add_action(BVMGR_CALENDAR_TICKET_COUNTS_CRON_HOOK, 'bvmgr_calendar_ticket_counts_nightly_scan');

if (!function_exists('bvmgr_calendar_ticket_counts_schedule_cron')) {
	function bvmgr_calendar_ticket_counts_schedule_cron(): void
	{
...
add_action('init', 'bvmgr_calendar_ticket_counts_schedule_cron', 40);
