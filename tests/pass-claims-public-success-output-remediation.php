...
$assert($GLOBALS['vms_test_nonce_calls'] === 3 && $GLOBALS['vms_test_nonce_args'] === array(array('bad-nonce', 'bvmgr_pass_claim_submit'), array('bad-nonce', 'vms_pass_claim_submit'), array('bad-nonce', 'bvmgr_pass_claim_submit')), 'Invalid nonce Pass Claims submission should try the exact canonical action and legacy fallback before the native verifier rejects the canonical selection.');
...
$assert($GLOBALS['vms_test_create_claim_calls'] === 1 && $GLOBALS['vms_test_nonce_calls'] === 2 && $GLOBALS['vms_test_find_token_calls'] === 1 && $GLOBALS['vms_test_get_batch_calls'] === 1 && $GLOBALS['vms_test_eligible_calls'] === 1 && $GLOBALS['vms_test_rate_limit_calls'] === 0 && $GLOBALS['vms_test_empty_notice_calls'] === 0, 'Failed Pass Claims submission should pass canonical selection through the native verifier while preserving the single claim-mutation attempt and pre-form reads.');
