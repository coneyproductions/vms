...
try {
	...
	$successResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_success', array($successPayload, 201), 'success-noise');
	...
	$errorResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_error', array($errorPayload, 422), 'error-noise');
	...
	$successDefaultResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_success', array(null));
	...
	$errorDefaultResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_error', array($errorDefaultPayload));
	...
	$successFlagsResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_success', array($successFlagsPayload, 202, $successFlags));
	...
	$errorFlagsResult = vms_test_run_wrapper('vms_ticketing_v2_ajax_send_error', array($errorFlagsPayload, 409, $errorFlags));
	...
	$atomicBody = vms_test_extract_function($v2Source, 'vms_ticketing_v2_ajax_atomic_add_to_cart');
	$silentBody = vms_test_extract_function($v2Source, 'vms_ticketing_v2_ajax_silent_add');
	$cartContextBody = vms_test_extract_function($v2Source, 'vms_ticketing_v2_ajax_cart_context');
	...
	vms_test_assert_not_contains('vms_ticketing_ajax_send_success(', $atomicBody, 'Atomic add should not route through the legacy success wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_send_error(', $atomicBody, 'Atomic add should not route through the legacy error wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_attach_noise(', $atomicBody, 'Atomic add should not route through the legacy noise helper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_send_success(', $silentBody, 'Silent add should not route through the legacy success wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_send_error(', $silentBody, 'Silent add should not route through the legacy error wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_attach_noise(', $silentBody, 'Silent add should not route through the legacy noise helper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_send_success(', $cartContextBody, 'Cart context should not route through the legacy success wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_send_error(', $cartContextBody, 'Cart context should not route through the legacy error wrapper.');
	vms_test_assert_not_contains('vms_ticketing_ajax_attach_noise(', $cartContextBody, 'Cart context should not route through the legacy noise helper.');

	vms_test_assert_same(2, substr_count(vms_test_normalize_code($atomicBody), vms_test_normalize_code("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'invalid_payload'), 400);")), 'Atomic add should keep both invalid_payload 400 error branches.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'bad_nonce'), 403);", $atomicBody, 'Atomic add should keep the bad_nonce 403 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'empty_selection'), 400);", $atomicBody, 'Atomic add should keep the empty_selection 400 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'cart_unavailable'), 400);", $atomicBody, 'Atomic add should keep the cart_unavailable 400 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array( 'ok' => false, 'message' => sanitize_text_field((string) (\$event_validation['code'] ?? 'event_unavailable')), 'notice_message' => sanitize_text_field((string) (\$event_validation['message'] ?? '')), ), (int) (\$event_validation['http'] ?? 400));", $atomicBody, 'Atomic add should keep dynamic event-validation status forwarding.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array( 'ok' => false, 'message' => \$message, 'errors' => \$errors, 'notice_messages' => \$notice_messages, ), 400);", $atomicBody, 'Atomic add should keep the rollback error response contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_success(array( 'ok' => true, 'cart_url' => function_exists('wc_get_cart_url') ? wc_get_cart_url() : home_url('/cart/'), 'added_tickets' => \$added_tickets, 'added_addons' => \$added_addons, 'added_total' => (\$added_tickets + \$added_addons), ));", $atomicBody, 'Atomic add should keep the final success response contract.');

	vms_test_assert_same(2, substr_count(vms_test_normalize_code($silentBody), vms_test_normalize_code("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'invalid_payload'), 400);")), 'Silent add should keep both invalid_payload 400 error branches.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'bad_nonce'), 403);", $silentBody, 'Silent add should keep the bad_nonce 403 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_success(array('ok' => true, 'added' => 0));", $silentBody, 'Silent add should keep the empty-items success contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'cart_unavailable'), 400);", $silentBody, 'Silent add should keep the cart_unavailable 400 contract.');
	...
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'ticketing_disabled'), 403);", $silentBody, 'Silent add should keep the ticketing_disabled 403 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'add_failed', 'errors' => \$errors), 400);", $silentBody, 'Silent add should keep the partial-failure error response contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_success(array('ok' => true, 'added' => \$added));", $silentBody, 'Silent add should keep the final success response contract.');

	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'bad_nonce'), 403);", $cartContextBody, 'Cart context should keep the bad_nonce 403 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_error(array('ok' => false, 'message' => 'cart_unavailable'), 400);", $cartContextBody, 'Cart context should keep the cart_unavailable 400 contract.');
	vms_test_assert_code_contains("vms_ticketing_v2_ajax_send_success(array( 'ok' => true, 'event_plan_id' => \$plan_id, 'tec_event_id' => \$tec_event_id, 'ga_qty_raw' => \$ga_qty_raw, 'ga_qty' => \$ga_qty, 'prior_qualifying_qty' => \$prior_qualifying_qty, 'prior_pool_qty_by_key' => \$prior_pool_qty_by_key, 'pool_qty_by_key' => \$pool_qty_by_key, 'has_checkout_blockers' => !empty(\$checkout_blocker_messages), 'checkout_blocker_messages' => \$checkout_blocker_messages, ));", $cartContextBody, 'Cart context should keep the final success response contract.');
