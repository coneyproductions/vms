...
try {
	...
	update_post_meta($foodNoResponseEmailVendor, bvmgr_add_dispatch_vendor_email_key(), 'add-no-response@example.test');
	...
	$scan = bvmgr_add_dispatch_get_event_plan_need_scan(20, 20);
	$contextsById = array();
	...
	$primaryRows = bvmgr_add_dispatch_context_vendor_need_rows((array) $contextsById[$missingPrimaryPlan], false);
	...
	$pastReason = bvmgr_add_dispatch_context_exclusion_reason(bvmgr_add_dispatch_get_event_plan_context($pastMissingPrimaryPlan));
	$cancelledReason = bvmgr_add_dispatch_context_exclusion_reason(bvmgr_add_dispatch_get_event_plan_context($cancelledPlan));
	...
	$foodContext = bvmgr_add_dispatch_get_event_plan_context($foodOpenPlan);
	...
	$assert(bvmgr_add_dispatch_type_label('food-truck') === 'Food Vendor', 'ADD labels should normalize food-truck to Food Vendor.');
	...
	$builderOff = bvmgr_add_dispatch_parse_builder_args(array(
		'target_mode' => 'secondary',
		'vendor_type' => 'food_truck',
	), (array) $foodContext);
	$collectedNoResponseEmailVendor = $candidateById(bvmgr_vendor_availability_collect_vendors(), $foodNoResponseEmailVendor);
	$candidatesOff = bvmgr_add_dispatch_collect_recipient_candidates((array) $foodContext, $builderOff);
	...
	$builderOn = bvmgr_add_dispatch_parse_builder_args(array(
		'target_mode' => 'secondary',
		'vendor_type' => 'food_truck',
		'include_no_response' => 1,
	), (array) $foodContext);
	$candidatesOn = bvmgr_add_dispatch_collect_recipient_candidates((array) $foodContext, $builderOn);
	...
	$safeNoResponseCopy = bvmgr_add_dispatch_no_response_explanation();
	...
	$eligibleOn = bvmgr_add_dispatch_collect_eligible_recipients((array) $foodContext, $builderOn);
	...
	$createdSelected = bvmgr_add_dispatch_create_request($foodOpenPlan, $builderOn, array($noResponseEmailOn));
	...
	$emailBody = bvmgr_add_dispatch_email_body_text(array(
		'include_no_response' => 1,
	...
	$dessertContext = bvmgr_add_dispatch_get_event_plan_context($dessertFullPlan);
	...
	$assert(empty(bvmgr_add_dispatch_dashboard_need_rows((array) $dessertContext, array())), 'Full vendor groups should be hidden from ADD dashboard by default.');
	$assert(!empty(bvmgr_add_dispatch_dashboard_need_rows((array) $dessertContext, array('show_full_groups' => true))), 'Full vendor groups should be available only when the diagnostic filter is enabled.');

	$marketCapacityContext = bvmgr_add_dispatch_get_event_plan_context($marketCapacityOnlyPlan);
	...
	$marketTargetContext = bvmgr_add_dispatch_get_event_plan_context($marketTargetPlan);
	...
	$marketTargetFullContext = bvmgr_add_dispatch_get_event_plan_context($marketTargetFullPlan);
	...
	$marketOverTargetContext = bvmgr_add_dispatch_get_event_plan_context($marketOverTargetPlan);
	...
	$overContext = bvmgr_add_dispatch_get_event_plan_context($overCapacityPlan);
	...
	$recentResponses = bvmgr_add_dispatch_get_recent_responses(1);
	$recentRequests = bvmgr_add_dispatch_get_recent_requests(1);
	...
	ob_start();
	bvmgr_add_dispatch_render_request_builder((array) $foodContext, $builderOn, $eligibleOn);
	...
	ob_start();
	bvmgr_add_dispatch_render_request_history(array(), 0);
