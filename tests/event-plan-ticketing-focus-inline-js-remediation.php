...
try {
	...
	$assert(strpos($ticketingBootstrapSource, '$handle = \'bvmgr-admin-ticketing\';') !== false, 'Ticketing bootstrap should retain the existing admin-ticketing handle.');
	...
	$assert(strpos($shellAssetSource, 'window.BVMGR_EVENT_PLAN_PERSIST_REQUESTED_SECTION = persistRequestedSection;') !== false, 'The separate shell asset should own the generic requested-section helper.');
