...
try {
	...
	$assert(substr_count($eventPlansSource, '<script') >= 4, 'Event Plan source should still contain the current live inline script surface for later B1 slices.');
	...
	$assert(strpos($eventPlansSource, 'window.vmsEventPlanInitSecondaryVendors = initSecondaryVendors;') === false, 'Event Plan source should no longer retain the migrated secondary-vendor initializer.');
	$assert(strpos($secondaryVendorAssetSource, 'window.vmsEventPlanInitSecondaryVendors = initSecondaryVendors;') !== false, 'Dedicated Secondary Vendors asset should now own the live secondary-vendor initializer.');
