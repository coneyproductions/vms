...
try {
	...
	$assert(strpos($eventPlansSource, 'window.BVMGR_EVENT_PLAN_PERSIST_REQUESTED_SECTION = persistRequestedSection;') === false, 'Event Plan source should no longer retain the migrated shell requested-section persistence helper.');
	$assert(strpos($eventPlansSource, 'window.BVMGR_EVENT_PLAN_REVEAL_REQUESTED_SECTION = revealRequestedSection;') === false, 'Event Plan source should no longer retain the migrated shell requested-section reveal helper.');
	$assert(strpos($shellAssetSource, 'window.BVMGR_EVENT_PLAN_PERSIST_REQUESTED_SECTION = persistRequestedSection;') !== false, 'Shell asset should now own the canonical requested-section persistence helper.');
	$assert(strpos($shellAssetSource, 'window.BVMGR_EVENT_PLAN_REVEAL_REQUESTED_SECTION = revealRequestedSection;') !== false, 'Shell asset should now own the canonical requested-section reveal helper.');
	$assert(strpos($eventPlansSource, 'window.BVMGR_EVENT_PLAN_INIT_STAFF = initStaff;') === false, 'Event Plan source should no longer retain the migrated staff initializer.');
	$assert(strpos($staffAssetSource, 'window.BVMGR_EVENT_PLAN_INIT_STAFF = initStaff;') !== false, 'Dedicated staff asset should now own the canonical public staff initializer.');
	$assert(strpos($eventPlansSource, 'window.BVMGR_EVENT_PLAN_INIT_SECONDARY_VENDORS = initSecondaryVendors;') === false, 'Event Plan source should no longer retain the migrated secondary-vendor initializer.');
	$assert(strpos($secondaryVendorAssetSource, 'window.BVMGR_EVENT_PLAN_INIT_SECONDARY_VENDORS = initSecondaryVendors;') !== false, 'Dedicated Secondary Vendors asset should now own the canonical live secondary-vendor initializer.');
	...
	$assert(strpos($adminUiAssetsSource, "'bvmgr-lineup-schedule-admin'") !== false, 'Admin UI enqueue path should still register the canonical lineup Event Plan asset.');
	...
	$assert(strpos($adminUiAssetsSource, "'bvmgr-event-plan-secondary-vendors'") !== false, 'Admin UI enqueue path should register the canonical Secondary Vendors asset.');
	...
	$assert(strpos($adminUiAssetsSource, "'bvmgr-event-plan-workflow'") !== false, 'Admin UI enqueue path should register the canonical workflow confirmations asset.');
	...
	$assert(strpos($ticketingBootstrapSource, "'bvmgr-admin-ticketing'") !== false, 'Ticketing bootstrap should still enqueue the canonical Event Plan ticketing asset.');
	...
	$assert(strpos($staffTasksAdminUiSource, "'bvmgr-tasks-event-plan-metabox'") !== false, 'Staff Tasks admin UI should still enqueue the canonical Event Plan staff-task asset.');
