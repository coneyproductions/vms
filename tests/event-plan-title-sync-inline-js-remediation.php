...
try {
	...
	$assert(strpos($eventPlansSource, 'const autoChk = document.getElementById(\'vms_auto_comp_venue\');') !== false, 'Adjacent venue/default-compensation controller should remain inline.');
	$assert(substr_count($eventPlansSource, '<script') >= 7, 'WPORG-22 B1 should still have seven other active inline Event Plan script blocks after the title migration.');
