...
$had_external_render_state = array_key_exists('vms_external_ticketing_panel_rendered', $GLOBALS);
$original_external_render_state = $GLOBALS['vms_external_ticketing_panel_rendered'] ?? null;
...
$cleanup = static function () use (&$created_posts, &$created_orders, &$original_post, &$original_get, &$original_request, &$original_wp_query, &$original_wp_the_query, &$original_global_post, &$sponsorship_mode_option, &$missing_option_marker, &$original_sponsorship_mode, &$had_sponsor_render_state, &$original_sponsor_render_state, &$had_external_render_state, &$original_external_render_state): void {
	...
	if ($had_external_render_state) {
		$GLOBALS['vms_external_ticketing_panel_rendered'] = $original_external_render_state;
	} else {
		unset($GLOBALS['vms_external_ticketing_panel_rendered']);
	}
...
try {
	...
	$assert(function_exists('vms_event_plan_get_ticket_destination'), 'Canonical ticket destination helper is not loaded.');
	$assert(function_exists('vms_ticketing_v2_validate_product_sale_context'), 'Ticket sale guard is not loaded.');
	$assert(function_exists('vms_build_tec_event_args'), 'TEC event payload builder is not loaded.');
	...
	$status_key = function_exists('vms_meta_key') ? (string) (vms_meta_key('event_plan', 'status') ?: '_vms_event_plan_status') : '_vms_event_plan_status';
	...
	$plan_marker_key = function_exists('vms_ticketing_v2_product_meta_key') ? vms_ticketing_v2_product_meta_key('event_plan_id') : '_vms_event_plan_id';
	$role_marker_key = function_exists('vms_ticketing_v2_product_meta_key') ? vms_ticketing_v2_product_meta_key('product_role') : '_vms_product_role';
	...
	$commerce_hook = vms_event_details_commerce_hook();
	$assert($commerce_hook === vms_event_details_external_ticketing_commerce_hook(), 'External ticketing no longer uses the canonical BVM commerce location.');
	$assert(vms_event_details_before_commerce_priority() < vms_event_details_commerce_priority(), 'The canonical sponsor priority must precede commerce.');
	...
	$assert(has_action($commerce_hook, array($native_provider, 'maybe_add_front_end_tickets_form')) === vms_event_details_commerce_priority(), 'Native Event Tickets commerce is not mounted at the canonical commerce priority.');
	$assert(has_action($commerce_hook, 'vms_event_details_render_external_ticketing_at_commerce_location') === vms_event_details_commerce_priority(), 'External ticketing is not mounted at the canonical commerce priority.');
	...
	$assert(
		$find_callback_priority($commerce_hook, 'VMS_Sponsorships_Shortcodes', 'render_automatic_event_page_banner') === vms_event_details_before_commerce_priority(),
	...
	$render_commerce_state = static function () use ($commerce_hook, $event_id, $set_event_query, $native_provider): string {
		...
		$GLOBALS['vms_external_ticketing_panel_rendered'] = array();
		ob_start();
	...
	$assert(vms_event_plan_get_ticketing_sales_mode($plan_id) === 'serenade_range', 'Missing mode metadata must resolve to native Serenade Range ticketing.');
	$assert(vms_event_plan_is_ticketing_enabled($plan_id), 'A native plan with ticketing enabled should remain enabled.');
	$native_destination = vms_event_plan_get_ticket_destination($plan_id, (string) get_permalink($event_id));
	...
	$native_sale = vms_ticketing_v2_validate_product_sale_context($product_id, $plan_id, $event_id, 'ga_ticket');
	$assert(!empty($native_sale['ok']), 'Existing native ticket sale context should remain valid.');
	$native_args = vms_build_tec_event_args($plan_id, $event_id);
	$assert(strpos((string) ($native_args['EventCost'] ?? ''), '$20') !== false, 'Native TEC payload did not expose the expected $20 price.');
	$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'Initial native Event Plan sync failed.');
	...
	$assert(vms_event_plan_is_externally_ticketed($plan_id), 'External sales mode did not persist.');
	$assert(vms_event_plan_get_external_ticket_provider($plan_id) === 'Test Tickets', 'Provider label did not persist.');
	$assert(vms_event_plan_get_relationship($plan_id) === 'hosted_third_party', 'Hosted relationship did not persist.');
	$assert(vms_event_plan_get_external_event_producer($plan_id) === 'Rehearsal Room Tyler', 'Producer label did not persist.');
	$assert(vms_event_plan_get_external_event_producer_website($plan_id) === '', 'Invalid presenter website was not rejected by the Event Plan save path.');
	$assert(vms_event_plan_get_external_ticket_url($plan_id) === '', 'Incomplete external draft should retain an empty destination.');
	...
	$assert(vms_event_plan_sanitize_external_ticket_url('javascript:alert(1)') === '', 'Unsafe external URL scheme was accepted.');
	$assert(vms_event_plan_sanitize_external_ticket_url('/relative/tickets') === '', 'Relative external URL was accepted.');
	$assert(vms_event_plan_sanitize_external_event_producer_website('javascript:alert(1)') === '', 'Unsafe presenter URL scheme was accepted.');
	$readiness_errors = vms_validate_event_plan($plan_id);
	...
	$assert(vms_event_plan_get_external_event_producer_website($plan_id) === $presenter_url, 'Valid presenter website did not persist.');

	$destination = vms_event_plan_get_ticket_destination($plan_id, (string) get_permalink($event_id));
	$assert(!empty($destination['is_external']) && $destination['url'] === $external_url, 'Canonical destination did not resolve externally.');
	$assert(!vms_event_plan_native_ticket_purchasing_allowed($plan_id), 'External mode still allows native ticket purchasing.');
	...
	$tec_args = vms_build_tec_event_args($plan_id, $event_id);
	...
	$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'External Event Plan did not update through the normal public-event publishing path.');
	...
	$legitimate_website_args = vms_build_tec_event_args($plan_id, $event_id);
	...
	$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'External resync with an independent event website failed.');
	...
	if (function_exists('vms_calendar_feed_cache_bust')) vms_calendar_feed_cache_bust();
	$calendar_events = vms_get_calendar_events(array(
		'context' => 'public',
	...
	$list_markup = vms_public_calendar_render_list_view(array($calendar_row), false, true, false);
	...
	$photo_cta = vms_events_photo_cta_context($calendar_row, array('is_cancelled' => false, 'is_rescheduled' => false));
	...
	$event_context = vms_event_details_context($event_id);
	...
	$external_schema = vms_event_details_clean_tec_offers_schema(array('offers' => array(array('url' => 'https://native.invalid', 'price' => '25.00'))), $event_context, $event_id);
	...
	$fallback_ticket_context = vms_event_details_ticket_context($event_id, $plan_id);
	...
	$set_event_query($event_id);
	$hosted_panel = vms_event_details_render_external_ticketing_panel($event_id, $plan_id);
	...
	$assert(has_action($commerce_hook, 'vms_event_details_render_external_ticketing_at_commerce_location') !== false, 'External purchase card is not registered at the configured Event Tickets commerce location.');
	$GLOBALS['vms_external_ticketing_panel_rendered'] = array();
	ob_start();
	do_action($commerce_hook);
	do_action($commerce_hook);
	...
	$plain_presenter_panel = vms_event_details_render_external_ticketing_panel($event_id, $plan_id);
	...
	$orphan_presenter_panel = vms_event_details_render_external_ticketing_panel($event_id, $plan_id);
	...
	$produced_panel = vms_event_details_render_external_ticketing_panel($event_id, $plan_id);
	...
	$query_args = vms_tec_suppress_tickets_for_cancelled_events(array('post_parent' => $event_id));
	$assert(($query_args['post__in'] ?? null) === array(0), 'Native TEC ticket selector query was not suppressed.');
	$assert(vms_ticketing_v2_render_entitlements_block($event_id, $plan_id) === '', 'Native ticket/add-on widget output was not suppressed.');
	$external_sale = vms_ticketing_v2_validate_product_sale_context($product_id, $plan_id, $event_id, 'ga_ticket');
	...
	if (function_exists('wc_clear_notices') && function_exists('WC') && WC() && isset(WC()->cart) && WC()->cart) {
		wc_clear_notices();
		$direct_add = vms_ticketing_v2_validate_add_to_cart(true, $product_id, 1, 0, array(), array());
		$assert($direct_add === false, 'Direct native add-to-cart was not blocked in external mode.');
		vms_ticketing_v2_enforce_no_cancelled_event_items_in_cart();
	...
	$commit = vms_ticketing_v2_commit_sync($plan_id, 'external-proof-preview');
	...
	$link_result = vms_ticketing_v2_ensure_tec_event_link($plan_id);
	...
	$assert(vms_event_plan_is_ticketing_enabled($plan_id), 'Switching back did not restore native ticketing: mode=' . get_post_meta($plan_id, '_vms_ticketing_sales_mode', true) . ', override=' . get_post_meta($plan_id, '_vms_ticketing_enabled_override', true));
	...
	$restored_sale = vms_ticketing_v2_validate_product_sale_context($product_id, $plan_id, $event_id, 'ga_ticket');
	$assert(!empty($restored_sale['ok']), 'Switching back did not restore native sale validation: ' . wp_json_encode($restored_sale));
	$restored_query = vms_tec_suppress_tickets_for_cancelled_events(array('post_parent' => $event_id));
	$assert(($restored_query['post__in'] ?? null) !== array(0), 'Switching back still suppresses native ticket queries.');
	$assert(vms_tec_suppress_ticket_forms_for_cancelled_event($commerce_hook, null) === $commerce_hook, 'Switching back did not restore the configured native ticket-form hook.');
	$assert(vms_event_details_render_external_ticketing_panel($event_id, $plan_id) === '', 'Native mode still rendered the external purchase card.');
	$native_again_args = vms_build_tec_event_args($plan_id, $event_id);
	$assert(strpos((string) ($native_again_args['EventCost'] ?? ''), '$20') !== false, 'Native price did not return in the second native TEC payload.');
	$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'Second native Event Plan sync failed.');
	$make_native_ticket_available();
	...
	$external_again_args = vms_build_tec_event_args($plan_id, $event_id);
	...
	$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'Second external Event Plan sync failed.');
	...
	$external_again_query = vms_tec_suppress_tickets_for_cancelled_events(array('post_parent' => $event_id));
	$assert(($external_again_query['post__in'] ?? null) === array(0), 'Second external switch restored the native selector query.');
	$final_panel = vms_event_details_render_external_ticketing_panel($event_id, $plan_id);
	...
	$final_context = vms_event_details_context($event_id);
	$final_schema = vms_event_details_clean_tec_offers_schema(array('offers' => array(array('url' => get_permalink($product_id), 'price' => '20.00'))), $final_context, $event_id);
	...
	if (getenv('VMS_TEST_KEEP_FIXTURES') === '1') {
		...
		if ($keep_mode === 'native') {
			update_post_meta($plan_id, '_vms_ticketing_sales_mode', 'serenade_range');
			$assert(vms_publish_event_to_calendar($plan_id, get_post($plan_id)), 'Could not leave the retained HTTP fixture in native mode.');
			$make_native_ticket_available();
