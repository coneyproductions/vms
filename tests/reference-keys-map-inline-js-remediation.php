...
try {
	...
	$assert(strpos($keysMapSource, 'function vms_admin_reference_keys_map_enqueue_assets()') !== false, 'Reference Keys Map should declare a dedicated admin_enqueue_scripts callback.');
	...
	$assert(
		preg_match(
			"~add_submenu_page\\(\\s*'vms-dashboard',\\s*__\\('Reference: Keys \\+ Identifiers', 'backstage-venue-manager'\\),\\s*__\\('Reference: Keys \\+ Identifiers', 'backstage-venue-manager'\\),\\s*'manage_options',\\s*'vms-reference-keys-map',\\s*'vms_admin_reference_keys_map_page'\\s*\\);~s",
			$menuSource
		) === 1,
