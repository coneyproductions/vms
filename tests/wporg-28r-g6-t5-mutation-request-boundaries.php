...
vms_t5_assert_order("check_admin_referer(bvmgr_nonce_action_for_request('bvmgr_event_plan_import_commit', '_wpnonce'), '_wpnonce');", 'bvmgr_event_plan_import_read_selected_rows_from_post($_POST)', $sources['event_import'], 'Event Plan Import should keep nonce verification before selected-row reads.');
vms_t5_assert_order("check_ajax_referer(bvmgr_nonce_action_for_request('bvmgr_ticketing_nonce', 'nonce'), 'nonce', false)", 'bvmgr_ticketing_b_request_payload_value($_POST, \'tiers\'', $sources['phase_b'], 'Phase-B tiers should keep nonce verification before payload reads.');
vms_t5_assert_order("check_ajax_referer(bvmgr_nonce_action_for_request('bvmgr_ticketing_nonce', 'nonce'), 'nonce', false)", 'bvmgr_ticketing_b_request_payload_value($_POST, \'config\'', $sources['phase_b'], 'Ticketing V2 config should keep nonce verification before payload reads.');
vms_t5_assert_order("check_ajax_referer(bvmgr_nonce_action_for_request('bvmgr_tours', 'nonce'), 'nonce', true);", '$this->read_ajax_prefs_from_request($_POST)', $sources['tours_service'], 'Tours prefs should keep nonce verification before prefs reads.');
