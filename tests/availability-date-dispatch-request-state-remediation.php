...
eval(vms_test_extract_function($adminUiSource, 'vms_add_dispatch_enqueue_admin_assets'));
eval(vms_test_extract_function($adminUiSource, 'vms_add_dispatch_should_render_shell_count'));
eval(vms_test_extract_function($adminUiSource, 'vms_add_dispatch_dashboard_filters'));
eval(vms_test_extract_function($adminUiSource, 'vms_add_dispatch_render_assignment_review'));
eval(vms_test_extract_function($helpersSource, 'vms_add_dispatch_get_request_token'));
...
vms_test_assert_contains(
	"\$dashboard_filters = vms_add_dispatch_dashboard_filters(\$_GET);",
	$adminUiSource,
...
$filters = vms_add_dispatch_dashboard_filters(array(
	'show_full_groups' => '1',
...
$_GET = array();
$_SERVER['REQUEST_URI'] = '';
vms_test_assert_same('query-token', vms_add_dispatch_get_request_token(), 'ADD response-token lookup should still prefer the routed query var.');

$GLOBALS['vms_test_query_vars']['vms_add_dispatch_token'] = '';
$_GET = array('vms_add_dispatch_token' => 'raw%20token');
$_SERVER['REQUEST_URI'] = '';
vms_test_assert_same('raw token', vms_add_dispatch_get_request_token(), 'ADD response-token lookup should still accept a normal scalar query-string token.');

$_GET = array('vms_add_dispatch_token' => array('bad-token'));
$_SERVER['REQUEST_URI'] = '/availability-dispatch/respond/uri%20token';
vms_test_assert_same('uri token', vms_add_dispatch_get_request_token(), 'ADD response-token lookup should reject array-shaped query-string tokens and fall back to the routed URI token.');

$_GET = array('choice' => 'yes');
vms_test_assert_same('available', vms_add_dispatch_get_request_choice(), 'ADD response-choice lookup should preserve the yes => available normalization.');
$_GET = array('choice' => 'unavailable');
vms_test_assert_same('unavailable', vms_add_dispatch_get_request_choice(), 'ADD response-choice lookup should preserve the direct available/unavailable choices.');
$_GET = array('choice' => array('yes'));
vms_test_assert_same('', vms_add_dispatch_get_request_choice(), 'ADD response-choice lookup should reject array-shaped choice values.');

$GLOBALS['vms_test_vms_screen'] = false;
$_GET = array('page' => 'vms-dashboard');
vms_test_assert_same(true, vms_add_dispatch_should_render_shell_count(), 'ADD shell-count gating should preserve the dashboard page slug.');
$_GET = array('page' => array('vms-dashboard'));
vms_test_assert_same(false, vms_add_dispatch_should_render_shell_count(), 'ADD shell-count gating should reject array-shaped page values.');
...
vms_add_dispatch_enqueue_admin_assets();
...
vms_add_dispatch_render_assignment_review(55);
ob_end_clean();
