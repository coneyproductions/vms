...
$assert(bvmgr_request_read_scalar(array('name' => 'Band O\\\'Clock'), 'name') === "Band O'Clock", 'Scalar reads should unslash valid strings.');
$assert(bvmgr_request_read_scalar(array(), 'missing') === '', 'Scalar reads should return empty for missing values.');
$assert(bvmgr_request_read_scalar(array('empty' => '   '), 'empty') === '', 'Scalar reads should trim empty strings to empty.');
$assert(bvmgr_request_read_scalar(array('name' => array('bad')), 'name') === '', 'Scalar reads should drop array-shaped input.');
$assert(bvmgr_request_read_scalar(array('name' => array(array('nested'))), 'name') === '', 'Scalar reads should drop nested array input.');
$assert(bvmgr_request_read_scalar(array('enabled' => true), 'enabled') === '1', 'Scalar reads should normalize boolean true when a string consumer expects a scalar.');
$assert(bvmgr_request_read_scalar(array('enabled' => false), 'enabled') === '', 'Scalar reads should normalize boolean false to an empty scalar string.');
$assert(bvmgr_request_read_scalar(array('count' => 42), 'count') === '42', 'Scalar reads should normalize integer scalars when a string consumer expects a scalar.');
$assert(bvmgr_request_read_text_field(array('field' => '  Hello  '), 'field') === 'Hello', 'Text field reads should sanitize and trim scalar input.');
$assert(bvmgr_request_read_text_field(array('field' => false), 'field') === '', 'Text field reads should safely reject false-like scalar input.');
$assert(bvmgr_request_read_textarea_field(array('field' => "Line 1\nLine 2"), 'field') === "Line 1\nLine 2", 'Textarea reads should preserve multiline content after unslashing.');
$assert(bvmgr_request_read_email(array('email' => ' user@example.test '), 'email') === 'user@example.test', 'Email reads should sanitize valid email addresses.');
$assert(bvmgr_request_read_email(array('email' => 'not-an-email'), 'email') === '', 'Email reads should reject invalid email addresses.');
$normalizedKey = bvmgr_request_read_key(array('tab' => 'Vendor_Portal'), 'tab');
$assert($normalizedKey === 'vendor_portal', 'Key reads should normalize to lowercase key format. Got ' . var_export($normalizedKey, true));
$assert(bvmgr_request_read_absint(array('id' => '42'), 'id') === 42, 'absint reads should preserve valid numeric identifiers.');
$assert(bvmgr_request_read_absint(array('id' => array('42')), 'id') === 0, 'absint reads should drop array-shaped identifiers.');

$assert(bvmgr_request_read_bool_flag(array('enabled' => '1'), 'enabled') === true, 'Bool flags should accept truthy scalar strings.');
$assert(bvmgr_request_read_bool_flag(array('enabled' => 'false'), 'enabled') === false, 'Bool flags should reject explicit false-like strings.');
$assert(bvmgr_request_read_bool_flag(array('enabled' => true), 'enabled') === true, 'Bool flags should preserve boolean true inputs.');
$assert(bvmgr_request_read_bool_flag(array('enabled' => ''), 'enabled') === false, 'Bool flags should reject empty scalar values.');
$assert(bvmgr_request_read_bool_flag(array('enabled' => array('1')), 'enabled') === false, 'Bool flags should reject array-shaped input.');

$_SERVER = array();
$assert(bvmgr_request_server_value('MISSING') === '', 'Server reads should return empty when the key is missing.');
$assert(bvmgr_request_method() === 'get', 'Request method should default to get when missing.');
$assert(bvmgr_request_method('HEAD') === 'head', 'Request method should normalize custom fallbacks.');
$assert(bvmgr_request_current_uri('/fallback') === '/fallback', 'Request URI should preserve the provided fallback when missing.');
$assert(bvmgr_request_remote_addr() === '', 'Remote address should return empty when missing.');
$assert(bvmgr_request_user_agent() === '', 'User agent should return empty when missing.');

$_SERVER['REQUEST_METHOD'] = array('POST');
$assert(bvmgr_request_server_value('REQUEST_METHOD') === '', 'Server reads should reject array-shaped values.');
$assert(bvmgr_request_method('HEAD') === 'head', 'Request method should fall back safely when the server value is malformed.');
$_SERVER['REQUEST_URI'] = array('/bad');
$assert(bvmgr_request_current_uri('/fallback') === '/fallback', 'Current URI should fall back safely when the server value is malformed.');
$_SERVER['HTTP_ACCEPT'] = array('text/html');
$assert(bvmgr_request_server_value('HTTP_ACCEPT') === '', 'Server reads should reject malformed accept headers.');
...
$assert(bvmgr_request_server_value('REQUEST_METHOD') === 'PoSt', 'Server reads should preserve scalar values after unslashing.');
$assert(bvmgr_request_server_value('HTTP_ACCEPT') === 'application/json, text/html', 'Server reads should unslash and preserve diagnostic headers.');
$assert(bvmgr_request_method() === 'post', 'Request method should normalize to lowercase keys.');
$assert(bvmgr_request_current_uri('/') === '/vendor-portal/dashboard?tab=dashboard', 'Request URI should normalize missing leading slashes and strip control bytes.');
$assert(bvmgr_request_remote_addr() === '127.0.0.1', 'Remote address should be trimmed and sanitized.');
$assert(strpos(bvmgr_request_user_agent(), 'Browser') === 0, 'User agent normalization should preserve the diagnostic prefix.');
$assert(strlen(bvmgr_request_user_agent()) === 255, 'User agent should be length-bounded.');
$_SERVER['REQUEST_URI'] = str_repeat('a', 2100);
$assert(strlen(bvmgr_request_current_uri('/')) === 2048, 'Request URI normalization should enforce the configured length bound.');

$safeLocalRedirect = bvmgr_request_local_redirect(home_url('/fallback'), '/vendor-portal/?tab=availability');
$assert($safeLocalRedirect === '/vendor-portal/?tab=availability', 'Valid local redirects should be preserved.');
$sameHostRedirect = bvmgr_request_local_redirect(home_url('/fallback'), 'https://example.test/vendor-portal/?tab=dashboard');
$assert($sameHostRedirect === 'https://example.test/vendor-portal/?tab=dashboard', 'Same-host absolute redirects should be preserved.');
$externalRedirect = bvmgr_request_local_redirect(home_url('/fallback'), 'https://evil.test/phish');
$assert($externalRedirect === home_url('/fallback'), 'External redirects should fall back to the safe local destination.');
$schemeRelativeRedirect = bvmgr_request_local_redirect(home_url('/fallback'), '//evil.test/phish');
$assert($schemeRelativeRedirect === home_url('/fallback'), 'Scheme-relative external redirects should be rejected.');
$malformedRedirect = bvmgr_request_local_redirect(home_url('/fallback'), '://broken');
$assert($malformedRedirect === home_url('/fallback'), 'Malformed redirect values should fall back safely.');
$arrayRedirect = bvmgr_request_local_redirect(home_url('/fallback'), array('bad'));
$assert($arrayRedirect === home_url('/fallback'), 'Array-shaped redirect values should resolve to the safe fallback.');
$emptyRedirect = bvmgr_request_local_redirect(home_url('/fallback'), '');
$assert($emptyRedirect === home_url('/fallback'), 'Empty redirect values should preserve the established safe fallback.');

$_GET['page'] = array('bad');
$assert(bvmgr_admin_ui_query_arg('page') === '', 'Admin UI query helper should drop array-shaped request values without warnings.');
$_GET['page'] = 'vms-guided-tours';
$assert(bvmgr_admin_ui_query_arg('page') === 'vms-guided-tours', 'Admin UI query helper should preserve valid scalar page slugs.');
