...
try {
	...
	$assert(strpos($eventPlansSource, 'const bypassSetBtn = document.getElementById(\'vms-tax-bypass-set\');') !== false, 'Primary-vendor tax should remain an active inline controller after the shell migration.');
	$assert(substr_count($eventPlansSource, '<script') >= 4, 'B1 should still have other active Event Plan inline script blocks after this shell-only slice.');
