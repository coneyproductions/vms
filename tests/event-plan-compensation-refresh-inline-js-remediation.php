...
try {
	...
	$assert(strpos($eventPlansSource, "document.addEventListener('vms_comp_options_updated', () => {") !== false, 'The large inline compensation controller should still listen for vms_comp_options_updated.');
	...
	$assert(substr_count($eventPlansSource, '<script') >= 6, 'WPORG-22 B1 should still have active inline Event Plan script blocks after the compensation migration.');
