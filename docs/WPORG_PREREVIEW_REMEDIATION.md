...
- Ordering and untouched neighboring boundaries:
  ...
  - Subsequent `WPORG-24` slices later closed the Ticketing stock preview / commit family and the integrity-scan result family. Remaining richer Settings boundaries still include the nested default-venue alert family with buttons and emphasis, the richer entitlement image sync completion + error details output, Settings API/editor output, and other content-local notice families.
