...
- Candidate outcome summary:
  ...
  - Guided Tours was deferred because its reset-success notice remains nested inside the `.vms-tours-admin-page` content wrapper and also carries an extra `data-vms-tour` attribute on the outer `div`, so it does not fit the simple explicit-notice contract or the direct captured-notice boundary requirement.
...
- Ordering and untouched neighboring boundaries:
  ...
  - Guided Tours remained unchanged as a nested tour-specific content boundary, and Schedule remained unchanged as a nested content boundary with richer coexistence.
