...
- Result: `PASS`
- Exact finding identifier: `WPORG-28R-G3`
- Starting mirror HEAD: `eafb553c20127e9b3d55735a567681bf97902046` (`Reconcile admin dashboard request boundaries`)
- Starting parent: `305a154f638c68699525a65fbde14535db500a57`
- Authorized runtime scope: `includes/cpt/event-plans.php`, `includes/core/event-plan-performance.php`, `includes/core/event-plan-save-profiler.php`, `includes/core/event-plan-review.php`, and `includes/cpt/event-plans/partials/workflow-status.php`
- Authorized support scope used: `tests/event-plan-performance-request-id-remediation.php`, `tests/event-plan-review-json-characterization.php`, `docs/WPORG_PREREVIEW_REMEDIATION.md`, `docs/WPORG_PLUGIN_CHECK_TRIAGE_1.0.0.md`, and `docs/wporg-remediation-ledger.md`
- Live-tree rule preserved: the sibling `../../vms/` tree remained read-only and unchanged throughout this child
- Pre-edit package command: `php scripts/build-public-release.php --output-dir /tmp/wporg-28rg3-pre.tx7DCg/build --force`
- Pre-edit package result: `/tmp/wporg-28rg3-pre.tx7DCg/build/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `aa51bb709761a5b77ea61e65428d407eb6811e8ff841dc1d23b2f1dd7caae0de`
- Pre-edit packaged Plugin Check result: exit `0`, `265` errors, `1508` warnings, `1773` total findings, `20` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1644`
- Exact G3 ownership: `164` packaged rows (`WordPress.Security.NonceVerification.Missing=23`, `WordPress.Security.NonceVerification.Recommended=76`, `WordPress.Security.ValidatedSanitizedInput.InputNotSanitized=30`, `WordPress.Security.ValidatedSanitizedInput.InputNotValidated=3`, `WordPress.Security.ValidatedSanitizedInput.MissingUnslash=32`) across the five authorized runtime files only

### Boundary Characterization

- The owned rows split across `CPT_SAVE_HANDLER`, `ADMIN_PAGE`, and `REPORTING` boundaries inside the Event Plan editor save path, read-only admin request routing, save profiler, workflow status partial, and Event Plan performance diagnostics.
- The child kept mutation, autosave, preview, refund, and reporting request surfaces distinct. Editor save reads now flow through a nonce-verified normalized POST snapshot, read-only GET and generic request reads now flow through centralized helper-backed unslashing, and profiling/review helpers retain narrow diagnostics-only or fallback-only request access with occurrence-specific suppressions instead of widening lifecycle behavior.
- The same-file deferred families remained unchanged by design: DB/meta-query rows in `includes/cpt/event-plans.php` still belong to `WPORG-28R-G13`, Event Plan performance and save-profiler trace logging rows still belong to `WPORG-28R-G17`, and the same-file accepted packaged `WordPress.Security.EscapeOutput.OutputNotEscaped` rows in `includes/cpt/event-plans.php` remain outside the blocker roadmap.
- The live local plugin tree was intentionally not synchronized in this child because the task explicitly prohibited changing `../../vms/`; the focused pure-PHP parity suites were adjusted to treat the live PHP files as read-only comparators while still asserting the current mirror contracts.

### Runtime And Test Changes

- Added centralized request snapshots in `includes/cpt/event-plans.php` for current GET, POST, and REQUEST data plus a nonce-verified Event Plan editor POST helper, then routed the authorized Event Plan editor render/save and request-wrapper call sites through those helpers instead of direct superglobal reads.
- Converted the G3-owned save-path reads in `includes/cpt/event-plans.php` to helper-backed scalar/array access, preserved the existing mutation nonce/capability ordering, and removed redundant post-helper `wp_unslash()` calls so normalized request values are not over-unslashed.
- Added helper-backed request sampling in `includes/core/event-plan-performance.php` and `includes/core/event-plan-save-profiler.php`, limited to diagnostics-only request context with narrow suppressions, and removed the remaining G3-owned direct request reads in the current-plan, scenario, admin-detection, and save-pass helpers.
- Added a local normalized POST fallback in `includes/core/event-plan-review.php` so `vms_event_plan_review_after_save()` reads the editor action through the same verified request boundary without keeping direct owned `$_POST` accesses in the save hook.
- Updated `tests/event-plan-performance-request-id-remediation.php` and `tests/event-plan-review-json-characterization.php` so the live PHP files remain read-only comparators while the mirror-only G3 remediation leaves `../../vms/` untouched; the live files still must remain readable and the current mirror contracts remain fully asserted.

### Verification

- PHP lint passed for every changed runtime file: `php -l includes/cpt/event-plans.php`, `php -l includes/core/event-plan-performance.php`, `php -l includes/core/event-plan-save-profiler.php`, `php -l includes/core/event-plan-review.php`, and `php -l includes/cpt/event-plans/partials/workflow-status.php`
- Focused suites passed: `php tests/event-plan-legacy-ticketing-integration-smoke.php`, `php tests/event-plan-performance-request-id-remediation.php`, `php tests/event-plan-readiness-details-output-remediation.php`, `php tests/decoded-json-validation.php`, and `php tests/event-plan-review-json-characterization.php`
- `git diff --check` passed
- Post-edit package command: `php scripts/build-public-release.php --output-dir /tmp/wporg-28rg3-post.QJKmSA/build --force --allow-dirty`
- Post-edit package result: `/tmp/wporg-28rg3-post.QJKmSA/build/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `568772457a2d5c1ad364b4700190ccc59fada6ad0c00fb284c7a2243f5db1cfb`
- Post-edit packaged Plugin Check result: exit `0`, `265` errors, `1344` warnings, `1609` total findings, `19` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1480`
- Exact G3 code deltas: `WordPress.Security.NonceVerification.Missing 75 -> 52 (-23)`, `WordPress.Security.NonceVerification.Recommended 258 -> 182 (-76)`, `WordPress.Security.ValidatedSanitizedInput.InputNotSanitized 91 -> 61 (-30)`, `WordPress.Security.ValidatedSanitizedInput.InputNotValidated 3 -> 0 (-3)`, and `WordPress.Security.ValidatedSanitizedInput.MissingUnslash 68 -> 36 (-32)`; every other remaining blocker-family count stayed unchanged, and `WordPress.Security.ValidatedSanitizedInput.InputNotValidated` disappeared from the packaged rule-code set entirely
- Exact G3 row result: the `164` owned packaged rows were removed exactly, no G3-owned nonce/input row remained in the post-edit package, and no new G3-owned nonce/input row appeared
- Deferred-family continuity: the same-file deferred DB/logging counts in the G3 file set stayed exactly `15` rows (`DirectQuery=1`, `NoCaching=1`, `NotPrepared=1`, `slow_db_query_meta_query=6`, and `error_log=6`), the same-file accepted `WordPress.Security.EscapeOutput.OutputNotEscaped` count in the G3 file set stayed exactly `48`, and the global packaged `WordPress.Security.EscapeOutput.OutputNotEscaped` total stayed exactly `127`

### Current Parent State

- Current blocker-family totals after `WPORG-28R-G3`: DB/SQL `1082`, nonce/input `331`, date/time `25`, and logging `42`
- `WPORG-28R-G3` is terminal under `verified`
- `WPORG-28R` remains blocked until every remaining `G4` through `G17` child closes and a fresh packaged strict-json rerun proves `SUBMISSION_BLOCKER=0`
- `WPORG-28`, `WPORG-28Q`, `Review-2 Name/Slug Closeout`, and `Review-13 Final Actions` all remain blocked or limited exactly as reflected in the ledger
- Exact next implementation child: `WPORG-28R-G4 — Ancillary CPT save boundaries`
...
- Result: `PASS`
- Exact finding identifier: `WPORG-28R-G6-T1`
- Title: `Normalize ticketing claims and verification request state`
- Starting mirror HEAD: `05bf92ece0010e2cc25190c74ee40f5a0ce7b2a2` (`Reconcile public and portal request boundaries`)
- Starting parent: `9647f377a943e0a57091f3c00df264a5ba560a93`
- Authorized runtime scope: `includes/integrations/ticketing-claims-admin.php`, `includes/integrations/ticketing-claims-customer.php`, `includes/integrations/ticketing-rules-v2.php`, `includes/integrations/ticketing-verifications.php`, and `includes/integrations/ticketing.php`
- Authorized support scope used: `tests/ticket-claims-assignee-validation.php`, `tests/ticketing-rules-v2-request-path-normalization.php`, `tests/ticketing-claims-admin-request-state-remediation.php`, `tests/ticketing-verifications-request-state-remediation.php`, `docs/WPORG_PREREVIEW_REMEDIATION.md`, `docs/WPORG_PLUGIN_CHECK_TRIAGE_1.0.0.md`, and `docs/wporg-remediation-ledger.md`
...
- Pre-edit package command: `php scripts/build-public-release.php --output-dir /tmp/wporg-28rg6-inventory.cXX8Of --force`
- Pre-edit package result: `/tmp/wporg-28rg6-inventory.cXX8Of/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `b1f691453029da5bccda978a30568c8d245f4725facd39b7dc9071a860a775d1`
- Pre-edit packaged Plugin Check result: exit `0`, `265` errors, `1202` warnings, `1467` total findings, `19` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1338`
- Exact G6-T1 ownership: `66` packaged rows across the five authorized runtime files only (`Recommended 64`, `InputNotSanitized 1`, `MissingUnslash 1`, `Missing 0`) covering `46` distinct boundaries

### Boundary Characterization

- This child stayed inside the read-only ticketing claims and verification surface: admin-page routing, admin notices, lookup/filter/search query state, account-panel expansion, Woo request-path probes, verification panel state, proof-view request IDs, and admin asset/post scoping.
- The child used subsystem-local helper wrappers around the existing shared request readers so malformed array inputs now fail closed before sanitization, while the current read-only GET/REQUEST semantics remain unchanged.
- No mutation nonce, capability, upload, redirect, AJAX payload, REST payload, or structured POST array ownership was broadened here; those remaining same-subsystem mutation/request-shape rows stay assigned to `WPORG-28R-G6-T2`.
- Same-file residual nonce/input rows in the five touched runtime files now total `35`, and every residual row maps to later `T2` or `T5` ownership rather than to `T1`.

### Runtime And Test Changes

- Added local read-only query helpers in `includes/integrations/ticketing-claims-admin.php` and routed claims-page slug detection, admin notices, metabox lookup/filter state, and admin-page filter state through those helpers with narrow `NonceVerification.Recommended` read-only suppressions.
- Added the customer benefits flag helper in `includes/integrations/ticketing-claims-customer.php` so account-panel expansion now reads through the shared scalar guard instead of direct `$_GET`.
- Added bounded request/query helpers in `includes/integrations/ticketing-rules-v2.php` so Woo `wc-ajax`, `add-to-cart`, and `rest_route` routing state now uses helper-backed scalar or presence probes while preserving current cart and Store API behavior.
- Added verification-panel helpers in `includes/integrations/ticketing-verifications.php` so notice, program, request ID, debug flag, and local-return URL state now flows through helper-backed scalar readers; the local redirect helper also closes the prior `InputNotSanitized` plus `MissingUnslash` pair on `vms_return_to`.
- Added the admin asset-state helper in `includes/integrations/ticketing.php` so the legacy ticketing admin enqueue path now reads `post` through the shared scalar guard before falling back to the global post object.
- Extended `tests/ticket-claims-assignee-validation.php` and `tests/ticketing-rules-v2-request-path-normalization.php`, and added the new focused suites `tests/ticketing-claims-admin-request-state-remediation.php` and `tests/ticketing-verifications-request-state-remediation.php`.

### Verification

- PHP lint passed for every changed PHP file: `php -l includes/integrations/ticketing-claims-admin.php`, `php -l includes/integrations/ticketing-claims-customer.php`, `php -l includes/integrations/ticketing-rules-v2.php`, `php -l includes/integrations/ticketing-verifications.php`, `php -l includes/integrations/ticketing.php`, `php -l tests/ticket-claims-assignee-validation.php`, `php -l tests/ticketing-rules-v2-request-path-normalization.php`, `php -l tests/ticketing-claims-admin-request-state-remediation.php`, and `php -l tests/ticketing-verifications-request-state-remediation.php`
- Focused G6-T1 suites passed: `php tests/ticket-claims-assignee-validation.php`, `php tests/ticketing-claims-ajax-output-buffer-ownership.php`, `php tests/ticketing-rules-v2-request-path-normalization.php`, `php tests/ticketing-v2-ajax-output-buffer-ownership.php`, `php tests/ticketing-output-buffer-lifecycle-characterization.php`, `php tests/verification-proof-normalization.php`, `php tests/private-file-upload-api-remediation.php`, `php tests/private-file-operations-boundary-remediation.php`, `php tests/event-plan-legacy-ticketing-integration-smoke.php`, `php tests/ticketing-claims-admin-request-state-remediation.php`, and `php tests/ticketing-verifications-request-state-remediation.php`
...
- Post-edit package command: `php scripts/build-public-release.php --output-dir /tmp/wporg-28rg6-t1-post.X3zo92 --force --allow-dirty`
- Post-edit package result: `/tmp/wporg-28rg6-t1-post.X3zo92/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `3e485e506985738ee43e5b05d39db4144d8ecf6c79d0e49b7befcb4788042213`
- Post-edit packaged Plugin Check command: `WP_CLI_PHP_ARGS='-d error_reporting=24575 -d display_errors=0' wp --path='/Users/treyconey/Local Sites/serenade-range-local-test-site/app/public' --skip-plugins=event-tickets,event-tickets-plus,the-events-calendar,woocommerce,woocommerce-square,vms plugin check '/tmp/wporg-28rg6-t1-post.X3zo92/extracted/backstage-venue-manager' --slug=backstage-venue-manager --mode=new --format=strict-json --fields=file,line,column,type,code,message,docs`
- Post-edit packaged Plugin Check result: exit `0`, `265` errors, `1136` warnings, `1401` total findings, `19` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and inferred `SUBMISSION_BLOCKER=1272`; the normalized strict JSON was saved at `/tmp/wporg-28rg6-t1-post.X3zo92/plugin-check.strict.json`, with the original noisy WP-CLI stdout preserved at `/tmp/wporg-28rg6-t1-post.X3zo92/plugin-check.stdout.raw`
- Exact G6-T1 code deltas: `WordPress.Security.NonceVerification.Missing 26 -> 26 (0)`, `WordPress.Security.NonceVerification.Recommended 126 -> 62 (-64)`, `WordPress.Security.ValidatedSanitizedInput.InputNotSanitized 25 -> 24 (-1)`, and `WordPress.Security.ValidatedSanitizedInput.MissingUnslash 12 -> 11 (-1)`; every other packaged rule-code count stayed unchanged, no new rule code appeared, and no blocker-family total increased
- Exact G6-T1 row result: the `66` owned packaged rows were removed exactly, no G6-T1-owned nonce/input row remained in the post-edit package, no new G6-T1-owned nonce/input row appeared, and no owned row migrated into another nonce/input code
- Residual continuity: the remaining nonce/input family now stands at `Missing 26`, `Recommended 62`, `InputNotSanitized 24`, and `MissingUnslash 11` (`123` total); same-file residual nonce/input rows in the five touched runtime files stayed bounded to `35` rows, all mapped to later `T2` or `T5` ownership

### Current Parent State

- Current blocker-family totals after `WPORG-28R-G6-T1`: DB/SQL `1082`, nonce/input `123`, date/time `25`, and logging `42`
- `WPORG-28R-G6-T1` is terminal under `verified`
...
- No `WPORG-28R-G6-T2`, `T3`, `T4`, or `T5` runtime remediation was attempted.
- No admissions or availability runtime file outside the five authorized T1 files was changed.
...
- `WPORG-28R-G8` is now terminal under `verified`; children `WPORG-28R-G8-T1` through `WPORG-28R-G8-T5` removed the full `233` rows across `52` boundaries.
- Current post-`T5` packaged baseline: `239` errors, `806` warnings, `1045` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=916`.
- Current blocker-family totals after `WPORG-28R-G8-T5`: DB/SQL `849`, nonce/input `0`, date/time `25`, and logging `42`.
- Current remaining G8 inventory: `0` rows across `0` boundaries.
- Exact next implementation child: `WPORG-28R-G9 — Admissions And Claim-State Query Boundaries`.
- Preserved mapping note: the preserved `g8-boundaries.tsv` still assigns `G8-B002` to `vms_staff_user_link_metabox_render()` lines `26-79`, but the current packaged slow-meta query is the later `save_post_vms_staff` closure at line `131`; the terminal post-`T5` G8 summary therefore uses `/tmp/wporg-28rg8-t5-work.ctlcQI/scope/current-boundary-ranges.tsv`, and the stale mapping does not affect `T1` through `T5` ownership counts.
- `WPORG-28R` remains blocked until `WPORG-28R-G9` through `WPORG-28R-G17` close and a fresh packaged strict-json rerun proves `SUBMISSION_BLOCKER=0`.
...
- Result: `PASS`
- Exact finding identifier: `WPORG-28R-G8-T1`
- Scope completed in this slice: only the Staff Tasks template, checklist, and admin selector repositories in `includes/modules/staff-tasks/store.php`, `includes/modules/staff-tasks/admin-ui.php`, and `includes/modules/staff-tasks/db.php`, plus the synchronized live counterparts under `../../vms/`
- Owned packaged rows: `48 -> 0`
- Distinct boundaries: `13 -> 0`
- Exact rule distribution: `UnescapedDBParameter 7`, `DirectQuery 16`, `NoCaching 12`, `InterpolatedNotPrepared 6`, and `NotPrepared 7`
- Evidence directory: `/tmp/wporg-28rg8-t1-work.2rY5sN`
- Mirror/live boundary: `store.php` and `db.php` stayed byte-identical across mirror/live after the edits; `admin-ui.php` still diverges only outside the owned selector helper, and every untouched live region stayed preserved

### Runtime And Test Changes

- Completed bounded identifier and preparation handling across the `11` owned `store.php` helpers, including explicit branch SQL construction in the list and applicability readers so Plugin Check no longer flags unfinished preparation on the built query strings.
- Updated `vms_tasks_admin_get_event_type_options()` to query the allowlisted `%i` `postmeta` identifier with line-local `DirectQuery` / `NoCaching` justification.
- Updated `vms_tasks_db_ready()` to probe Staff Tasks table existence with `SHOW TABLES LIKE %s` against `esc_like($table)` and line-local `DirectQuery` / `NoCaching` justification.
- Added and expanded `tests/staff-tasks-repository-sql-remediation.php` to assert exact SQL shapes, argument ordering, suppression inventory, and mirror/live target parity.

### Verification

- Focused suite passed: `php tests/staff-tasks-repository-sql-remediation.php`.
- Support reruns passed: `php tests/staff-tasks-overrides-json-remediation.php`, `php tests/staff-tasks-signature-json-remediation.php`, `php tests/authorization-boundary-hardening.php`, `php tests/vendor-portal-availability-autosave-remediation.php`, `php tests/runtime-stub-guards.php`, `php tests/release-compatibility-harness.php`, and `php tests/public-release-build-pipeline.php`.
- PHP lint passed for mirror/live `store.php`, `admin-ui.php`, and `db.php`, plus `tests/staff-tasks-repository-sql-remediation.php`; `git diff --check` also passed.
- Post-edit package command: `php scripts/build-public-release.php --output-dir '/tmp/wporg-28rg8-t1-work.2rY5sN/post-build-final.1785626108' --allow-dirty --force`
- Post-edit package result: `/tmp/wporg-28rg8-t1-work.2rY5sN/post-build-final.1785626108/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `318fc1c913f664a9778eaeedf8a88816eab44444a2cba8a47ecb753c788486a8`
- Post-edit packaged Plugin Check command: `php -d error_reporting=0 -d display_errors=0 /opt/homebrew/bin/wp --path='/Users/treyconey/Local Sites/serenade-range-local-test-site/app/public' --skip-plugins=event-tickets,event-tickets-plus,the-events-calendar,woocommerce,woocommerce-square,vms plugin check '/tmp/wporg-28rg8-t1-work.2rY5sN/post-plugin-check-final.1785626169/extracted.1785626169/backstage-venue-manager' --slug=backstage-venue-manager --mode=new --format=strict-json --fields=file,line,column,type,code,message,docs`
- Post-edit packaged Plugin Check result: exit `0`, empty stderr, `254` errors, `976` warnings, `1230` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1101`; `plugin-list.cmp=0`, and the final strict JSON is `/tmp/wporg-28rg8-t1-work.2rY5sN/post-plugin-check-final.1785626169/plugin-check.strict.json`.
- Exact T1 code deltas: `UnescapedDBParameter 143 -> 136 (-7)`, `DirectQuery 298 -> 282 (-16)`, `NoCaching 260 -> 248 (-12)`, `InterpolatedNotPrepared 137 -> 131 (-6)`, and `NotPrepared 67 -> 60 (-7)`; every other packaged rule-code count stayed unchanged, no new rule code appeared, and no unrelated blocker family increased.

### Current Parent State

- Current blocker-family totals after `WPORG-28R-G8-T1`: DB/SQL `1034`, nonce/input `0`, date/time `25`, and logging `42`.
- `WPORG-28R-G8-T1` is terminal under `verified`.
- `WPORG-28R-G8` remains `in progress` with `185` rows across `39` boundaries; the remaining G8 rule mix is `UnescapedDBParameter 29`, `DirectQuery 54`, `NoCaching 46`, `InterpolatedNotPrepared 30`, `NotPrepared 10`, `slow_meta_key 7`, `slow_meta_query 6`, and `slow_meta_value 3`.
- Exact next implementation child at the time of this slice: `WPORG-28R-G8-T2 — Task-instance assignment, supersession, and staff-portal read models`.
- `WPORG-28R` remained blocked at the time of this slice until `WPORG-28R-G8-T2` through `WPORG-28R-G17` could close and a fresh packaged strict-json rerun could prove `SUBMISSION_BLOCKER=0`.

### Non-Actions

- No runtime outside the authorized mirror/live Staff Tasks files, no unrelated live-tree region, and no package metadata or release exclusions were changed in this child.
...
- Result: `PASS`
- Exact finding identifier: `WPORG-28R-G8-T2`
- Scope completed in this slice: only the task-instance assignment, supersession, and staff-portal read-model repositories in `includes/modules/staff-tasks/store.php` and `includes/portal/staff-portal.php`, plus the synchronized live counterparts under `../../vms/` and the directly invalidated support inventory update in `tests/staff-tasks-repository-sql-remediation.php`
- Owned packaged rows: `53 -> 0`
- Distinct boundaries: `12 -> 0`
- Exact rule distribution: `DirectQuery 15`, `InterpolatedNotPrepared 8`, `NoCaching 14`, `NotPrepared 6`, `UnescapedDBParameter 8`, `slow_meta_key 1`, and `slow_meta_value 1`
- Evidence directory: `/tmp/wporg-28rg8-t2-work.54u1i4`
- Mirror/live boundary: `store.php` stayed byte-identical across mirror/live after the edits, `staff-portal.php` still diverges only outside the owned portal helpers, and `/tmp/wporg-28rg8-t2-work.54u1i4/t2-live-untouched-regions.tsv` proves every untouched live region stayed preserved

### Runtime And Test Changes

- Normalized the task-instance single-row, filtered-list, filtered-count, recurrence, de-duplication, and supersession reads to inline prepared `%i` custom-table queries with line-local `DirectQuery` / `NoCaching` / `UnescapedDBParameter` justification while preserving the existing lifecycle, ordering, and filter semantics.
- Left the task-instance insert plus assignment and status mutations on the plugin-owned custom table, but documented the narrow `wpdb::insert()` / `wpdb::update()` direct-query justifications line-locally so the repository contract stays explicit without widening the scope.
- Reworked `vms_tasks_resolve_scheduled_role_user_id()` so the staffing join stays prepared on the custom staffing tables and the bounded reverse-link fallback now uses a prepared `usermeta` read instead of the former `get_users()` meta query, removing the owned `slow_meta_key` and `slow_meta_value` occurrences without changing the surrounding ownership or portal behavior.
- Normalized `vms_staff_portal_get_event_crew_rows()` and `vms_staff_portal_get_assignment_rows()` to inline prepared join queries while preserving the authenticated staff identity, event-plan context, limit, ordering, and response shape.
- Added `tests/staff-task-instance-and-portal-repository-sql-remediation.php` and narrowed the historical `tests/staff-tasks-repository-sql-remediation.php` PHPCS inventory assertion to the exact `T1`-owned subset so the new same-file `T2` suppressions stay supported without reopening the `T1` verification contract.

### Verification

- Focused suites passed: `php tests/staff-task-instance-and-portal-repository-sql-remediation.php` and `php tests/staff-tasks-repository-sql-remediation.php`.
...
- PHP lint passed for mirror/live `store.php` and `staff-portal.php`, plus `tests/staff-task-instance-and-portal-repository-sql-remediation.php` and `tests/staff-tasks-repository-sql-remediation.php`.
- Post-edit package command: `php scripts/build-public-release.php --output-dir '/tmp/wporg-28rg8-t2-work.54u1i4/post-build' --allow-dirty --force`
- Post-edit package result: `/tmp/wporg-28rg8-t2-work.54u1i4/post-build/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `ff397a96d1e77075910683dbec028291d71cbd2ad825f9585173a12c97815348`
- Post-edit packaged Plugin Check command: `php -d error_reporting=0 -d display_errors=0 /opt/homebrew/bin/wp --path='/Users/treyconey/Local Sites/serenade-range-local-test-site/app/public' --skip-plugins=event-tickets,event-tickets-plus,the-events-calendar,woocommerce,woocommerce-square,vms plugin check '/tmp/wporg-28rg8-t2-work.54u1i4/plugin-check-post/extracted.1785642363/backstage-venue-manager' --slug=backstage-venue-manager --mode=new --format=strict-json --fields=file,line,column,type,code,message,docs`
- Post-edit packaged Plugin Check result: exit `0`, empty stderr, `246` errors, `931` warnings, `1177` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1048`; `plugin-list.cmp=0`, and the final strict JSON is `/tmp/wporg-28rg8-t2-work.54u1i4/plugin-check-post/plugin-check.strict.json`.
- Exact T2 code deltas: `UnescapedDBParameter 136 -> 128 (-8)`, `DirectQuery 282 -> 267 (-15)`, `NoCaching 248 -> 234 (-14)`, `InterpolatedNotPrepared 131 -> 123 (-8)`, `NotPrepared 60 -> 54 (-6)`, `slow_meta_key 69 -> 68 (-1)`, and `slow_meta_value 8 -> 7 (-1)`; every other packaged rule-code count stayed unchanged, no new rule code appeared, and no unrelated blocker family increased.
- Current packaged target-row audit at `/tmp/wporg-28rg8-t2-work.54u1i4/plugin-check-post/target-row-summary.tsv` proves both `t1_rows=0` and `t2_rows=0`.

### Current Parent State

- Current blocker-family totals after `WPORG-28R-G8-T2`: DB/SQL `981`, nonce/input `0`, date/time `25`, and logging `42`.
- `WPORG-28R-G8-T1` and `WPORG-28R-G8-T2` are both terminal under `verified`.
- `WPORG-28R-G8` remains `in progress` with `132` rows across `27` boundaries; the remaining G8 rule mix is `UnescapedDBParameter 21`, `DirectQuery 39`, `NoCaching 32`, `InterpolatedNotPrepared 22`, `NotPrepared 4`, `slow_meta_key 6`, `slow_meta_query 6`, and `slow_meta_value 2`.
- Exact next implementation child: `WPORG-28R-G8-T3 — Staffing template and event-slot repositories`.
- Preserved mapping note: the preserved `g8-boundaries.tsv` still assigns `G8-B002` to `vms_staff_user_link_metabox_render()` lines `26-79`, but the current packaged slow-meta query is the later `save_post_vms_staff` closure at line `131`; that stale mapping does not affect `T1` or `T2` ownership counts.
- `WPORG-28R` remains blocked until `WPORG-28R-G8-T3` through `WPORG-28R-G17` close and a fresh packaged strict-json rerun proves `SUBMISSION_BLOCKER=0`.
...
- Result: `PASS`
- Exact finding identifier: `WPORG-28R-G8-T5`
- Scope completed in this slice: only the slow-meta-query and reverse-link fallback repositories in mirror/live `includes/admin/staff-list-columns.php`, `includes/admin/staff-user-link.php`, `includes/admin/staff-vendor-link.php`, `includes/admin/vendor-staff-link.php`, `includes/core/staffing.php`, `includes/modules/staff-tasks/admin-ui.php`, and `includes/modules/staff-tasks/generator.php`, plus the new focused suite `tests/staffing-final-repository-sql-remediation.php` and the continuity inventory updates in `tests/staffing-matrix-rollup-reporting-repository-sql-remediation.php` and `tests/staffing-repository-sql-remediation.php`
- Owned packaged rows: `14 -> 0`
- Distinct boundaries: `9 -> 0`
- Exact rule distribution: `slow_meta_key 6`, `slow_meta_query 6`, and `slow_meta_value 2`
- Evidence directory: `/tmp/wporg-28rg8-t5-work.ctlcQI`
- Mirror/live boundary: `includes/admin/staff-list-columns.php`, `includes/admin/staff-user-link.php`, `includes/admin/staff-vendor-link.php`, and `includes/modules/staff-tasks/generator.php` stayed byte-identical across mirror/live after sync, while `includes/admin/vendor-staff-link.php`, `includes/core/staffing.php`, and `includes/modules/staff-tasks/admin-ui.php` still diverge only outside the owned T5 boundaries and the focused parity harness proves the owned bundles remained identical

### Runtime And Test Changes

- Replaced the remaining reverse-link slow-meta fallbacks in `vms_staff_admin_list_linked_user_id()`, the `save_post_vms_staff` closure, the `save_post_vms_vendor` closure, and the vendor/staff duplicate-link cleanup closure with prepared direct `usermeta` or `postmeta` reads so the bounded fallback behavior stays intact without `get_users()` or `get_posts()` meta queries.
- Reworked the remaining staffing-family dashboard, rebuild, and generator date reads to prepared direct `posts` and `postmeta` queries, including a single literal prepared rebuild query that keeps the date window, venue filter, status set, and rollup behavior unchanged while removing the final slow-meta-query findings.
- Updated the Staff Tasks admin fallback selector to fetch IDs without meta ordering, hydrate event dates, drop empty dates, and preserve the same event ordering in PHP so the bounded read stays behaviorally stable without the former slow `meta_key` ordering path.
- Added `tests/staffing-final-repository-sql-remediation.php` to classify the exact `T1` through `T5` suppression inventories, reject unknown or invented suppressions, enforce the final staffing-family mirror/live parity model, and exercise the T5 runtime contracts without reopening any earlier child inventory.

### Verification

- Focused suites passed: `php tests/staffing-final-repository-sql-remediation.php`, `php tests/staffing-matrix-rollup-reporting-repository-sql-remediation.php`, `php tests/staffing-repository-sql-remediation.php`, `php tests/staff-task-instance-and-portal-repository-sql-remediation.php`, and `php tests/staff-tasks-repository-sql-remediation.php`.
- Support reruns passed: `php tests/staff-tasks-overrides-json-remediation.php`, `php tests/staff-tasks-signature-json-remediation.php`, `php tests/vendor-portal-availability-autosave-remediation.php`, `php tests/authorization-boundary-hardening.php`, `php tests/runtime-stub-guards.php`, `php tests/release-compatibility-harness.php`, and `php tests/public-release-build-pipeline.php`.
- PHP lint passed for mirror/live `includes/admin/staff-list-columns.php`, `includes/admin/staff-user-link.php`, `includes/admin/staff-vendor-link.php`, `includes/admin/vendor-staff-link.php`, `includes/core/staffing.php`, `includes/modules/staff-tasks/admin-ui.php`, and `includes/modules/staff-tasks/generator.php`, plus `tests/staffing-final-repository-sql-remediation.php`, `tests/staffing-matrix-rollup-reporting-repository-sql-remediation.php`, and `tests/staffing-repository-sql-remediation.php`.
- Post-edit package command: `php scripts/build-public-release.php --output-dir '/tmp/wporg-28rg8-t5-work.ctlcQI/post-build' --allow-dirty --force`
- Post-edit package result: `/tmp/wporg-28rg8-t5-work.ctlcQI/post-build/backstage-venue-manager-1.2.0-public-release.zip` with SHA-256 `09e768d6afd87d31530436729362d68848e29a20c117993e16f81eadf8562fe8`
- Post-edit packaged Plugin Check command: `php -d error_reporting=0 -d display_errors=0 /opt/homebrew/bin/wp --path='/Users/treyconey/Local Sites/serenade-range-local-test-site/app/public' --skip-plugins=event-tickets,event-tickets-plus,the-events-calendar,woocommerce,woocommerce-square,vms plugin check '/tmp/wporg-28rg8-t5-work.ctlcQI/post-plugin-check/extracted.7M7tmp/backstage-venue-manager' --slug=backstage-venue-manager --mode=new --format=strict-json --fields=file,line,column,type,code,message,docs`
- Post-edit packaged Plugin Check result: exit `0`, empty stderr, `239` errors, `806` warnings, `1045` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=916`; the raw strict JSON is `/tmp/wporg-28rg8-t5-work.ctlcQI/post-plugin-check/plugin-check.stdout.raw`, and the plugin activation-state snapshots at `/tmp/wporg-28rg8-t5-work.ctlcQI/post-plugin-check/plugin-list.before.json` and `/tmp/wporg-28rg8-t5-work.ctlcQI/post-plugin-check/plugin-list.after.json` remained byte-identical.
- Exact `T5` code deltas: `slow_meta_key 68 -> 62 (-6)`, `slow_meta_query 93 -> 87 (-6)`, and `slow_meta_value 7 -> 5 (-2)`; every other packaged rule-code count stayed unchanged, no new rule code appeared, and no unrelated blocker family increased.
- Ownership reconciliation at `/tmp/wporg-28rg8-t5-work.ctlcQI/analysis/post-edit/ownership-reconciliation.txt` proves `t5_rows=0`, `t5_boundaries=0`, `g8_db_rows=0`, and `non_t5_g8_rows=0`.

### Current Parent State

- Current blocker-family totals after `WPORG-28R-G8-T5`: DB/SQL `849`, nonce/input `0`, date/time `25`, and logging `42`.
- `WPORG-28R-G8-T1`, `WPORG-28R-G8-T2`, `WPORG-28R-G8-T3`, `WPORG-28R-G8-T4`, and `WPORG-28R-G8-T5` are terminal under `verified`.
- `WPORG-28R-G8` is now terminal under `verified` with `233 -> 0` rows and `52 -> 0` boundaries.
- Exact next implementation child: `WPORG-28R-G9 — Admissions And Claim-State Query Boundaries`.
- Preserved mapping note: the preserved `g8-boundaries.tsv` still assigns `G8-B002` to `vms_staff_user_link_metabox_render()` lines `26-79`, but the current packaged slow-meta query is the later `save_post_vms_staff` closure at line `131`; the terminal post-`T5` G8 summary therefore uses `/tmp/wporg-28rg8-t5-work.ctlcQI/scope/current-boundary-ranges.tsv`, and the stale mapping does not affect `T1` through `T5` ownership counts.
- `WPORG-28R` remains blocked until `WPORG-28R-G9` through `WPORG-28R-G17` close and a fresh packaged strict-json rerun proves `SUBMISSION_BLOCKER=0`.
...
- Result: `PASS`
- Scope: ticket-inventory forensics and mutation-audit DB/SQL, vendor-portal DB/SQL, vendor-schema descriptor false positives, and the check-in-close returned descriptor false positive.
- Primary source commit scanned: `5b6adb0c0505a7284d7d47535f2be85020ee46d8`.
- Primary runtime commits: `7c682b5` (schema/check-in descriptors), `568320e` (vendor portal), and `5b6adb0` (ticket diagnostics).
- Durable current counts and provenance: `docs/wporg-current-scan-state.json`.

### Runtime Changes

- `includes/ticketing/ticket-inventory-forensics.php` plus `includes/ticketing/ticket-mutation-audit.php` DB/SQL moved `27 -> 0`. Custom table identifiers now use `%i`; value placeholders, 90-day pruning, fresh DESC history reads, bounded limits, insert formats, failure/zero-row semantics, and return identities remain unchanged. Mutation Audit's separate `error_log` and `debug_backtrace` rows remain deferred.
- `includes/portal/vendor-portal.php` DB/SQL moved `22 -> 0` through exact annotations on four already-prepared fresh custom-table operations and seven required portal meta-query boundaries. No SQL, query arguments, caching, authentication, authorization, or output behavior changed.
- `includes/core/registry/vendor-schema.php` descriptor rows moved `28 -> 0`, and the returned `meta_key` descriptor in `includes/helpers/checkin-close.php` moved `1 -> 0`. These are annotation-only lexical false-positive remediations; neither boundary executes a query.
- Shared runtime was synchronized to the real local plugin only inside the reviewed ownership boundaries. Full parity was retained for the two ticket-diagnostics files, vendor schema, and check-in close; the intentionally divergent vendor-portal file was patched only at the exact owned occurrences.

### Verification

- New executable focused suites passed: `tests/ticket-diagnostics-repository-sql-remediation.php`, `tests/vendor-portal-repository-sql-remediation.php`, and `tests/vendor-schema-descriptor-slow-query-remediation.php`.
- Independent reviewers reconciled each slice to the Wave 2 strict JSON, reproduced the parent findings with installed PHPCS, proved current target findings zero, audited annotation truthfulness and behavior, and returned clean approvals before integration.
- Primary-tree focused tests, mirror/live lints and parity checks, release-compatibility harness self-tests, runtime-stub guards, and all `18` public-release pipeline cases passed.
- Authoritative public build passed at `/tmp/wporg-wave3-integrated.ZxiaOt/build-authoritative/backstage-venue-manager-1.2.0-public-release.zip`, SHA-256 `56dbf1983afaff8d9bb9a4f47a27363aa5300e74147d573a6fed82fc50e94e7d`.
...
- Final packaged Plugin Check exited `0` with empty stderr. Strict JSON is `/tmp/wporg-wave3-integrated.ZxiaOt/plugin-check/plugin-check.strict.json`, SHA-256 `3fcc51cd1311afa47bac1b4cc9c958a38293ec86f5a2b78ec14127a7259ef38a`; before/after activation-state snapshots are byte-identical and their stderr files are empty.
- Final scan: `166` errors, `195` warnings, `361` total, `14` unique codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=125`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=236`.
- Final blocker families: DB/SQL `169`, nonce/input `0`, date/time `25`, and logging `42`.
- Exact Wave 3 deltas were `UnescapedDBParameter 17 -> 12 (-5)`, `DirectQuery 42 -> 31 (-11)`, `NoCaching 35 -> 26 (-9)`, `InterpolatedNotPrepared 14 -> 9 (-5)`, `slow_meta_key 58 -> 17 (-41)`, and `slow_meta_query 66 -> 59 (-7)`; every other rule count remained unchanged.

### Current Parent State

- `WPORG-28R-G10` remains `in progress` with `45` DB/SQL rows after the verified ticket-diagnostics slice.
...
## G16 Operational Logging Packaged Checkpoint
...
- Result: `PASS`
- Source commit: `d4079b0bf2c8affb1f50ac587ddb858aa94197e2`
- Clean public package: `/tmp/wporg-g16-checkpoint-final.aOSh8U/build/backstage-venue-manager-1.2.0-public-release.zip`
- Package SHA-256: `6a9b45cd6e2cbb1fd6723f064b1c867a425b99c8604750791badfedfb2f62798`
- Clean build completed at `2026-08-08T19:52:45+00:00`.
- Canonical strict JSON: `/tmp/wporg-g16-checkpoint-final.aOSh8U/plugin-check.strict.json`
- Strict-JSON SHA-256: `b0ebbddec1d17ce9a8770ae9ec385665f49962c6ebc1a3f2f1520e81d281b49c`
- Plugin Check exited `0`; scanner stderr is empty.
- Current scan: `125` errors, `16` warnings, `141` total findings, `5` unique codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=125`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=16`.
- Current blocker families: DB/SQL `0`, nonce/input `0`, date/time `0`, logging `16`.
...
- `WPORG-28R-G16` is scanner-verified complete: runtime guards `2 -> 0`, Group A `11 -> 0`, Group B `9 -> 0`, and Group C `4 -> 0`. `WPORG-28R-G17` owns the exact remaining `16` developer-diagnostic rows, including the sole `debug_backtrace` row.
...
- An initial G16 checkpoint scan exposed one `WordPress.WP.AlternativeFunctions.parse_url_parse_url` blocker in the new bounded adapter. Commit `d4079b0bf2c8affb1f50ac587ddb858aa94197e2` replaced that call with `wp_parse_url()`, and the final package contains only the five expected codes with no alternative-function finding.
- Mirror/live synchronization was verified at every owned G10/G11/G13/G14/G15/G16 boundary while preserving intentional structural divergence outside those boundaries.
