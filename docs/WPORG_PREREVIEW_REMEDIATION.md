...
- Ordering and untouched neighboring boundaries:
  ...
  - Guided Tours remained unchanged in that slice as a nested tour-specific content boundary; a later dedicated `WPORG-24J` pass closes that separate reset-notice family without changing shell behavior. Schedule remained unchanged as a nested content boundary with richer coexistence.
...
- Local bootstrap limitation: because the local WordPress bootstrap still loads the installed sibling `vms` plugin instead of `packages/vms-github-reconcile`, this slice stayed package-source / direct-renderer based and did not copy the shared runtime file into the installed tree.
- Status: `WPORG-24` remains open. This accepted `WPORG-24K` slice closes only the Staff Certifications pending-review warning family; the separate entitlement image-sync structured-result family, the shared Administrator-shell raw `$captured_notices_html` / `$content_html` boundaries, the separate Schedule content-local notice family, the Social Sharing event-panel lazy-load HTML family, and broader Event Plans partial / AJAX output boundaries remain separate.
...
- Finite markup contract and unchanged boundaries:
  ...
  - `includes/admin-ui/shell.php`, Administrator-shell allowlists, raw `$captured_notices_html`, raw `$content_html`, raw `$actions_html`, the Schedule no-selection warning, the Schedule no-venues warning, the richer unpublished-venue/action-required notice families, Schedule calculations, queries, filters, grouping, ordering, URLs, and ordinary body markup remain unchanged.
...
- Local bootstrap limitation: because the local WordPress bootstrap still loads the installed sibling `vms` plugin instead of `packages/vms-github-reconcile`, this slice stayed package-source / direct-renderer based and did not copy the shared runtime file into the installed tree.
- Status: `WPORG-24` remains open. This accepted `WPORG-24M` slice closes only the repeated Schedule invalid-bounds family; the separate entitlement image-sync structured-result family, the shared Administrator-shell raw `$captured_notices_html` / `$content_html` boundaries, the remaining Schedule no-selection/no-venues warning and unpublished-venue rich notice families, the Social Sharing event-panel lazy-load HTML family, and broader Event Plans partial / AJAX output boundaries remain separate.
