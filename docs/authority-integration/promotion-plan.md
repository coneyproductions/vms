...
3. Require process exit `0` AND JSON `ok: true`. Any orphan, active duplicate, invalid confirmed window, overlapping commitment, incompatible lifecycle addition, non-InnoDB/missing transaction table, SQL failure or malformed receipt stops the workflow **before backup/source replacement/migration**. Inactive duplicate history remains evidence and is preserved. A missing additive column/index means `lifecycle_migration_required`, which is allowed only when every data/engine gate passes. The expanded gate also covers unassigned orphan slots, role/value/status references, invalid or duplicate caches, base identities/auto-increments and revision/request-ID prerequisites. Missing optional proposed windows remain supported; existing-plan canceled/trash slots, canceled cached context, terminal duplicate history and audit-only references are reported without blanket deletion. The population retention policy defines these distinctions; no automatic cleanup is part of this gate.
...
September 6 resumed validation completed the evidence-backed 165-slot/100-cache
cleanup without normal-local migration or source replacement. The candidate also
fixes MySQL transaction setup ordering: schema inspection must precede the
next-transaction isolation declaration, which must be immediately followed by
START TRANSACTION. This preserves actual READ COMMITTED behavior under MySQL
8.0.35; the independent-connection visibility regression is part of acceptance.
Use the resulting validated integration commit, including resource-supervised
rehearsal receipts, rather than the earlier draft. Fresh promotion authorization,
a fresh gate, writer freeze, source backup and post-migration acceptance remain
required. This validation does not execute that promotion.
