...
- Fresh packaged post-`WPORG-28R-G8-T4` baseline from `2026-08-02`: `239` errors, `820` warnings, `1059` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=930`.
...
- `WPORG-28R-G8-T4` removed the next `56` DB/SQL blocker rows without changing any non-DB blocker-family total; the global DB/SQL deltas were `UnescapedDBParameter 117 -> 107 (-10)`, `DirectQuery 246 -> 228 (-18)`, `NoCaching 218 -> 202 (-16)`, `InterpolatedNotPrepared 113 -> 101 (-12)`, and `NotPrepared 50 -> 50 (0)`.
...
| Database and SQL safety | `863` | `includes/core/staffing.php`, `includes/modules/admissions/pass-claims.php`, `includes/modules/availability-date-dispatch/helpers.php`, `includes/social-share/queue-repo.php` | `SUBMISSION_BLOCKER` | Continue with `WPORG-28R-G8-T5` through `WPORG-28R-G13`; finish the slow-meta-query and reverse-link slice before the broader reporting/schema tail. |
...
- Family sums reconcile exactly: `G8-G13 = 925`, `G14-G15 = 25`, `G16-G17 = 42`, grand total `992`. The nonce/input family is now closed at `0`.
...
- Status: `in progress` on `2026-08-02` after verified children `WPORG-28R-G8-T1`, `WPORG-28R-G8-T2`, `WPORG-28R-G8-T3`, and `WPORG-28R-G8-T4` preserved at `/tmp/wporg-28rg8-t1-work.2rY5sN`, `/tmp/wporg-28rg8-t2-work.54u1i4`, `/tmp/wporg-28rg8-t3-work.qiAdPz`, and `/tmp/wporg-28rg8-t4-work.4CeoYs`
- Original pre-edit count: `233` (`UnescapedDBParameter 36`, `DirectQuery 70`, `NoCaching 58`, `InterpolatedNotPrepared 36`, `NotPrepared 17`, `slow_meta_key 7`, `slow_meta_query 6`, `slow_meta_value 3`); current remaining count: `14` (`slow_meta_key 6`, `slow_meta_query 6`, `slow_meta_value 2`)
- Distinct boundaries: `52 -> 9`
...
- Next execution target: `WPORG-28R-G8-T5 — Slow meta queries and reverse-link fallbacks`
- Scope: execute the remaining `14`-row / `9`-boundary slow-meta-query and reverse-link fallback slice in `includes/admin/staff-list-columns.php`, `includes/admin/staff-user-link.php`, `includes/admin/staff-vendor-link.php`, `includes/admin/vendor-staff-link.php`, `includes/core/staffing.php`, `includes/modules/staff-tasks/admin-ui.php`, and `includes/modules/staff-tasks/generator.php` now that `WPORG-28R-G8-T4` is verified and the G8 remainder is down to `14` rows across `9` boundaries.
- Scope guardrails: do not reopen `G1` through `G6`, verified children `T1` through `T4`, or retired `G7`; keep the remaining slow-meta decisions narrow, preserve the current staffing/reporting behavior, and continue deferring the documented later DB/SQL, date/time, and logging work to their assigned children.
