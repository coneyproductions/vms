...
- Fresh packaged post-`WPORG-28R-G8-T3` baseline from `2026-08-02`: `239` errors, `876` warnings, `1115` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=986`.
...
- `WPORG-28R-G8-T3` removed the next `62` DB/SQL blocker rows without changing any non-DB blocker-family total; the global DB/SQL deltas were `UnescapedDBParameter 128 -> 117 (-11)`, `DirectQuery 267 -> 246 (-21)`, `NoCaching 234 -> 218 (-16)`, `InterpolatedNotPrepared 123 -> 113 (-10)`, and `NotPrepared 54 -> 50 (-4)`.
...
| Database and SQL safety | `919` | `includes/core/staffing.php`, `includes/modules/admissions/pass-claims.php`, `includes/modules/availability-date-dispatch/helpers.php`, `includes/social-share/queue-repo.php` | `SUBMISSION_BLOCKER` | Continue with `WPORG-28R-G8-T4` through `WPORG-28R-G13`; finish the staffing matrix, rollup, and reporting slice before the later slow-meta-query follow-up. |
...
- Family sums reconcile exactly: `G8-G13 = 981`, `G14-G15 = 25`, `G16-G17 = 42`, grand total `1048`. The nonce/input family is now closed at `0`.
...
- Status: `in progress` on `2026-08-02` after verified children `WPORG-28R-G8-T1`, `WPORG-28R-G8-T2`, and `WPORG-28R-G8-T3` preserved at `/tmp/wporg-28rg8-t1-work.2rY5sN`, `/tmp/wporg-28rg8-t2-work.54u1i4`, and `/tmp/wporg-28rg8-t3-work.qiAdPz`
- Original pre-edit count: `233` (`UnescapedDBParameter 36`, `DirectQuery 70`, `NoCaching 58`, `InterpolatedNotPrepared 36`, `NotPrepared 17`, `slow_meta_key 7`, `slow_meta_query 6`, `slow_meta_value 3`); current remaining count: `70` (`UnescapedDBParameter 10`, `DirectQuery 18`, `NoCaching 16`, `InterpolatedNotPrepared 12`, `slow_meta_key 6`, `slow_meta_query 6`, `slow_meta_value 2`)
- Distinct boundaries: `52 -> 15`
...
- Focused tests: `tests/staffing-repository-sql-remediation.php`, `tests/staff-task-instance-and-portal-repository-sql-remediation.php`, `tests/staff-tasks-repository-sql-remediation.php`, `tests/staff-tasks-overrides-json-remediation.php`, `tests/staff-tasks-signature-json-remediation.php`, `tests/authorization-boundary-hardening.php`, `tests/vendor-portal-availability-autosave-remediation.php`
- Fresh baseline evidence: `2026-08-02` public package and packaged strict-json reruns reproduced `239` errors, `876` warnings, `1115` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=986`; blocker-family totals now stand at `DB/SQL 919`, `nonce/input 0`, `date/time 25`, and `logging 42`, with empty build/plugin-check stderr.
- Decomposition:
  ...
  `WPORG-28R-G8-T4 — Staffing matrix, rollup, and reporting repositories` now owns `56` rows across `6` boundaries in `includes/core/staffing.php` and `includes/admin/staffing.php`; it is now the exact next implementation child because it mixes the riskiest staffing mutations, recompute lifecycles, and reporting reads.
...
- Mirror/live audit: the original G8 inventory still shows `6` of the `11` owned files byte-identical end to end and the other `5` function-identical only; the T1-specific after-edit audit at `/tmp/wporg-28rg8-t1-work.2rY5sN/t1-mirror-live.after.tsv` confirms `store.php` and `db.php` remained byte-identical across mirror/live while `admin-ui.php` still diverges only outside the owned selector helper, the T2-specific after-edit audits at `/tmp/wporg-28rg8-t2-work.54u1i4/t2-mirror-live.after.tsv`, `/tmp/wporg-28rg8-t2-work.54u1i4/t2-target-functions.after.tsv`, and `/tmp/wporg-28rg8-t2-work.54u1i4/t2-live-untouched-regions.tsv` confirm the `store.php` target file stayed byte-identical across mirror/live and `staff-portal.php` still diverges only outside the owned portal helpers, and the T3-specific after-edit audit at `/tmp/wporg-28rg8-t3-work.qiAdPz/post-edit-structure-final.1785651724` plus `/tmp/wporg-28rg8-t3-work.qiAdPz/function-parity-mirror.final.php` and `/tmp/wporg-28rg8-t3-work.qiAdPz/function-parity-live.final.php` confirm `staffing.php` still diverges only outside the owned template/event-slot helpers while every extracted T3 target function remained identical after sync.
- Preserved mapping note: the preserved `g8-boundaries.tsv` still assigns `G8-B002` to `vms_staff_user_link_metabox_render()` lines `26-79`, but the current packaged slow-meta query is the later `save_post_vms_staff` closure at line `131`; the stale mapping does not affect `T1`, `T2`, or `T3` ownership counts.
- Closure / defers: owned packaged DB rows `233 -> 70`; verified children `T1`, `T2`, and `T3` removed `163` rows across `37` boundaries, leaving `70` rows across `15` boundaries for `T4` and `T5`; the future completion target after a successful full G8 closeout remains `849` remaining DB/SQL blockers and `916` projected overall blockers; defer same-file logging in `includes/modules/staff-tasks/generator.php` to `G17`
...
- Next execution target: `WPORG-28R-G8-T3 — Staffing template and event-slot repositories`
- Scope: execute the next `62`-row / `12`-boundary staffing template and event-slot repository slice in `includes/core/staffing.php` now that `WPORG-28R-G8-T1` and `WPORG-28R-G8-T2` are verified and the G8 remainder is down to `132` rows across `27` boundaries.
- Scope guardrails: do not reopen `G1` through `G6`, verified children `T1` or `T2`, or retired `G7`; keep `T4` and `T5` for later commits, and continue deferring the documented later DB/SQL, date/time, and logging work to their assigned children.
