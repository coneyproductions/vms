...
- Fresh packaged post-`WPORG-28R-G8-T2` baseline from `2026-08-02`: `246` errors, `931` warnings, `1177` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1048`.
...
- `WPORG-28R-G8-T2` removed the next `53` DB/SQL blocker rows without changing any non-DB blocker-family total; the global DB/SQL deltas were `UnescapedDBParameter 136 -> 128 (-8)`, `DirectQuery 282 -> 267 (-15)`, `NoCaching 248 -> 234 (-14)`, `InterpolatedNotPrepared 131 -> 123 (-8)`, `NotPrepared 60 -> 54 (-6)`, `slow_meta_key 69 -> 68 (-1)`, and `slow_meta_value 8 -> 7 (-1)`.
...
| Database and SQL safety | `981` | `includes/core/staffing.php`, `includes/modules/admissions/pass-claims.php`, `includes/modules/availability-date-dispatch/helpers.php`, `includes/social-share/queue-repo.php` | `SUBMISSION_BLOCKER` | Continue with `WPORG-28R-G8-T3` through `WPORG-28R-G13`; finish the staffing repository slice before the later staffing matrix, reporting, and slow-meta-query follow-up. |
...
- Status: `in progress` on `2026-08-02` after verified children `WPORG-28R-G8-T1` and `WPORG-28R-G8-T2` preserved at `/tmp/wporg-28rg8-t1-work.2rY5sN` and `/tmp/wporg-28rg8-t2-work.54u1i4`
- Original pre-edit count: `233` (`UnescapedDBParameter 36`, `DirectQuery 70`, `NoCaching 58`, `InterpolatedNotPrepared 36`, `NotPrepared 17`, `slow_meta_key 7`, `slow_meta_query 6`, `slow_meta_value 3`); current remaining count: `132` (`UnescapedDBParameter 21`, `DirectQuery 39`, `NoCaching 32`, `InterpolatedNotPrepared 22`, `NotPrepared 4`, `slow_meta_key 6`, `slow_meta_query 6`, `slow_meta_value 2`)
- Distinct boundaries: `52 -> 27`
...
- Focused tests: `tests/staff-task-instance-and-portal-repository-sql-remediation.php`, `tests/staff-tasks-repository-sql-remediation.php`, `tests/staff-tasks-overrides-json-remediation.php`, `tests/staff-tasks-signature-json-remediation.php`, `tests/authorization-boundary-hardening.php`, `tests/vendor-portal-availability-autosave-remediation.php`
- Fresh baseline evidence: `2026-08-02` public package and packaged strict-json reruns reproduced `246` errors, `931` warnings, `1177` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1048`; blocker-family totals now stand at `DB/SQL 981`, `nonce/input 0`, `date/time 25`, and `logging 42`, with empty build/plugin-check stderr.
- Decomposition:
  ...
  `WPORG-28R-G8-T3 — Staffing template and event-slot repositories` now owns the next `62` rows across `12` boundaries in `includes/core/staffing.php`; land it before `T4` because the later matrix and rollup work consumes the same template, slot, and assignment repository contracts.
...
- Mirror/live audit: the original G8 inventory still shows `6` of the `11` owned files byte-identical end to end and the other `5` function-identical only; the T1-specific after-edit audit at `/tmp/wporg-28rg8-t1-work.2rY5sN/t1-mirror-live.after.tsv` confirms `store.php` and `db.php` remained byte-identical across mirror/live while `admin-ui.php` still diverges only outside the owned selector helper, and the T2-specific after-edit audits at `/tmp/wporg-28rg8-t2-work.54u1i4/t2-mirror-live.after.tsv`, `/tmp/wporg-28rg8-t2-work.54u1i4/t2-target-functions.after.tsv`, and `/tmp/wporg-28rg8-t2-work.54u1i4/t2-live-untouched-regions.tsv` confirm the `store.php` target file stayed byte-identical across mirror/live, `staff-portal.php` still diverges only outside the owned portal helpers, and every untouched live region stayed preserved.
- Preserved mapping note: the preserved `g8-boundaries.tsv` still assigns `G8-B002` to `vms_staff_user_link_metabox_render()` lines `26-79`, but the current packaged slow-meta query is the later `save_post_vms_staff` closure at line `131`; the stale mapping does not affect `T1` or `T2` ownership counts.
- Closure / defers: owned packaged DB rows `233 -> 132`; verified children `T1` and `T2` removed `101` rows across `25` boundaries, leaving `132` rows across `27` boundaries for `T3` through `T5`; the future completion target after a successful full G8 closeout remains `849` remaining DB/SQL blockers and `916` projected overall blockers; defer same-file logging in `includes/modules/staff-tasks/generator.php` to `G17`
