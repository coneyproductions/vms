...
- Fresh packaged post-`WPORG-28R-G8-T1` baseline from `2026-08-01`: `254` errors, `976` warnings, `1230` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1101`.
...
- `WPORG-28R-G8-T1` removed the first `48` DB/SQL blocker rows without changing any non-DB blocker-family total; the global DB/SQL deltas were `UnescapedDBParameter 143 -> 136 (-7)`, `DirectQuery 298 -> 282 (-16)`, `NoCaching 260 -> 248 (-12)`, `InterpolatedNotPrepared 137 -> 131 (-6)`, and `NotPrepared 67 -> 60 (-7)`.
...
| Database and SQL safety | `1034` | `includes/core/staffing.php`, `includes/modules/admissions/pass-claims.php`, `includes/modules/availability-date-dispatch/helpers.php`, `includes/social-share/queue-repo.php` | `SUBMISSION_BLOCKER` | Continue with `WPORG-28R-G8-T2` through `WPORG-28R-G13`; finish the remaining Staff Tasks / staff-portal slice before the staffing repository, reporting, and slow-meta-query follow-up. |
...
- Family sums reconcile exactly: `G8-G13 = 1034`, `G14-G15 = 25`, `G16-G17 = 42`, grand total `1101`. The nonce/input family is now closed at `0`.
...
- Status: `in progress` on `2026-08-01` after verified child `WPORG-28R-G8-T1` preserved at `/tmp/wporg-28rg8-t1-work.2rY5sN`
- Original pre-edit count: `233` (`UnescapedDBParameter 36`, `DirectQuery 70`, `NoCaching 58`, `InterpolatedNotPrepared 36`, `NotPrepared 17`, `slow_meta_key 7`, `slow_meta_query 6`, `slow_meta_value 3`); current remaining count: `185` (`UnescapedDBParameter 29`, `DirectQuery 54`, `NoCaching 46`, `InterpolatedNotPrepared 30`, `NotPrepared 10`, `slow_meta_key 7`, `slow_meta_query 6`, `slow_meta_value 3`)
- Distinct boundaries: `52 -> 39`
...
- Focused tests: `tests/staff-tasks-repository-sql-remediation.php`, `tests/staff-tasks-overrides-json-remediation.php`, `tests/staff-tasks-signature-json-remediation.php`, `tests/authorization-boundary-hardening.php`, `tests/vendor-portal-availability-autosave-remediation.php`
- Fresh baseline evidence: `2026-08-01` public package and packaged strict-json reruns reproduced `254` errors, `976` warnings, `1230` total findings, `15` unique rule codes, `KNOWN_ACCEPTED=0`, `KNOWN_NONBLOCKING=129`, `NEW_FINDING=0`, `UNMAPPED=0`, and `SUBMISSION_BLOCKER=1101`; blocker-family totals now stand at `DB/SQL 1034`, `nonce/input 0`, `date/time 25`, and `logging 42`, with empty build/plugin-check stderr.
- Decomposition:
...
- Mirror/live audit: the original G8 inventory still shows `6` of the `11` owned files byte-identical end to end and the other `5` function-identical only; the T1-specific after-edit audit at `/tmp/wporg-28rg8-t1-work.2rY5sN/t1-mirror-live.after.tsv` confirms `store.php` and `db.php` remain byte-identical across mirror/live, `admin-ui.php` still diverges only outside the owned selector helper, and `/tmp/wporg-28rg8-t1-work.2rY5sN/t1-live-untouched-regions.tsv` preserved every untouched live region.
- Closure / defers: owned packaged DB rows `233 -> 185`; verified child `T1` removed `48` rows across `13` boundaries, leaving `185` rows across `39` boundaries for `T2` through `T5`; the future completion target after a successful full G8 closeout remains `849` remaining DB/SQL blockers and `916` projected overall blockers; defer same-file logging in `includes/modules/staff-tasks/generator.php` to `G17`
...
- Next execution target: `WPORG-28R-G8-T2 — Task-instance assignment, supersession, and staff-portal read models`
- Scope: execute the next `53`-row / `12`-boundary Staff Tasks and Staff Portal read-model slice in `includes/modules/staff-tasks/store.php` and `includes/portal/staff-portal.php` now that `WPORG-28R-G8-T1` is verified and the G8 remainder is down to `185` rows across `39` boundaries.
- Scope guardrails: do not reopen `G1` through `G6` or verified child `T1`, do not recreate a standalone `WPORG-28R-G7` nonce/input child, keep `T3` through `T5` for later commits, and continue deferring the documented later DB/SQL, date/time, and logging work to their assigned children.
